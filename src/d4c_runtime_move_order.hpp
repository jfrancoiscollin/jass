...
namespace jass::d4c_runtime_order {
...
inline std::array<std::atomic<std::uint64_t>, 4> g_predicted_rank{};
...
inline bool read_npy96(const std::string& path, std::array<double, WIDTH>& out,
    ...
    static_assert(std::endian::native == std::endian::little,
                  "D4c sealed npy runtime requires little-endian host");
...
inline const Runtime& runtime() {
    static const Runtime rt = [] {
        Runtime out;
        const char* path = std::getenv("JASS_D4C_RUNTIME_MODEL");
        if (!path) return out;
        std::string err;
        if (!read_npy96(path, out.beta, err)) {
            std::cerr << "D4C_RUNTIME_INVALID: " << err << '\n'; std::abort();
        }
...
inline double score(const Runtime& rt, int phase,
...
inline void reset_counters() noexcept {
    g_eligible_nodes.store(0); g_hoists.store(0);
    for (auto& v : g_predicted_rank) v.store(0);
}
...
inline void note_prediction(int rank0) noexcept {
    if (rank0 >= 0 && rank0 < 4) g_predicted_rank[static_cast<std::size_t>(rank0)].fetch_add(1, std::memory_order_relaxed);
}
...
inline std::uint64_t predicted_rank(std::size_t rank0) noexcept {
    return rank0 < 4 ? g_predicted_rank[rank0].load(std::memory_order_relaxed) : 0;
}
