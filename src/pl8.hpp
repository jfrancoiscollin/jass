...
struct Model {
    ...
    std::array<double, LATENT_WIDTH * INPUT_WIDTH> w1{};
...
public:
    ...
    std::array<double, INPUT_WIDTH> input(const Position& pos) const noexcept { return extractor_.extract(pos); }
private:
    FeatureExtractor extractor_;
    Model model_;
};

// PL8-local SHA implementation avoids any F6 runtime dependency while enforcing
// the preregistered byte-identical CURRICULUM identity at loader activation.
std::string sha256_file(const std::string& path, std::string* error = nullptr);

bool save_model(const std::string& path, const Model& model, std::string* error = nullptr);
std::optional<Model> load_model(const std::string& path, std::string* error = nullptr);
