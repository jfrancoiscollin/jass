...
//
// Experimental direct-decision (D) move-ordering head.
//
// IMPORTANT scientific contract:
//   * this policy NEVER contributes to leaf evaluation, alpha/beta values,
//     TT scores, pruning thresholds, or game adjudication;
//   * it only supplies a relative score among already-legal CAPTURE moves;
//   * unset JASS_TB_MOVE_ORDER_POLICY is a strict dormant path;
//   * a requested but malformed policy fails closed instead of silently
//     falling back to baseline.
//
// The 126 inputs reproduce jobs/tools/tb_frontier_pairwise.py exactly:
...
// One immutable policy per process. Scientific harnesses activate the candidate
// by launching the SAME binary through a wrapper that sets the env var; the
// baseline launches it directly with the variable absent.
inline const Policy* active() {
    static const std::optional<Policy> policy = []() -> std::optional<Policy> {
        const char* path = std::getenv("JASS_TB_MOVE_ORDER_POLICY");
        if (path == nullptr || *path == '\0') return std::nullopt;
        std::string error;
        auto loaded = load(path, &error);
        if (!loaded) {
            std::cerr << "TB_MOVE_ORDER_POLICY_ERROR path=" << path
                      << " reason=" << error << '\n';
            std::cerr.flush();
            std::exit(2);
        }
