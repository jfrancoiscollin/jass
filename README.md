...
- A negamax alpha-beta search with iterative deepening, transposition
  ...
  leaves.
- A long-lived `Engine` facade that owns the TT, the game's hash
  history and an opening book so successive moves of the same game
  share their lookup data, detect repetitions naturally and skip the
  search in the first few plies of book theory.
- A 2-vs-1 kings endgame bitbase generated by retrograde analysis
  (lazy build, ~0.5 s on first probe) that resolves WIN/LOSS/DRAW
  for every KKvK and KvKK position.
- An NNUE-style linear evaluation framework with a binary
  weight-loader; the default-constructed network mirrors the
  handcrafted eval, and trained weights can be dropped in without
  changing the rest of the engine.
- A self-play tournament harness (`./jass --tournament da db pairs`)
  for regression testing two engine configurations against each
  other.
- A small HUB-flavoured CLI front-end (`./build/jass`) for shell or
  GUI use, with `go depth | movetime | infinite`, `wtime/btime/winc/
  binc/movestogo` tournament time controls, `stop`,
  `setoption threads N`, and `bestmove ... pv=... book=1` output.
