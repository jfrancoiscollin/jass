...
#   run_pattern_reconstruction_cpx.sh m24p|m14p|m15p|m15c|m15c2probe|m15c2|m15c2rprobe|m15c2r|m15c3probe|m15c3|m15c4probe|m15c4|m17p|m17p2|m17p2r|m18p|m21p
#
...
cell=${1:?expected one of: m24p, m14p, m15p, m15c, m15c2probe, m15c2, m15c2rprobe, m15c2r, m15c3probe, m15c3, m15c4probe, m15c4, m17p, m17p2, m17p2r, m18p, m21p}
...
case "$cell" in
  ...
  m15c4)
    tool=run_pattern_conditional_dose_screen.py
    config=l1_pattern_conditional_residual_path.yaml
    timeout_seconds=43200
    ;;
  m15c4probe)
    tool=run_pattern_conditional_dose_screen.py
...
extra_args=()
if [[ "$cell" == "m15p" || "$cell" == "m15c" || "$cell" == "m15c2" || "$cell" == "m15c2r" || "$cell" == "m15c3" || "$cell" == "m15c4" || "$cell" == "m18p" || "$cell" == "m21p" ]]; then
  extra_args+=(--progress-output "$artefact_root/PROGRESS.json")
fi
if [[ "$cell" == "m15c2probe" || "$cell" == "m15c2rprobe" || "$cell" == "m15c3probe" || "$cell" == "m15c4probe" ]]; then
...
out = Path(sys.argv[2])
cell = sys.argv[3]
if cell in {"m15c2probe", "m15c2rprobe", "m15c3probe", "m15c4probe"}:
    summary = full
else:
    summary = {
        "schema": full["schema"],
