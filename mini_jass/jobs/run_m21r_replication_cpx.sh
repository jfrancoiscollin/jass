#!/usr/bin/env bash
# M21R — replication a graines fraiches, porte recablee sur l'ARENA.
set -Eeuo pipefail
...
oracle="$local_artefacts/oracle.l1.m21r-cpx.jsonl"
run_dir="$local_artefacts/runs/m21r-learning-signal-composition-cpx"
summary="$local_artefacts/m21r_learning_signal_composition.cpx.json"
...
"$python_bin" "$repo/mini_jass/tools/run_learning_signal_composition.py" \
  --config "$repo/mini_jass/configs/l1_learning_signal_composition_replication.yaml" \
  --oracle "$oracle" \
  --run-dir "$run_dir" \
