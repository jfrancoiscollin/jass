#!/usr/bin/env bash
# M15-C5 HOME entrypoint. Usage: ... probe|full
set -Eeuo pipefail
...
[[ "$job_id" == home-* && "$host" == User ]] || {
  echo "M15-C5 requires HOME (job=$job_id host=$host)" >&2
  exit 2
}
[[ "$(nproc)" -eq 16 ]] || {
  echo "M15-C5 sizing is valid only for HOME nproc=16" >&2
  exit 2
}

work="$result_root/mini-jass-pattern-m15c5-$mode"
build="$work/build"
run_dir="$work/run"
full_result="$work/result.full.json"
oracle="$repo/mini_jass/artefacts/oracle.l1.pattern-m15c5-$job_id.jsonl"
...
# 10 GiB leaves room for build, oracle, per-seed JSON and runner publication.
available_kib=$(df -Pk "$result_root" | awk 'NR==2 {print $4}')
[[ "$available_kib" -ge 10485760 ]] || {
  echo "M15-C5 disk guard: ${available_kib} KiB available, need 10485760" >&2
  exit 3
}
[[ -x "$venv/bin/python" ]] || {
...
timeout -k 60s "${timeout_seconds}s" \
  "$python_bin" "$repo/mini_jass/tools/run_pattern_conditional_feedback.py" \
    --config "$repo/mini_jass/configs/l1_pattern_conditional_feedback.yaml" \
...
if mode == "probe":
    if full.get("status") != "TIMING_ONLY" or full.get("scientific_metrics_published") is not False:
        raise SystemExit("invalid M15-C5 timing probe")
    summary = full
    lines = [
        "cell=m15c5probe",
    ...
    if int(full.get("aggregate", {}).get("paired_seed_count", 0)) == 0:
        raise SystemExit("M15-C5 n=0 is a hard failure")
    summary = {
        "schema": full["schema"],
