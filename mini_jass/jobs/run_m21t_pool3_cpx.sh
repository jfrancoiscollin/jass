#!/usr/bin/env bash
# M21T — troisieme pool disjoint, les deux criteres preinscrits.
set -Eeuo pipefail
...
oracle="$local_artefacts/oracle.l1.m21t-cpx.jsonl"
run_dir="$local_artefacts/runs/m21t-learning-signal-composition-cpx"
summary="$local_artefacts/m21t_learning_signal_composition.cpx.json"
...
"$python_bin" "$repo/mini_jass/tools/run_learning_signal_composition.py" \
  --config "$repo/mini_jass/configs/l1_learning_signal_composition_pool3.yaml" \
  --oracle "$oracle" \
  --run-dir "$run_dir" \
...
for key in ("single_pool_criterion_met", "chained_criterion_met",
            "criteria_agree", "chained_probability_above_zero"):
    if key in r:
        lines.append(f"{key}={r[key]}")
rep = r.get("replication") or {}
for pair in rep.get("pairwise_z", []):
    lines.append(f"pairwise_z_{pair['pools'][0]}_vs_{pair['pools'][1]}={pair['z']}")
for key in ("pool_count", "worst_absolute_z", "chained_z"):
    if key in rep:
        lines.append(f"replication_{key}={rep[key]}")
for key in ("prior_pool", "prior_mean", "replication_mean", "between_pool_z",
            "pools_disagree", "same_sign", "chained_mean",
            "chained_standard_error", "shrinkage_vs_prior"):
    if key in rep:
        lines.append(f"replication_{key}={rep[key]}")
lines.extend([
    f"primary_arena_mean={r['primary_arena_mean']}",
    f"primary_learning_mean={r['primary_learning_mean']}",
    f"learning_confidently_negative={r['learning_confidently_negative']}",
    f"volume_effect_arena={r['volume_effect_arena']}",
    f"recency_effect_arena={r['recency_effect_arena']}",
    f"novelty_minus_matched_arena={r['novelty_minus_matched_arena']}",
    "recency_shows_label_strength_anticorrelation="
    f"{r['recency_shows_label_strength_anticorrelation']}",
