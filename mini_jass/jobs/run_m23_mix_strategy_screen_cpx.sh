...
oracle="$local_artefacts/oracle.l1.m23-cpx.jsonl"
run_dir="$local_artefacts/runs/m23-mix-strategy-screen-cpx"
summary="$local_artefacts/m23_mix_strategy_screen.cpx.json"
...
"$python_bin" "$repo/mini_jass/tools/run_mix_strategy_screen.py" \
  --config "$repo/mini_jass/configs/l1_mix_strategy_screen.yaml" \
  --oracle "$oracle" \
  --run-dir "$run_dir" \
  --compact-output "$summary" \
  --execution-host "$host"
phase science_20_packs_plus_140_trainings
...
lines = [
    ...
    f"primary_arena_ci95={r['primary_arena_ci95']}",
    f"mixing_beats_current_only={r['mixing_beats_current_only']}",
    f"screen_never_concludes_on_shape={r['screen_never_concludes_on_shape']}",
    f"arms_compared={r['arms_compared']}",
    f"shape_candidate_arm={cand['arm']}",
    f"shape_candidate_arena_AT_SELECTION={cand['arena_mean_AT_SELECTION']}",
    f"shape_candidate_is_a_result={cand['is_a_result']}",
    f"shape_candidate_expected_shrinkage={cand['expected_shrinkage_band']}",
]
census = s.get("census_summary", {})
for key in ("unit_samples_per_generation", "target_unique_samples",
            "unique_samples_by_arm", "unique_states_by_arm",
            "wide_current_available"):
    if key in census:
        lines.append(f"census_{key}={census[key]}")
for name in sorted(c):
    row = c[name]
    lines.append(f"{name}__role={row['role']}")
    for endpoint in ("arena_vs_initial", "learning_delta"):
        lines.append(f"{name}__{endpoint}={row[endpoint]['mean']}")
        lines.append(f"{name}__{endpoint}_ci95={row[endpoint]['confidence_95']}")
