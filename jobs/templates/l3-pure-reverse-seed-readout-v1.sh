...
# promote a model or chain another job.
set -Eeuo pipefail
...
[ "$(nproc)" -ge 16 ] || die "CPX62 requires 16 logical CPUs"
...
python3 - "$ART/verified-arms.json" "$ART/verified-topk-openings.json" \
  "$ART/verified-hard-openings.json" "$IN/arms-summary.json" \
...
arms_report, topk_report, hard_report, summary = (
...
for arm, expected in (("control", control_sha), ("treatment", treatment_sha)):
    require(
        summary.get("arms", {}).get(arm, {}).get("model_sha256") == expected,
        f"{arm} model hash mismatch",
    )
    require(
        summary.get("arms", {}).get(arm, {}).get("fit", {}).get("converged")
        is True,
...
[ "$(sha256sum "$W/CONTROL.pjtw" | awk '{print $1}')" = "$EXPECTED_CONTROL_MODEL_SHA" ] ||
  die "CONTROL model hash drift"
[ "$(sha256sum "$W/TREATMENT.pjtw" | awk '{print $1}')" = "$EXPECTED_TREATMENT_MODEL_SHA" ] ||
  die "TREATMENT model hash drift"
