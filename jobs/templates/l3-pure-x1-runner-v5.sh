...
# L3-PURE C2-X1 lineage-native exploration screen.
# Four corners form the resolution-III half fraction C=AB; a fifth centre cell
# diagnoses curvature. Every cell starts from the same material seed, uses Q00
# search and terminal WDL only, and disables the moving frontier.
set -Eeuo pipefail

: "${JASS_CODE_DIR:?runner v5 must provide JASS_CODE_DIR}"
: "${JASS_RESULT_DIR:?runner v5 must provide JASS_RESULT_DIR}"
: "${JASS_ARTEFACT_DIR:?runner v5 must provide JASS_ARTEFACT_DIR}"
: "${JASS_JOB_ID:?runner v5 must provide JASS_JOB_ID}"
: "${ARM:?set ARM=A}"
: "${L3_VARIANT:?set the pre-registered C2-X1 variant}"
[ "$ARM" = A ] || { echo "ABORT: C2-X1 requires ARM=A" >&2; exit 2; }
: "${FRONTIER_FRAC:=0}"
[ "$FRONTIER_FRAC" = 0 ] || { echo "ABORT: C2-X1 forbids frontier seeding" >&2; exit 2; }

L3_EXPERIMENT="C2-X1"
case "$L3_VARIANT" in
  X_LLH)
    EXPECTED_RANDOM_OPEN=4 EXPECTED_EPS=4 EXPECTED_DECAY=60
    FACTOR_A=-1 FACTOR_B=-1 FACTOR_C=1 DESIGN_ROLE=corner ;;
  X_HLL)
    EXPECTED_RANDOM_OPEN=8 EXPECTED_EPS=4 EXPECTED_DECAY=30
    FACTOR_A=1 FACTOR_B=-1 FACTOR_C=-1 DESIGN_ROLE=corner ;;
  X_LHL)
    EXPECTED_RANDOM_OPEN=4 EXPECTED_EPS=8 EXPECTED_DECAY=30
    FACTOR_A=-1 FACTOR_B=1 FACTOR_C=-1 DESIGN_ROLE=corner ;;
  X_HHH_CONTROL)
    EXPECTED_RANDOM_OPEN=8 EXPECTED_EPS=8 EXPECTED_DECAY=60
    FACTOR_A=1 FACTOR_B=1 FACTOR_C=1 DESIGN_ROLE=current_recipe_control ;;
  X_CENTER)
    EXPECTED_RANDOM_OPEN=6 EXPECTED_EPS=6 EXPECTED_DECAY=45
    FACTOR_A=0 FACTOR_B=0 FACTOR_C=0 DESIGN_ROLE=curvature_centre ;;
  *) echo "ABORT: unknown C2-X1 variant: $L3_VARIANT" >&2; exit 2 ;;
esac
...
[ "$L3_SEARCH_OVERRIDES" = "$EXPECTED_SEARCH_OVERRIDES" ] || {
  echo "ABORT: C2-X1 requires the Q00 search fingerprint" >&2
  exit 2
}
...
readonly EXPECTED_RANDOM_OPEN EXPECTED_EPS EXPECTED_DECAY
readonly FACTOR_A FACTOR_B FACTOR_C DESIGN_ROLE
RES="$W/RESULTS.txt"
PROG="$W/PROGRESS.txt"
: > "$RES"
: > "$PROG"
...
say "=== $JASS_JOB_ID — L3-PURE $L3_EXPERIMENT $L3_VARIANT ==="
...
[ "$NGEN" -eq 2 ] || die "C2-X1 is pre-registered for exactly 2 generations"
[ "$FRESH" -eq 150000 ] || die "C2-X1 is pre-registered for exactly 150000 records/generation"
[ "$NSHARDS" -eq 8 ] || die "C2-X1 requires NSHARDS=8"
[ "$PAR_GEN" -eq 8 ] || die "C2-X1 requires PAR_GEN=8"
[ "$MAXPLIES" -eq 260 ] || die "C2-X1 requires MAXPLIES=260"
[ "$LABEL_DEPTH" -eq 4 ] || die "C2-X1 keeps the ignored label-depth argument at 4"
[ "$RANDOM_OPEN_PLIES" -eq "$EXPECTED_RANDOM_OPEN" ] || die "C2-X1 random opening drift"
[ "$EXPLORE_EPS" -eq "$EXPECTED_EPS" ] || die "C2-X1 epsilon drift"
[ "$EXPLORE_DECAY_PLIES" -eq "$EXPECTED_DECAY" ] || die "C2-X1 decay drift"
[ "$HOLDOUT_MOD" -eq 10 ] || die "C2-X1 requires HOLDOUT_MOD=10"
[ "$BASE_SEED" -eq 271828 ] || die "C2-X1 primary screen requires BASE_SEED=271828"
[ "$MAXIT" -eq 25 ] || die "C2-X1 requires MAXIT=25"
[ "$L2" = 3e-5 ] || die "C2-X1 requires L2=3e-5"
[ "$CHUNK" -eq 500000 ] || die "C2-X1 requires CHUNK=500000"
NPROC="$(nproc)"
...
python3 - "$ART/l3-run-config.json" "$L3_VARIANT" "$CODE_SHA" \
  ...
  "$L2" "$MAXIT" "$CHUNK" "$FACTOR_A" "$FACTOR_B" "$FACTOR_C" \
  "$DESIGN_ROLE" <<'PY'
...
payload = {
  ...
  "design": {
    "kind": "resolution_III_half_fraction_plus_centre",
    "generator": "C=AB",
    "factors": {
      "A_random_open_plies": int(sys.argv[19]),
      "B_initial_epsilon": int(sys.argv[20]),
      "C_decay_end": int(sys.argv[21]),
    },
    "aliases": ["A=BC", "B=AC", "C=AB"],
    "role": sys.argv[22],
    "current_recipe_control": "X_HHH_CONTROL",
  },
  "geometry": "8cf",
  "build_features": {
...
python3 jobs/tests/test_l3_exploration_metrics.py > "$W/test-x1-metrics.log" 2>&1 \
  || die "C2-X1 exploration metric tests failed"
python3 jobs/tests/test_l3_doe_x1_prepared.py > "$W/test-c2-x1.log" 2>&1 \
  || die "C2-X1 contract tests failed"
...
# Runner-v5 pins one immutable SHA. Verify the performance-critical sources in
...
if sources.get("frontier", 0) != 0:
    raise SystemExit("C2-X1 profile contains forbidden frontier records")
if sources.get("standard", 0) != payload["records"]:
    raise SystemExit("C2-X1 profile provenance does not cover every record")
PY
  ...
  # This is initialization only, never an anchor in the objective.
  warm_start_args=()
...
payload.update({
  "scientific_status": "complete_exploration_screen_cell",
  "screen_only": True,
...
say "=== L3-PURE $L3_EXPERIMENT $L3_VARIANT complete; candidate is not promoted ==="
