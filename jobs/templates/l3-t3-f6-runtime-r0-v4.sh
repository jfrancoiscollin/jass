#!/usr/bin/env bash
# R0-v4: data-free ZERO-wrapper transparency and production leaf contract.
set -Eeuo pipefail
...
cd "$JASS_CODE_DIR"
source jobs/templates/t3-f6-runtime-exclusions-v4.sh
...
PREREG_CODE="e857a5a951afa3c78957c7ad92afb67e4b0dae3b"
...
GEN_SEED=2026092501
SELECT_SEED=2026092502
PERMUTE_SEED=2026092503
SEARCH_SEED=2026092504
BENCH_SEED=2026092505
CANDIDATES=40000
SELECTED=4096
CACHE_MB=128
...
monitor(){
  (t0=$(date +%s); while true; do
    { printf 'time_fr=%s\n' "$(TZ=Europe/Paris date '+%Y-%m-%dT%H:%M:%S%z')"
      ...
      printf 'artefacts=%s\n' "$(find "$ART" -maxdepth 1 -type f | wc -l)"; } >"$PROG.tmp"
...
emit_terminal(){
  ...
  cp "$IN/t3-zero-v4.json" "$ART/t3-zero-v4.json"
  cp "$IN/curriculum.pjtw" "$ART/curriculum.pjtw"
  [ -f "$IN/r0-v1-corpus.fen" ] && cp "$IN/r0-v1-corpus.fen" "$ART/r0-v1-corpus.fen"
  [ -f "$IN/r0-v2-corpus.fen" ] && cp "$IN/r0-v2-corpus.fen" "$ART/r0-v2-corpus.fen"
  [ -f "$ART/autopsy-exclusions.fen" ] || true
  [ -f "$IN/r0-v3-mechanics.tsv" ] && cp "$IN/r0-v3-mechanics.tsv" "$ART/r0-v3-mechanics.tsv"
  [ -f "$IN/scan-parents.tsv" ] && cp "$IN/scan-parents.tsv" "$ART/scan-parents.tsv"
  [ -f "$IN/scan-siblings.tsv" ] && cp "$IN/scan-siblings.tsv" "$ART/scan-siblings.tsv"
...
[[ "$JASS_JOB_ID" =~ ^cpx62-[0-9]+-l3-t3-f6-runtime-r0-v4$ ]] || die "job nomenclature drift"
[ "$(git rev-parse HEAD)" = "$EXPECTED_CODE_SHA" ] || die "code SHA mismatch"
git merge-base --is-ancestor "$PREREG_CODE" HEAD || die "v4 prereg is not an ancestor"
...
python3 -m py_compile jobs/tools/t3_f6_zero_artifact_v4.py jobs/tools/t3_f6_r0_select_v4.py \
  jobs/tools/t3_f6_runtime_parity_v4.py jobs/tools/t3_f6_r0_readout_v4.py
"$PY" -m unittest jobs.tests.test_t3_f6_runtime_protocol \
  jobs.tests.test_t3_f6_runtime_v2_protocol jobs.tests.test_t3_f6_runtime_v3_protocol \
  jobs.tests.test_t3_f6_runtime_v4_protocol >"$W/python-tests.log" 2>&1

stage generate-authenticate-data-free-zero
python3 jobs/tools/t3_f6_zero_artifact_v4.py --out "$IN/t3-zero-v4.json" --print-sha256 >"$W/zero-sha.txt"
[ "$(sha256sum "$IN/t3-zero-v4.json" | awk '{print $1}')" = "$ZERO_SHA" ] || die "ZERO artifact SHA drift"
python3 - "$ART/zero-probe-manifest.json" "$IN/t3-zero-v4.json" jobs/tools/t3_f6_zero_artifact_v4.py "$ZERO_SHA" <<'PY_ZERO'
import hashlib,json,sys
from pathlib import Path
out,artifact,generator=map(Path,sys.argv[1:4]); expected=sys.argv[4]
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
payload={'schema':'jass.t3_f6_zero_probe_manifest.v4','passed':True,
 'verdict':'T3_ZERO_DATA_FREE_PROBE_FROZEN','artifact_sha256':sha(artifact),
 'expected_sha256':expected,'generator_sha256':sha(generator),'architecture':[66,256,128,64,1],
 'all_weights_and_biases_zero':True,'normalization_mean':0,'normalization_std':1,
 'data_inputs':0,'fits':0,'training':False,'scientific_candidate':False,'force_authorized':False}
if payload['artifact_sha256']!=expected: raise SystemExit('ZERO SHA drift')
...
if sha(root/'t3-a-f6-only.json')!=msha or sha(root/'curriculum.pjtw')!=csha: raise SystemExit('frozen evaluator SHA drift')
PY_AUTH
...
python3 jobs/tools/fetch_result_files.py --prefix "$T3_F6_R0_V1_PREFIX" --expected-state "$T3_F6_R0_V1_EXPECTED_STATE" \
  --file "$T3_F6_R0_V1_REMOTE=r0-v1-corpus.fen" --out-dir "$IN" --report "$ART/verified-r0-v1.json" >"$W/fetch-r0-v1.log" 2>&1
python3 jobs/tools/fetch_result_files.py --prefix "$T3_F6_R0_V2_PREFIX" --file "$T3_F6_R0_V2_REMOTE=r0-v2-corpus.fen" \
  --out-dir "$IN" --report "$ART/verified-r0-v2.json" >"$W/fetch-r0-v2.log" 2>&1
python3 jobs/tools/fetch_result_files.py --prefix "$T3_F6_AUTOPSY_PREFIX" --file "$T3_F6_AUTOPSY_REMOTE=negamax-autopsy.json" \
  --out-dir "$IN" --report "$ART/verified-autopsy.json" >"$W/fetch-autopsy.log" 2>&1
python3 - "$IN/negamax-autopsy.json" "$ART/autopsy-exclusions.fen" <<'PY_AUTOPSY_FEN'
import json,sys
from pathlib import Path
from jobs.tools.t3_f6_r0_select_v4 import fen_fingerprint
...
PY_AUTOPSY_FEN
python3 jobs/tools/fetch_result_files.py --prefix "$T3_F6_R0_V3_PREFIX" --file "$T3_F6_R0_V3_MECHANICS_REMOTE=r0-v3-mechanics.tsv" \
  --out-dir "$IN" --report "$ART/verified-r0-v3.json" >"$W/fetch-r0-v3.log" 2>&1
python3 jobs/tools/fetch_result_files.py --prefix "$T3_F6_SCAN_SELECTION_PREFIX" \
  --file "$T3_F6_SCAN_PARENTS_REMOTE=scan-parents.tsv" --file "$T3_F6_SCAN_SIBLINGS_REMOTE=scan-siblings.tsv" \
  --out-dir "$IN" --report "$ART/verified-scan-selection.json" >"$W/fetch-scan.log" 2>&1
FORCE_ARGS+=(--exclude-fen "$IN/r0-v1-corpus.fen" --exclude-fen "$IN/r0-v2-corpus.fen")
IDENTITY_ARGS+=(--exclude-tsv "$IN/r0-v3-mechanics.tsv" --exclude-tsv "$IN/scan-parents.tsv" --exclude-tsv "$IN/scan-siblings.tsv")
[ "$IDENTITY_COUNT" -eq 10 ] && [ "$FORCE_COUNT" -eq 24 ] || die "base exclusion count drift"
...
cmake --build "$W/build" -j8 --target jass jass_tests t3_f6_runtime_probe \
  ...
  t3_f6_runtime_contract_v4 >"$W/build.log" 2>&1
env -u JASS_EGDB_PATH -u JASS_EGDB_CACHE_MB ctest --test-dir "$W/build" --output-on-failure >"$W/ctest.log" 2>&1
J="$W/build/jass"; V4_PROBE="$W/build/t3_f6_runtime_contract_v4"

stage generate-and-seal-r0-v4-corpus
for pass in a b; do
  "$J" --gen-opening-pool "$CANDIDATES" "$W/r0-v4-candidates-$pass.fen" 8 160 9 "$GEN_SEED" >"$W/r0-v4-generate-$pass.log" 2>&1
done
cmp -s "$W/r0-v4-candidates-a.fen" "$W/r0-v4-candidates-b.fen" || die "R0-v4 candidate replay drift"
python3 jobs/tools/t3_f6_r0_select_v4.py --candidates "$W/r0-v4-candidates-a.fen" \
  "${IDENTITY_ARGS[@]}" "${FORCE_ARGS[@]}" --exclude-json-fens "$IN/negamax-autopsy.json" \
  --selection-seed "$SELECT_SEED" --permutation-seed "$PERMUTE_SEED" \
  --search-seed "$SEARCH_SEED" --benchmark-seed "$BENCH_SEED" \
  ...
  --out-search-fen "$ART/r0-search-roots.fen" --report "$ART/r0-selection.json" >"$W/r0-select.log" 2>&1
[ "$(grep -c . "$ART/r0-corpus.fen")" -eq "$SELECTED" ] || die "R0-v4 corpus cardinality drift"

stage zero-wrapper-and-real-search-contract
"$V4_PROBE" "$ART/r0-corpus.fen" "$ART/r0-search-roots.fen" "$IN/curriculum.pjtw" \
  "$IN/t3-a-f6-only.json" --zero-probe "$IN/t3-zero-v4.json" "$Q00" \
  "$EXPECTED_CODE_SHA" "$ART/r0-wrapper-contract.json" >"$W/wrapper.log" 2>&1
WRAPPER_PASS=$(python3 -c 'import json,sys;print(str(json.load(open(sys.argv[1]))["passed"]).lower())' "$ART/r0-wrapper-contract.json")
if [ "$WRAPPER_PASS" != true ]; then
  python3 jobs/tools/t3_f6_r0_readout_v4.py --selection "$ART/r0-selection.json" --wrapper "$ART/r0-wrapper-contract.json" \
    --zero-manifest "$ART/zero-probe-manifest.json" --model "$IN/t3-a-f6-only.json" --zero "$IN/t3-zero-v4.json" \
    --curriculum "$IN/curriculum.pjtw" --code-sha "$EXPECTED_CODE_SHA" --out "$ART/JASS_CONTROL_SUMMARY.json" >"$W/readout.log" 2>&1
...
"$W/build/t3_f6_relative_probe" "$ART/r0-corpus.fen" "$IN/curriculum.pjtw" \
  ...
  --v4-relative-only >"$W/relative.log" 2>&1
...
if [ "$RELATIVE_PASS" != true ]; then
  python3 jobs/tools/t3_f6_r0_readout_v4.py --selection "$ART/r0-selection.json" --wrapper "$ART/r0-wrapper-contract.json" \
    --relative "$ART/r0-relative-contract.json" --zero-manifest "$ART/zero-probe-manifest.json" \
    --model "$IN/t3-a-f6-only.json" --zero "$IN/t3-zero-v4.json" --curriculum "$IN/curriculum.pjtw" \
    --code-sha "$EXPECTED_CODE_SHA" --out "$ART/JASS_CONTROL_SUMMARY.json" >"$W/readout.log" 2>&1
  emit_terminal; exit 0
fi

stage build-frozen-parity-references
...
cmake -S "$W/v1-src" -B "$W/v1-build" -DCMAKE_BUILD_TYPE=Release -DJASS_EGDB=ON \
  -DJASS_EGDB_SRC_DIR=/root/egdb_intl -DJASS_ENDGAME_FEATURES=ON -DJASS_KING_MOBILITY=ON \
  -DJASS_SCAN_PARITY=ON -DJASS_TEMPO_STAGE=ON >"$W/v1-cmake.log" 2>&1
...
c++ -std=c++20 -O3 -DNDEBUG -DJASS_ENDGAME_FEATURES=1 -DJASS_KING_MOBILITY=1 \
  -DJASS_SCAN_PARITY=1 -DJASS_TEMPO_STAGE=1 -I"$W/rf1-src/src" -I"$W/rf1-src/pattern_jass/src" \
  "$W/rf1-src/jobs/tools/residual_feature_dump.cpp" "$W/rf1-src/src/residual_features.cpp" \
  "$W/rf1-build/libjass_lib.a" -pthread -o "$W/residual_feature_dump_rf1" >"$W/rf1-link.log" 2>&1
...
"$PY" jobs/tools/t3_f6_runtime_parity_v4.py --native-tsv "$W/r0-native.tsv" \
  --reference-rffd "$W/r0-reference.rffd" --model "$IN/t3-a-f6-only.json" \
  --zero "$IN/t3-zero-v4.json" --curriculum "$IN/curriculum.pjtw" \
  --wrapper-contract "$ART/r0-wrapper-contract.json" --out "$ART/r0-python-native-parity.json" >"$W/parity.log" 2>&1
...
printf 'hello\nquit\n' | env JASS_T3_F6_MODEL="$IN/t3-zero-v4.json" "$J" --pattern "$IN/curriculum.pjtw" >"$W/load-zero.log" 2>&1; ZERO_RC=$?
set -e
[ "$EMPTY_RC" -ne 0 ] && [ "$WRONG_RC" -ne 0 ] && [ "$ZERO_RC" -ne 0 ] || die "production loader did not fail closed"
python3 - "$ART/loader-auth.json" "$EMPTY_RC" "$WRONG_RC" "$ZERO_RC" <<'PY_LOADER'
import json,sys
from pathlib import Path
Path(sys.argv[1]).write_text(json.dumps({'schema':'jass.t3_f6_loader_auth.v4','passed':True,
 'off_absent_exact':True,'on_exact_load':True,'empty_env_rejected':int(sys.argv[2])!=0,
 'wrong_artifact_rejected':int(sys.argv[3])!=0,'zero_artifact_rejected_by_production':int(sys.argv[4])!=0},indent=2,sort_keys=True)+'\n')
PY_LOADER
...
stage runtime-contract-and-r0-v4-verdict
...
python3 - "$ART/runtime-contract.json" "$J" "$V4_PROBE" "$MODEL_SHA" "$ZERO_SHA" "$CURRICULUM_SHA" "$Q00" "$EGDIR" <<'PY_RUNTIME'
import hashlib,json,sys
from pathlib import Path
out,exe,probe=map(Path,sys.argv[1:4]); model,zero,curr,q00,egdir=sys.argv[4:]
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
payload={'schema':'jass.t3_f6_runtime_contract.v4','passed':True,'executable_sha256':sha(exe),
 'probe_executable_sha256':sha(probe),'model_sha256':model,'zero_sha256':zero,'curriculum_sha256':curr,
 'same_executable_force_arms':True,'candidate_env':'JASS_T3_F6_MODEL','control_env':'ABSENT',
 'zero_production_env_allowed':False,'leaf_only':True,'search_params':q00,'threads':1,'tt_mb':16,
 'egdb':'ON','egdb_path':egdir,'egdb_cache_mb':128,'book':'OFF','maxplies':160,
 'build_flags':['JASS_EGDB=ON','JASS_ENDGAME_FEATURES=ON','JASS_KING_MOBILITY=ON','JASS_SCAN_PARITY=ON','JASS_TEMPO_STAGE=ON']}
out.write_text(json.dumps(payload,indent=2,sort_keys=True)+'\n')
PY_RUNTIME
python3 jobs/tools/t3_f6_r0_readout_v4.py --selection "$ART/r0-selection.json" \
  --wrapper "$ART/r0-wrapper-contract.json" --relative "$ART/r0-relative-contract.json" \
  ...
  --runtime-contract "$ART/runtime-contract.json" --zero-manifest "$ART/zero-probe-manifest.json" \
  --model "$IN/t3-a-f6-only.json" --zero "$IN/t3-zero-v4.json" --curriculum "$IN/curriculum.pjtw" \
  --executable "$J" --probe-executable "$V4_PROBE" --reference-rffd "$W/r0-reference.rffd" \
