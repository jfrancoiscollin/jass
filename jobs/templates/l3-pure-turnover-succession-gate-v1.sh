#!/usr/bin/env bash
# L3-PURE: champion-succession gate for TURNOVER against the incumbent F2M.
#
# Authorised by home-0995. Runs every cell unconditionally: the primary
# TURNOVER/F2M comparison on a fresh 1500-opening pool, the Gen2
# non-regression guard, and P3/P4 conversion against the frozen defender.
# Views are summed, per L3_VIEW_AGREEMENT_AND_POWER_20260726.md.
#
# Only a human review may promote. This job never does.
set -Eeuo pipefail
...
: "${DOSE_READOUT_PREFIX:?}"; : "${EXPECTED_DOSE_READOUT_JOB:?}"
...
python3 jobs/tools/fetch_result_files.py --prefix "$PREFLIGHT_PREFIX" \
  --file artefacts/JASS_CONTROL_SUMMARY.json=succession-preflight.json \
  --file artefacts/turnover-succession-openings.fen=open-eval.fen \
  --file artefacts/turnover-succession-openings.json=openings-manifest.json \
  --out-dir "$IN" --report "$ART/verified-succession-preflight.json" \
  > "$W/fetch-succession-preflight.log" 2>&1
...
for spec in \
  "verified-succession-preflight.json:$EXPECTED_PREFLIGHT_JOB" \
...
python3 - "$IN/succession-preflight.json" "$IN/dose-readout.json" \
  "$EXPECTED_OPENING_SHA256" "$OPENING_SEED" <<'XPY'
import json
import sys

preflight = json.load(open(sys.argv[1]))
readout = json.load(open(sys.argv[2]))
pool_sha, seed = sys.argv[3:]
openings = preflight.get("succession_openings", {})
if preflight.get("verdict") != "TURNOVER_SUCCESSION_PREFLIGHT_READY":
    raise SystemExit("preflight verdict mismatch")
if preflight.get("gate_authorized") is not True:
    raise SystemExit("preflight does not authorise the gate")
...
for view in q00 native; do
  stage "primary-$view-vs-f2m"
  pids=()
  run_gate "$view" TURNOVER F2M &
  pids+=("$!")
  wait_all "$view primary wave" "${pids[@]}"
done

# Aucun filtre d'éligibilité : home-0993 a déjà établi TURNOVER au-dessus de
# F2M, la porte joue donc toutes ses cellules inconditionnellement.
ELIGIBLE=(TURNOVER)
if [ "${#ELIGIBLE[@]}" -gt 0 ]; then
  ...
  for view in q00 native; do
    ...
    for arm in "${ELIGIBLE[@]}"; do
      # F2M est la cellule primaire, déjà jouée ; la garde ne mesure que Gen2.
      run_gate "$view" "$arm" GEN2 &
      pids+=("$!")
    done
...
stage aggregate-succession-gate
python3 jobs/tools/l3_succession_gate.py \
  --force-dir "$ART/force" \
  --conversion-dir "$ART/conversion" \
  --dose-readout "$IN/dose-readout.json" \
  ...
  --out "$ART/turnover-succession-gate.json" \
  --summary-out "$ART/JASS_CONTROL_SUMMARY.json" \
  > "$W/aggregate.log" 2>&1
VERDICT="$(python3 - "$ART/turnover-succession-gate.json" <<'XPY'
import json
import sys
