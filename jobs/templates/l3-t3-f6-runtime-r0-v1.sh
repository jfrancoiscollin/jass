#!/usr/bin/env bash
# R0 production-leaf contract for the frozen T3-A/F6 evaluator. No strength games.
set -Eeuo pipefail
...
MODEL_PREFIX="r2:jass-data/runs/$MODEL_JOB/$MODEL_ATTEMPT"
...
CURRICULUM_PREFIX="r2:jass-data/runs/$CURRICULUM_JOB/$CURRICULUM_ATTEMPT"
PREREG_CODE="60e676867"
RF1_EXTRACTOR_CODE="e5c4a0d6e88e99c06819100c4b5dbc697bbe3a53"
GEN_SEED=2026090901
SELECT_SEED=2026090902
PERMUTE_SEED=2026090903
BENCH_SEED=2026090904
CANDIDATES=40000
SELECTED=4096
CACHE_MB=128
...
finalize(){ rc=$?; trap - EXIT ERR TERM INT; set +e
  ...
  git worktree remove --force "$W/prereg-src" 2>/dev/null || true
...
[[ "$JASS_JOB_ID" =~ ^cpx62-[0-9]+-l3-t3-f6-runtime-r0-v1$ ]] || die "job nomenclature drift"
...
python3 -m py_compile jobs/tools/t3_f6_r0_select.py jobs/tools/t3_f6_runtime_parity.py \
  jobs/tools/t3_f6_search_profile.py jobs/tools/t3_f6_r0_readout.py
...
python3 jobs/tools/fetch_result_files.py --prefix "$MODEL_PREFIX" \
  ...
  --out-dir "$IN" --report "$ART/verified-t3-a.json" >"$W/fetch-model.log" 2>&1 || die "T3-A fetch failed"
python3 jobs/tools/fetch_result_files.py --prefix "$CURRICULUM_PREFIX" \
  ...
  --out-dir "$IN" --report "$ART/verified-curriculum.json" >"$W/fetch-curriculum.log" 2>&1 || die "CURRICULUM fetch failed"
...
stage fetch-identity-only-exclusions
...
while IFS='|' read -r label prefix remote_path; do
  ...
  python3 jobs/tools/fetch_result_files.py --prefix "$prefix" --file "$remote_path=$label.tsv.gz" \
    --out-dir "$IN" --report "$ART/verified-identity-$label.json" \
    >"$W/fetch-identity-$label.log" 2>&1 || die "identity exclusion fetch failed: $label"
...
while IFS='|' read -r label prefix remote_path; do
  ...
  python3 jobs/tools/fetch_result_files.py --prefix "$prefix" --file "$remote_path=$label.fen" \
    --out-dir "$IN" --report "$ART/verified-force-exclusion-$label.json" \
    >"$W/fetch-force-exclusion-$label.log" 2>&1 || die "force exclusion fetch failed: $label"
  FORCE_ARGS+=(--exclude-fen "$IN/$label.fen"); FORCE_COUNT=$((FORCE_COUNT+1))
done <<<"$T3_F6_FORCE_EXCLUDE_SPECS"
[ "$IDENTITY_COUNT" -eq 10 ] || die "identity exclusion count drift"
[ "$FORCE_COUNT" -eq 24 ] || die "force exclusion count drift"
...
cmake --build "$W/build" -j8 --target jass jass_tests t3_f6_runtime_probe t3_f6_invariance_probe \
  >"$W/build.log" 2>&1
env -u JASS_EGDB_PATH -u JASS_EGDB_CACHE_MB ctest --test-dir "$W/build" --output-on-failure \
  >"$W/ctest.log" 2>&1
J="$W/build/jass"

stage build-prereg-off-reference
git worktree add --detach "$W/prereg-src" "$PREREG_CODE" >"$W/prereg-worktree.log" 2>&1
(cd "$W/prereg-src" && python3 pattern_jass/tools/gen_patterns.py --emit --variant 8cf) >"$W/gen-prereg.log" 2>&1
cmake -S "$W/prereg-src" -B "$W/prereg-build" -DCMAKE_BUILD_TYPE=Release -DJASS_EGDB=ON \
  -DJASS_EGDB_SRC_DIR=/root/egdb_intl -DJASS_ENDGAME_FEATURES=ON \
  -DJASS_KING_MOBILITY=ON -DJASS_SCAN_PARITY=ON -DJASS_TEMPO_STAGE=ON >"$W/prereg-cmake.log" 2>&1
cmake --build "$W/prereg-build" -j8 --target jass >"$W/prereg-build.log" 2>&1
PREREG_J="$W/prereg-build/jass"

stage generate-target-blind-r0-corpus
for pass in a b; do
  "$J" --gen-opening-pool "$CANDIDATES" "$W/r0-candidates-$pass.fen" 8 160 9 "$GEN_SEED" \
    >"$W/r0-generate-$pass.log" 2>&1
done
cmp -s "$W/r0-candidates-a.fen" "$W/r0-candidates-b.fen" || die "R0 candidate replay drift"
python3 jobs/tools/t3_f6_r0_select.py --candidates "$W/r0-candidates-a.fen" \
  ...
  --report "$ART/r0-selection.json" \
  >"$W/r0-select.log" 2>&1
[ "$(grep -c . "$ART/r0-corpus.fen")" -eq "$SELECTED" ] || die "R0 corpus cardinality drift"

stage build-frozen-rf1-reference-extractor
...
"$W/residual_feature_dump_rf1" "$W/r0-benchmark.jnnw" "$W/r0-reference.rffd" \
  >"$W/rf1-dump.log" 2>&1

stage invariance-and-native-parity
"$W/build/t3_f6_invariance_probe" "$IN/curriculum.pjtw" "$IN/t3-a-f6-only.json" \
  "$ART/r0-invariance.json" "$PERMUTE_SEED" >"$W/invariance.log" 2>&1
...
"$PY" jobs/tools/t3_f6_runtime_parity.py --native-tsv "$W/r0-native.tsv" \
  --reference-rffd "$W/r0-reference.rffd" --model "$IN/t3-a-f6-only.json" \
  --curriculum "$IN/curriculum.pjtw" --out "$ART/r0-python-native-parity.json" \
  >"$W/parity.log" 2>&1

stage loader-fail-closed-contract
printf 'hello\nquit\n' | env -u JASS_T3_F6_MODEL "$J" --pattern "$IN/curriculum.pjtw" \
  >"$W/load-off.log" 2>&1
grep -q '^ready' "$W/load-off.log" || die "OFF exact load failed"
printf 'hello\nquit\n' | env JASS_T3_F6_MODEL="$IN/t3-a-f6-only.json" \
  "$J" --pattern "$IN/curriculum.pjtw" >"$W/load-on.log" 2>&1
grep -q '^ready' "$W/load-on.log" || die "ON exact load failed"
set +e
printf 'hello\nquit\n' | env JASS_T3_F6_MODEL= "$J" --pattern "$IN/curriculum.pjtw" \
  >"$W/load-empty.log" 2>&1
EMPTY_RC=$?
printf 'hello\nquit\n' | env JASS_T3_F6_MODEL="$ART/r0-selection.json" \
  "$J" --pattern "$IN/curriculum.pjtw" >"$W/load-wrong.log" 2>&1
WRONG_RC=$?
set -e
[ "$EMPTY_RC" -ne 0 ] || die "empty T3 env did not fail closed"
[ "$WRONG_RC" -ne 0 ] || die "wrong T3 artifact did not fail closed"
python3 - "$ART/loader-auth.json" "$EMPTY_RC" "$WRONG_RC" <<'PY_LOADER'
import json,sys
from pathlib import Path
Path(sys.argv[1]).write_text(json.dumps({'schema':'jass.t3_f6_loader_auth.v1','passed':True,
...
python3 jobs/tools/t3_f6_search_profile.py --exe "$J" --prereg-exe "$PREREG_J" \
  --curriculum "$IN/curriculum.pjtw" --model "$IN/t3-a-f6-only.json" \
  --corpus "$ART/r0-corpus.fen" --search-params "$Q00" \
  --out "$ART/r0-search-profile.json" >"$W/search-profile.log" 2>&1

stage runtime-contract-and-r0-verdict
...
payload={'schema':'jass.t3_f6_runtime_contract.v1','passed':True,'executable_sha256':sha(exe),
 ...
 'search_params':q00,'threads':1,'tt_mb':16,'egdb':'ON','egdb_path':egdir,
 'egdb_cache_mb':128,'book':'OFF','maxplies':160,
...
PY_RUNTIME
python3 jobs/tools/t3_f6_r0_readout.py --selection "$ART/r0-selection.json" \
  --invariance "$ART/r0-invariance.json" --parity "$ART/r0-python-native-parity.json" \
  --runtime-profile "$ART/r0-runtime-profile.json" --search-profile "$ART/r0-search-profile.json" \
  --loader-auth "$ART/loader-auth.json" --runtime-contract "$ART/runtime-contract.json" \
  --model "$IN/t3-a-f6-only.json" --curriculum "$IN/curriculum.pjtw" \
  --executable "$J" --reference-rffd "$W/r0-reference.rffd" --code-sha "$EXPECTED_CODE_SHA" \
...
: >"$ART/VERDICT__R0_PRODUCTION_LEAF_CONTRACT_PASS"
: >"$ART/STRENGTH_GAMES__0"
: >"$ART/POOL1_AUTHORIZED__TRUE"
: >"$ART/PROMOTION_AUTHORIZED__FALSE"
: >"$ART/BAKE__FALSE"
say "R0_PRODUCTION_LEAF_CONTRACT_PASS strength_games=0 pool1_authorized=true promotion=false"
