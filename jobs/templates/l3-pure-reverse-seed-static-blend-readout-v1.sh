#!/usr/bin/env bash
# Independent BLEND50 versus TURNOVER readout.
#
# The fixed static blend is authenticated before 6000 paired-colour games on
# 1500 new openings disjoint from earlier L3-PURE signal readouts.  This job
# measures strength; it cannot select alpha, promote, or chain another job.
set -Eeuo pipefail
...
: "${CHAMPION_PREFIX:?}"; : "${CHAMPION_JOB:?}"; : "${CHAMPION_ATTEMPT:?}"
: "${CHAMPION_CODE_SHA:?}"; : "${EXPECTED_CHAMPION_SHA:?}"
: "${EXPECTED_REVERSE_SHA:?}"; : "${EXPECTED_BLEND_SHA:?}"
...
finalize(){
  ...
  rm -f "$W"/*.pjtw "$W"/*.fen 2>/dev/null || true
  exit "$rc"
}
trap finalize EXIT
...
[ "$OPENING_SEED" -eq 1105001 ] || die "opening seed drift"
[ "$(tr ',' '\n' <<<"$Q00" | wc -l)" -eq 63 ] || die "Q00 drift"
grep -q "root_is_drawn" src/search.cpp || die "engine predates drawn-root fix"
monitor

stage local-tests
python3 -m py_compile jobs/tools/l3_static_blend_readout.py
python3 jobs/tests/test_l3_static_blend_readout.py \
  > "$W/test-readout.log" 2>&1 || die "readout tests red"

stage fetch-and-authenticate-immutable-inputs
python3 jobs/tools/fetch_result_files.py --prefix "$SOURCE_PREFIX" \
  --file artefacts/JASS_CONTROL_SUMMARY.json=build-summary.json \
  --file artefacts/blend50.pjtw.gz=blend50.pjtw.gz \
  --out-dir "$IN" --report "$ART/verified-blend.json" \
  > "$W/fetch-blend.log" 2>&1
...
python3 - "$ART/verified-blend.json" "$ART/verified-champion.json" \
  "$ART/verified-topk-openings.json" "$ART/verified-hard-openings.json" \
  "$ART/verified-reverse-openings.json" \
  "$ART/verified-failed-x2-openings.json" "$IN/build-summary.json" <<'PY'
import json
import os
import sys

blend_report, champion_report, topk, hard, reverse, failed_x2, summary = (
    json.load(open(path, encoding="utf-8")) for path in sys.argv[1:8]
)
...
for report, identity, label in (
    (
        blend_report,
        (
            os.environ["EXPECTED_SOURCE_JOB"],
            os.environ["EXPECTED_SOURCE_ATTEMPT"],
            os.environ["EXPECTED_SOURCE_CODE_SHA"],
        ),
        "blend",
    ),
    (
    ...
    (
        topk,
        (
            os.environ["EXPECTED_TOPK_OPENINGS_JOB"],
            os.environ["EXPECTED_TOPK_OPENINGS_ATTEMPT"],
            os.environ["EXPECTED_TOPK_OPENINGS_CODE_SHA"],
        ),
        "topk openings",
    ),
    (
        hard,
        (
            os.environ["EXPECTED_HARD_OPENINGS_JOB"],
            os.environ["EXPECTED_HARD_OPENINGS_ATTEMPT"],
            os.environ["EXPECTED_HARD_OPENINGS_CODE_SHA"],
        ),
        "hard openings",
    ),
    (
        reverse,
        (
            os.environ["EXPECTED_REVERSE_OPENINGS_JOB"],
            os.environ["EXPECTED_REVERSE_OPENINGS_ATTEMPT"],
            os.environ["EXPECTED_REVERSE_OPENINGS_CODE_SHA"],
        ),
        "reverse openings",
    ),
    (
        failed_x2,
        (
            os.environ["EXPECTED_FAILED_X2_OPENINGS_JOB"],
            os.environ["EXPECTED_FAILED_X2_OPENINGS_ATTEMPT"],
            os.environ["EXPECTED_FAILED_X2_OPENINGS_CODE_SHA"],
        ),
        "failed-x2 openings",
    ),
):
...
require(
    ...
    and summary.get("verdict") == "L3_PURE_REVERSE_SEED_BLEND50_READY"
    and summary.get("code_sha") == os.environ["EXPECTED_SOURCE_CODE_SHA"]
    and construction.get("single_factor") == "static_pjtw_weight_blend"
    and construction.get("alpha_champion") == 0.5
    and construction.get("alpha_reverse_seed") == 0.5
    and construction.get("training_records") == 0
    and construction.get("self_play_games") == 0
    and models.get("champion_sha256") == os.environ["EXPECTED_CHAMPION_SHA"]
    and models.get("reverse_seed_sha256") == os.environ["EXPECTED_REVERSE_SHA"]
    and models.get("blend_sha256") == os.environ["EXPECTED_BLEND_SHA"]
    and summary.get("static_linearity_probe", {}).get("passed") is True
    and summary.get("readout_authorized") is True
    and summary.get("scientific_result") is False
    and summary.get("promotion_authorized") is False
    and summary.get("automatic_next_job", "missing") is None,
    "blend build certificate mismatch",
)
PY

gunzip -c "$IN/champion.pjtw.gz" > "$W/TURNOVER.pjtw"
gunzip -c "$IN/blend50.pjtw.gz" > "$W/BLEND50.pjtw"
[ "$(sha256sum "$W/TURNOVER.pjtw" | awk '{print $1}')" = \
  "$EXPECTED_CHAMPION_SHA" ] || die "champion model hash drift"
[ "$(sha256sum "$W/BLEND50.pjtw" | awk '{print $1}')" = \
  "$EXPECTED_BLEND_SHA" ] || die "blend model hash drift"
...
python3 jobs/tools/select_independent_opening_pool.py \
  ...
  --out "$ART/blend50-readout-openings.fen" \
  --manifest "$ART/blend50-readout-openings.json" \
  > "$W/select-openings.log" 2>&1
cp "$ART/blend50-readout-openings.fen" "$W/open-eval.fen"
python3 - "$ART/blend50-readout-openings.json" "$NOPEN" \
  "$OPENING_SEED" <<'PY'
import json
import sys
...
run_gate(){
  ...
  timeout 10800 python3 jobs/tools/run_jass_gate_bounded.py \
    --jass-a "$J8" --jass-b "$J8" \
    --pattern-a "$W/BLEND50.pjtw" --pattern-b "$W/TURNOVER.pjtw" \
    ...
    --out "$ART/force/force-$view-BLEND50-vs-TURNOVER.json" \
...
python3 -m jobs.tools.l3_static_blend_readout \
  --force-dir "$ART/force" --build-summary "$IN/build-summary.json" \
  --opening-manifest "$ART/blend50-readout-openings.json" \
  ...
  --champion-model-sha "$EXPECTED_CHAMPION_SHA" \
  --reverse-model-sha "$EXPECTED_REVERSE_SHA" \
  --blend-model-sha "$EXPECTED_BLEND_SHA" \
  --out "$ART/blend50-vs-turnover-readout.json" \
