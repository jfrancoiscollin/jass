#!/usr/bin/env bash
# 0959: exact Scan verification pruning + threat-node semantics diagnostic.
# Diagnostic only: no training, promotion or automatic continuation.
set -Eeuo pipefail
: "${JASS_CODE_DIR:?}"; : "${JASS_RESULT_DIR:?}"; : "${JASS_ARTEFACT_DIR:?}"
: "${JASS_JOB_ID:?}"; : "${EXPECTED_JOB_ID:?}"; : "${EXPECTED_CODE_SHA:?}"
: "${GAUGE_PREFIX:?}"; : "${CAUSAL_PREFIX:?}"; : "${SEARCH_PREFIX:?}"
...
ARMS=(SCAN_CORE SCAN_VERIFY SCAN_VERIFY_THREAT)
DEPTHS=(10 12)
CONV_NSHARDS=4; TRACE_NSHARDS=8; PAR_CELL_GROUPS=2
TARGET_PER_STRATUM=300; DEFENDER_DEPTH=10; MAX_PLIES=260
TRACE_DEPTHS="8,10,12"; BOOTSTRAP_SAMPLES=200000; READOUT_SEED=959101
CACHE_MB=128
...
[ "${SCAN_SEMANTICS_GO:-0}" = 1 ] || die "Scan-semantics authorization missing"
...
python3 -m py_compile \
  jobs/tools/l3_scan_semantics_variants.py \
  jobs/tools/l3_scan_semantics_replay.py \
  jobs/tools/l3_scan_semantics_report.py
python3 -m unittest \
  jobs.tests.test_l3_scan_semantics_audit \
  jobs.tests.test_l3_search_tree_audit \
  > "$W/test-scan-semantics.log" 2>&1
...
python3 jobs/tools/fetch_result_files.py --prefix "$SEARCH_PREFIX" \
  --file artefacts/search-tree-audit.json=source-0958.json \
  --file artefacts/search-tree-sentinels.json=search-tree-sentinels.json \
  --file artefacts/scan-static-parity.json=source-0958-static-parity.json \
  --file artefacts/conversion/SCAN_LMR-p3_mince.json=CORE-D10-p3_mince.json \
  --file artefacts/conversion/SCAN_LMR-p4_egal.json=CORE-D10-p4_egal.json \
  --out-dir "$IN" --report "$ART/verified-0958.json" > "$W/fetch-0958.log" 2>&1
...
cp "$IN/source-0958.json" "$ART/source-0958.json"
cp "$IN/source-0958-static-parity.json" "$ART/source-0958-static-parity.json"
...
for stratum in "${STRATA[@]}"; do
  cp "$IN/CORE-D10-$stratum.json" "$ART/conversion/SCAN_CORE-D10-$stratum.json"
done
...
src=json.loads((root/"source-0958.json").read_text())
parity=json.loads((root/"source-0958-static-parity.json").read_text())
...
if src.get("localization",{}).get("verdict")!="MISSING_SCAN_SEARCH_MECHANISM_OR_DEPTH_SEMANTICS":
    raise SystemExit("0958 localization mismatch")
if parity.get("verdict")!="SCAN_STATIC_PORT_EXACT" or parity.get("comparison",{}).get("max_abs_delta")!=0:
    raise SystemExit("0958 exact static parity proof missing")
if len(sent.get("sentinels",[]))!=48:
    raise SystemExit("0958 sentinel count mismatch")
params={d.get("search_params") for d in docs}
defender={d.get("defender_search_params") for d in docs}
if len(params)!=1 or params!=defender: raise SystemExit("Q00 fingerprint mismatch")
q00=params.pop()
if not isinstance(q00,str) or len(q00.split(","))!=63:
    raise SystemExit("Q00 is not fully resolved")
Path(sys.argv[2]).write_text(q00+"\n")
PY
python3 jobs/tools/l3_scan_semantics_variants.py --base-file "$W/Q00.txt" \
  --out-dir "$W/params" --manifest "$ART/scan-semantics-variant-manifest.json" \
  > "$W/scan-semantics-variants.log" 2>&1
Q00="$(cat "$W/Q00.txt")"
...
run_cell(){
  local arm="$1" depth="$2" params="$3" stratum="$4" pool="$5"
  local pids=() inputs=() shard out
  for shard in $(seq 0 $((CONV_NSHARDS-1))); do
    out="$W/$arm-D$depth-$stratum-$shard.json"; inputs+=("$out")
    timeout 21600 python3 jobs/tools/conv_fixed_wdl.py \
      ...
      --out "$out" > "$W/$arm-D$depth-$stratum-$shard.log" 2>&1 & pids+=("$!")
  done
  wait_all "$arm/D$depth/$stratum" "${pids[@]}"
  python3 jobs/tools/aggregate_conv_shards.py --inputs "${inputs[@]}" \
    ...
    --out "$ART/conversion/$arm-D$depth-$stratum.json" \
    > "$W/$arm-D$depth-$stratum-aggregate.log" 2>&1
}

stage full-gauge-node-semantics
for depth in "${DEPTHS[@]}"; do
  for stratum in "${STRATA[@]}"; do
    pids=()
    for arm in "${ARMS[@]}"; do
      [ "$depth" -eq 10 ] && [ "$arm" = SCAN_CORE ] && continue
      params="$(cat "$W/params/$arm.txt")"
      run_cell "$arm" "$depth" "$params" "$stratum" "$W/$stratum.jnnw" &
      pids+=("$!")
      if [ "${#pids[@]}" -ge "$PAR_CELL_GROUPS" ]; then
        wait_all "D$depth/$stratum arm batch" "${pids[@]}"; pids=()
      fi
    done
    [ "${#pids[@]}" -eq 0 ] || wait_all "D$depth/$stratum arm batch" "${pids[@]}"
  done
done
...
for shard in $(seq 0 $((TRACE_NSHARDS-1))); do
  timeout 21600 python3 jobs/tools/l3_scan_semantics_replay.py \
    --sentinels "$ART/search-tree-sentinels.json" \
    --variants "$ART/scan-semantics-variant-manifest.json" \
...
tar -C "$ART/traces" -czf "$ART/scan-node-semantics-traces.tar.gz" .

stage aggregate-node-semantics-readout
python3 jobs/tools/l3_scan_semantics_report.py \
  --sentinels "$ART/search-tree-sentinels.json" \
  --replay-inputs "${replay_inputs[@]}" --conversion-dir "$ART/conversion" \
  --strata "${STRATA[@]}" --bootstrap-samples "$BOOTSTRAP_SAMPLES" \
  --seed "$READOUT_SEED" --out "$ART/scan-node-semantics-audit.json" \
  --summary-out "$ART/JASS_CONTROL_SUMMARY.json" | tee -a "$RES"
printf '%s\n' SCAN_NODE_SEMANTICS_AUDIT_READY_HUMAN_REVIEW \
  > "$ART/VERDICT__SCAN_NODE_SEMANTICS_AUDIT_READY_HUMAN_REVIEW"
...
stage complete
say "SCAN_NODE_SEMANTICS_AUDIT_READY_HUMAN_REVIEW training=false promotion=false"
