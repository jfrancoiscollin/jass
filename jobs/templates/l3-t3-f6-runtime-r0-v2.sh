#!/usr/bin/env bash
# R0-v2: relative production-leaf contract for frozen T3-A/F6. No strength games.
set -Eeuo pipefail
...
PREREG_CODE="b6c747091aea265cd3f7ddeb4175fe05912ad255"
...
GEN_SEED=2026091701
SELECT_SEED=2026091702
PERMUTE_SEED=2026091703
BENCH_SEED=2026091704
CANDIDATES=40000
SELECTED=4096
CACHE_MB=128
...
[[ "$JASS_JOB_ID" =~ ^cpx62-[0-9]+-l3-t3-f6-runtime-r0-v2$ ]] || die "job nomenclature drift"
[ "$(git rev-parse HEAD)" = "$EXPECTED_CODE_SHA" ] || die "code SHA mismatch"
git merge-base --is-ancestor "$PREREG_CODE" HEAD || die "prereg is not an ancestor of implementation"
...
python3 -m py_compile jobs/tools/t3_f6_r0_select.py jobs/tools/t3_f6_runtime_parity_v2.py \
  jobs/tools/t3_f6_search_profile.py jobs/tools/t3_f6_r0_readout_v2.py \
  jobs/tools/t3_f6_strength_readout_v2.py
...
FORCE_ARGS+=(--exclude-fen "$IN/r0-v1-corpus.fen")
...
stage generate-target-blind-r0-v2-corpus
for pass in a b; do
  "$J" --gen-opening-pool "$CANDIDATES" "$W/r0-v2-candidates-$pass.fen" 8 160 9 "$GEN_SEED" >"$W/r0-v2-generate-$pass.log" 2>&1
done
cmp -s "$W/r0-v2-candidates-a.fen" "$W/r0-v2-candidates-b.fen" || die "R0-v2 candidate replay drift"
python3 jobs/tools/t3_f6_r0_select.py --candidates "$W/r0-v2-candidates-a.fen" \
...
[ "$(grep -c . "$ART/r0-corpus.fen")" -eq "$SELECTED" ] || die "R0-v2 corpus cardinality drift"
...
stage relative-position-symmetry-negamax-gates
"$W/build/t3_f6_relative_probe" "$ART/r0-corpus.fen" "$IN/curriculum.pjtw" \
  "$IN/t3-a-f6-only.json" "$ART/r0-relative-contract.json" "$PERMUTE_SEED" >"$W/relative.log" 2>&1
...
if [ "$RELATIVE_PASS" != true ]; then
  python3 jobs/tools/t3_f6_r0_readout_v2.py --selection "$ART/r0-selection.json" \
  ...
  cp "$IN/t3-a-f6-only.json" "$ART/t3-a-f6-only.json"; cp "$IN/curriculum.pjtw" "$ART/curriculum.pjtw"
  cp "$IN/r0-v1-corpus.fen" "$ART/r0-v1-corpus.fen"
  : >"$ART/VERDICT__$VERDICT"; : >"$ART/STRENGTH_GAMES__0"; : >"$ART/POOL1_AUTHORIZED__FALSE"
  : >"$ART/PROMOTION_AUTHORIZED__FALSE"; : >"$ART/BAKE__FALSE"
  say "$VERDICT strength_games=0 pool1_authorized=false promotion=false"
  exit 0
fi
...
import json,sys
from pathlib import Path
Path(sys.argv[1]).write_text(json.dumps({'schema':'jass.t3_f6_loader_auth.v2','passed':True,
...
stage runtime-contract-and-r0-v2-verdict
...
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
payload={'schema':'jass.t3_f6_runtime_contract.v2','passed':True,'executable_sha256':sha(exe),
...
PY_RUNTIME
python3 jobs/tools/t3_f6_r0_readout_v2.py --selection "$ART/r0-selection.json" \
  --relative "$ART/r0-relative-contract.json" --parity "$ART/r0-python-native-parity.json" \
...
cp "$IN/t3-a-f6-only.json" "$ART/t3-a-f6-only.json"; cp "$IN/curriculum.pjtw" "$ART/curriculum.pjtw"
cp "$IN/r0-v1-corpus.fen" "$ART/r0-v1-corpus.fen"
gzip -n -c "$J" >"$ART/jass-t3-f6-force.gz"; gzip -n -c "$W/r0-native.tsv" >"$ART/r0-native.tsv.gz"
gzip -n -c "$W/r0-reference.rffd" >"$ART/r0-reference.rffd.gz"
: >"$ART/VERDICT__$VERDICT"; : >"$ART/STRENGTH_GAMES__0"
: >"$ART/POOL1_AUTHORIZED__${AUTHORIZED^^}"; : >"$ART/PROMOTION_AUTHORIZED__FALSE"; : >"$ART/BAKE__FALSE"
