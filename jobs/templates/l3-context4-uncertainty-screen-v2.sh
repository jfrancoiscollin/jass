...
# Technical v2: identical preregistered science to v1, with certified 1428
# force-summary protocol-scope authentication repaired after diagnostic 1433.
#
...
[[ "$JASS_JOB_ID" =~ ^cpx62-[0-9]+-l3-context4-uncertainty-screen-v2$ ]] || die "invalid job nomenclature"
...
python3 -m py_compile jobs/tools/l3_context4_uncertainty_screen.py jobs/tools/l3_context4_source_contract.py
"$PY" -m unittest jobs.tests.test_l3_context4_uncertainty_screen jobs.tests.test_l3_context4_source_contract >"$W/tests.log" 2>&1
