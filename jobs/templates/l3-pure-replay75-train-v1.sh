#!/usr/bin/env bash
# L3-PURE REPLAY75: fit the preflighted 25% historical / 75% recent corpus.
set -Eeuo pipefail
...
python3 jobs/tools/fetch_result_files.py --prefix "$PREFLIGHT_PREFIX" \
  --file artefacts/JASS_CONTROL_SUMMARY.json=replay75-preflight.json \
  --file artefacts/replay75.jnnw.gz=replay75.jnnw.gz \
  --file artefacts/replay75.jsm.gz=replay75.jsm.gz \
  --file artefacts/replay75-mix.json=replay75-mix.json \
  --file artefacts/replay75-split.json=replay75-split.json \
  --file artefacts/replay75-coverage.json=replay75-coverage.json \
...
preflight = json.load(open(src / "replay75-preflight.json"))
mix = json.load(open(src / "replay75-mix.json"))
split = json.load(open(src / "replay75-split.json"))
coverage = json.load(open(src / "replay75-coverage.json"))
...
if (
    preflight.get("verdict") != "REPLAY75_PREFLIGHT_READY"
    ...
    or preflight.get("experiment_variant") != "REPLAY75_RECENCY75"
    or preflight.get("parent") != "F2M"
    or preflight.get("records") != 2_000_000
    or preflight.get("historical_replay_records") != 1_500_000
    or preflight.get("fresh_records") != 500_000
    or preflight.get("mix_seed") != 832_040
    ...
    raise SystemExit("REPLAY75 preflight certificate mismatch")
if (
...
gunzip -c "$IN/replay75.jnnw.gz" > "$W/replay75.raw.jnnw"
gunzip -c "$IN/replay75.jsm.gz" > "$W/replay75.raw.jsm"
python3 - "$W" "$IN/replay75-preflight.json" "$F2M_MODEL_SHA" <<'PY'
import hashlib
import json
import struct
import sys
...
if (
    sha(w / "replay75.raw.jnnw") != preflight["jnnw_sha256"]
    or sha(w / "replay75.raw.jsm") != preflight["jsm_sha256"]
):
    raise SystemExit("REPLAY75 corpus hash drift")
head = (w / "replay75.raw.jnnw").read_bytes()[:8]
if head[:4] != b"JNNW" or struct.unpack_from("<I", head, 4)[0] != 2_000_000:
    raise SystemExit("REPLAY75 corpus count/header mismatch")
PY
cp "$IN/replay75.jnnw.gz" "$ART/replay75.jnnw.gz"
cp "$IN/replay75.jsm.gz" "$ART/replay75.jsm.gz"
cp "$IN/replay75-mix.json" "$ART/replay75-mix.json"
cp "$IN/replay75-coverage.json" "$ART/replay75-coverage.json"
...
"$W/venv/bin/python" - "$W" "$IN/replay75-preflight.json" \
  "$ART/replay75-split.json" <<'PY'
import hashlib
import json
import sys
from pathlib import Path
...
if split != preflight["split_manifest"]:
    raise SystemExit("REPLAY75 split manifest drift")
if (
    hashlib.sha256((w / "replay75.fit.jnnw").read_bytes()).hexdigest()
    != preflight["split_jnnw_sha256"]
    or hashlib.sha256((w / "replay75.fit.jsm").read_bytes()).hexdigest()
    != preflight["split_jsm_sha256"]
):
    raise SystemExit("REPLAY75 split hash drift")
PY
HOLDOUT="$("$W/venv/bin/python" -c \
  'import json,sys; print(json.load(open(sys.argv[1]))["holdout_records"])' \
  "$ART/replay75-split.json")"
[ "$HOLDOUT" -gt 0 ] || die "REPLAY75 holdout missing"

phase full-feature-dump-and-converged-fit
"$J" --dump-eval-features "$W/replay75.fit.jnnw" "$W/replay75.feat" \
  > "$W/features.log" 2>&1
set +e
env JASS_PATTERNS_DIR="$GEOM" PYTHONPATH="$GEOM:pattern_jass/tools" \
  ...
  --data "$W/replay75.fit.jnnw" --feat "$W/replay75.feat" \
  --out "$W/replay75.pjtw" \
  ...
  --optimizer-report "$ART/replay75-optimizer.json" \
  > "$W/fit.log" 2> "$W/fit.time"
FIT_RC=$?
set -e
if [ -s "$W/replay75.pjtw" ]; then
  gzip -n -c "$W/replay75.pjtw" > "$ART/replay75-checkpoint.pjtw.gz"
fi
[ "$FIT_RC" -eq 0 ] ||
  die "REPLAY75 fit failed rc=$FIT_RC; corpus/checkpoint preserved"
[ -s "$W/replay75.pjtw" ] || die "REPLAY75 model missing"
grep -q 'HOLDOUT_LOGLOSS' "$W/fit.log" || die "REPLAY75 holdout result missing"
"$W/venv/bin/python" - "$ART/replay75-optimizer.json" <<'PY' || die "REPLAY75 optimiser did not converge"
import json
import sys
if not json.load(open(sys.argv[1])).get("success"):
    raise SystemExit(1)
PY
cp "$ART/replay75-checkpoint.pjtw.gz" "$ART/replay75.pjtw.gz"

phase publish-training-screen
"$W/venv/bin/python" - "$W" "$ART" "$IN/replay75-preflight.json" \
...
optimizer = json.load(open(art / "replay75-optimizer.json"))
split = json.load(open(art / "replay75-split.json"))
model_sha = hashlib.sha256((w / "replay75.pjtw").read_bytes()).hexdigest()
payload = {
    "schema": 1,
    "verdict": "REPLAY75_TRAINING_SCREEN_READY",
    "code_sha": code_sha,
...
(art / "replay75-training-summary.json").write_text(serialized)
(art / "JASS_CONTROL_SUMMARY.json").write_text(serialized)
(art / "VERDICT__REPLAY75_TRAINING_SCREEN_READY").write_text(
    "REPLAY75_TRAINING_SCREEN_READY\n"
)
...
PY
phase complete
say "REPLAY75_TRAINING_SCREEN_READY evaluation=true promotion=false automatic_next_job=null"
