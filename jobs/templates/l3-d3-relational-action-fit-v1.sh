...
: "${D3_PREFLIGHT_JOB:?}"; : "${D3_PREFLIGHT_ATTEMPT:?}"; : "${D3_PREFLIGHT_CODE:?}"
cd "$JASS_CODE_DIR"
...
P_ROOT="r2:jass-data/runs/$D3_PREFLIGHT_JOB/$D3_PREFLIGHT_ATTEMPT"
VENV="${JASS_L3_NUMERIC_VENV:-/var/tmp/jass-l3-numeric-venv-current-v1}"; PY="$VENV/bin/python"
FIT_TIMEOUT="${D3_FIT_TIMEOUT_SECONDS:-3600}"
...
say "D3 relational-action terminal fit start job=$JASS_JOB_ID code=$SPEC_CODE fits=1 strength_games=0"

# Authenticate the real-data zero-fit preflight first.
timeout 1800s python3 jobs/tools/fetch_result_files.py --prefix "$P_ROOT" \
  --file artefacts/D3_RELATIONAL_PREFLIGHT.json=D3_RELATIONAL_PREFLIGHT.json \
  --file artefacts/scientific-summary.json=d3-preflight-summary.json \
  --out-dir "$IN" --report "$ART/verified-d3-preflight.json" >"$W/fetch-preflight.log" 2>&1

# Re-fetch immutable source bytes for the terminal fit.
...
"$PY" - "$ART/verified-d3-preflight.json" "$ART/verified-d1-control.json" "$ART/verified-c.json" "$IN" \
  "$D3_PREFLIGHT_JOB" "$D3_PREFLIGHT_ATTEMPT" "$D3_PREFLIGHT_CODE" \
...
if (P.get('job_id'),P.get('attempt_id'),P.get('code_sha'),P.get('result_state'))!=(pj,pa,pc,'completed'):
 raise SystemExit('D3 preflight source identity drift')
...
p=json.load(open(root+'/D3_RELATIONAL_PREFLIGHT.json')); ps=json.load(open(root+'/d3-preflight-summary.json'))
if p != ps: raise SystemExit('D3 preflight summary drift')
if p.get('verdict')!='D3_RELATIONAL_ACTION_PREFLIGHT_COMPLETE_V1' or p.get('fits')!=0:
 raise SystemExit('D3 preflight did not establish support')
if p.get('valid_metrics_read')!=0 or p.get('test_metrics_read')!=0:
 raise SystemExit('D3 preflight consumed heldout metrics')
...
timeout "$FIT_TIMEOUT" env JASS_PATTERNS_DIR="$GEOM" PYTHONPATH="$GEOM:pattern_jass/tools:." \
  "$PY" jobs/tools/d3_relational_action_residual.py fit \
    --preflight "$IN/D3_RELATIONAL_PREFLIGHT.json" \
    ...
    --base-model "$W/WDL_CONTROL.pjtw" --adapter "$ART/D3_RELATIONAL_ADAPTER.npy" \
    --fit-report "$ART/D3_RELATIONAL_FIT.json" --out "$ART/D3_RELATIONAL_TRANSFER_READOUT.json" \
    >"$W/d3-fit.log" 2>&1
cp "$ART/D3_RELATIONAL_TRANSFER_READOUT.json" "$ART/scientific-summary.json"
printf '1\n' >"$ART/FULL_FITS__1"
printf '0\n' >"$ART/STRENGTH_GAMES__0"
printf 'FALSE\n' >"$ART/PROMOTION_AUTHORIZED__FALSE"
printf 'FALSE\n' >"$ART/BAKE_AUTHORIZED__FALSE"
VERDICT=$("$PY" - "$ART/D3_RELATIONAL_TRANSFER_READOUT.json" <<'PY'
import json,sys
print(json.load(open(sys.argv[1]))['verdict'])
PY
)
say "D3 terminal verdict=$VERDICT fits=1 strength_games=0 promotion=false bake=false"
