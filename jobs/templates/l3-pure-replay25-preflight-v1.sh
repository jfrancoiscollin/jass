#!/usr/bin/env bash
# L3-PURE REPLAY25: deterministic full-corpus, runtime and evaluation-pool preflight.
set -Eeuo pipefail
...
PARENT_RECORDS=500000
FRESH_RECORDS=1500000
...
if (
    ...
    raise SystemExit("turnover confirmation does not authorize REPLAY25 preflight")
...
phase construct-replay25-twice
/usr/bin/time -v -o "$W/mix1.time" python3 tools/selfplay_frontier.py mix \
  ...
  --out-data "$W/replay25.raw.jnnw" --out-meta "$W/replay25.raw.jsm" \
  --manifest "$ART/replay25-mix.json" > "$W/replay25-mix.log" 2>&1
/usr/bin/time -v -o "$W/mix2.time" python3 tools/selfplay_frontier.py mix \
  --source PARENT "$W/f2m.raw.jnnw" "$W/f2m.raw.jsm" 1 \
  --source FRESH "$W/m2.raw.jnnw" "$W/m2.raw.jsm" 3 \
  --target-records "$TOTAL_RECORDS" --seed "$MIX_SEED" --namespace-openings \
  --out-data "$W/replay25-repeat.jnnw" --out-meta "$W/replay25-repeat.jsm" \
  --manifest "$W/replay25-repeat-mix.json" > "$W/replay25-repeat-mix.log" 2>&1
cmp -s "$W/replay25.raw.jnnw" "$W/replay25-repeat.jnnw" ||
  die "REPLAY25 data reconstruction is not byte-identical"
cmp -s "$W/replay25.raw.jsm" "$W/replay25-repeat.jsm" ||
  die "REPLAY25 metadata reconstruction is not byte-identical"
python3 - "$ART/replay25-mix.json" "$F2M_CORPUS_SHA" "$F2M_META_SHA" \
...
if (
    ...
    or manifest.get("seed") != 618_034
    or manifest.get("records") != 2_000_000
    or sources.get("PARENT", {}).get("selected_records") != 500_000
    or sources.get("FRESH", {}).get("selected_records") != 1_500_000
    ...
    raise SystemExit("REPLAY25 exact mix contract mismatch")
PY
gzip -n -c "$W/replay25.raw.jnnw" > "$ART/replay25.jnnw.gz"
gzip -n -c "$W/replay25.raw.jsm" > "$ART/replay25.jsm.gz"
...
/usr/bin/time -v -o "$W/split2.time" python3 tools/selfplay_frontier.py split \
  --data "$W/replay25.raw.jnnw" --meta "$W/replay25.raw.jsm" \
  --out-data "$W/replay25-repeat.fit.jnnw" \
  --out-meta "$W/replay25-repeat.fit.jsm" \
  --holdout-mod 10 --seed "$SPLIT_SEED" \
  --manifest "$W/replay25-repeat-split.json" \
  > "$W/replay25-repeat-split.log" 2>&1
cmp -s "$W/replay25.fit.jnnw" "$W/replay25-repeat.fit.jnnw" ||
  die "REPLAY25 split data is not byte-identical"
cmp -s "$W/replay25.fit.jsm" "$W/replay25-repeat.fit.jsm" ||
  die "REPLAY25 split metadata is not byte-identical"
...
env PYTHONPATH="$GEOM:pattern_jass/tools" \
  ...
  python3 jobs/tools/l3_bucket_visits.py --data "$W/replay25.raw.jnnw" \
  --out "$ART/replay25-coverage.json" > "$W/coverage.log" 2>&1
...
python3 jobs/tools/select_independent_opening_pool.py "${opening_args[@]}" \
  --out "$ART/replay25-eval-openings.fen" \
  --manifest "$ART/replay25-eval-openings.json" \
  > "$W/select-openings-a.log" 2>&1
opening_args[1]="$W/open-candidates-b.fen"
python3 jobs/tools/select_independent_opening_pool.py "${opening_args[@]}" \
  --out "$W/replay25-eval-openings-repeat.fen" \
  --manifest "$W/replay25-eval-openings-repeat.json" \
  > "$W/select-openings-b.log" 2>&1
cmp -s "$ART/replay25-eval-openings.fen" \
  "$W/replay25-eval-openings-repeat.fen" ||
...
mix = json.load(open(art / "replay25-mix.json"))
split = json.load(open(art / "replay25-split.json"))
coverage = json.load(open(art / "replay25-coverage.json"))
openings = json.load(open(art / "replay25-eval-openings.json"))
...
if (
    sources["PARENT"]["selected_records"] != 500_000
    or sources["FRESH"]["selected_records"] != 1_500_000
...
payload = {
    ...
    "mix_seed": 618_034,
    "split_seed": 577_215,
    "jnnw_sha256": sha(w / "replay25.raw.jnnw"),
    "jsm_sha256": sha(w / "replay25.raw.jsm"),
    "split_jnnw_sha256": sha(w / "replay25.fit.jnnw"),
    "split_jsm_sha256": sha(w / "replay25.fit.jsm"),
    "mix_manifest": mix,
    "split_manifest": split,
    "coverage": coverage,
    "evaluation_openings": {
        "seed": 1_836_311,
        "sha256": sha(art / "replay25-eval-openings.fen"),
...
(art / "replay25-preflight.json").write_text(serialized)
(art / "JASS_CONTROL_SUMMARY.json").write_text(serialized)
(art / "VERDICT__REPLAY25_PREFLIGHT_READY").write_text(
...
PY
phase complete
say "REPLAY25_PREFLIGHT_READY training=true promotion=false automatic_next_job=null"
