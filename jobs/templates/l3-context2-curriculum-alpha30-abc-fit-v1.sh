#!/usr/bin/env bash
# Refit the complete Curriculum recipe with aligned/shuffled CTX2 alpha=0.30.
# Reuse the certified pure CTX2 predictions; do not refit either teacher or A.
...
PURE_CTX2_ROOT="r2:jass-data/runs/cpx62-1384-l3-context2-curriculum-full-relabel-fit-v1/20260817T073156Z-05554755"
CURRENT_CTX2_A30_ROOT="r2:jass-data/runs/home-1373-l3-context2-phase-tactical-fit-v1/20260816T214312Z-9e224d6e"
...
MAXIT=2000; CHUNK=20000; FIT_TIMEOUT=28800
  ...
  ( t0=$(date +%s)
    while true; do
      { printf 'time_fr=%s\n' "$(TZ=Europe/Paris date '+%Y-%m-%dT%H:%M:%S%z')"
        ...
        for arm in B-mega C-mega B-current C-current; do
          [ -f "$W/fit-$arm.log" ] &&
...
[[ "$JASS_JOB_ID" =~ ^cpx62-([0-9]+)-l3-context2-curriculum-alpha30-abc-fit-v1$ ]] ||
...
say "host=$(hostname) nproc=$(nproc) free_mb=$DFA mode=ctx2_curriculum_alpha30_abc"
monitor

stage repository-contract-tests
python3 -m py_compile jobs/tools/jass_megacorpus_materialize.py \
  jobs/tools/l3_conditional_targets.py jobs/tools/l3_context2_recompose_targets.py \
  jobs/tools/verify_optimizer_convergence.py
"$PY" -m unittest jobs.tests.test_l3_context2_recompose_targets \
  jobs.tests.test_l3_context2_curriculum_alpha30_protocol \
...
timeout 1800s python3 jobs/tools/fetch_result_files.py --prefix "$ABC_ROOT" \
  ...
  --file artefacts/current_2m-context30.npy.gz=current-ctx1-alpha30.npy.gz \
  --file artefacts/current_2m-conditional-targets.json=current-ctx1-alpha30.json \
  --file artefacts/mega_full_4m-context30.npy.gz=mega-ctx1-alpha30.npy.gz \
  --file artefacts/mega_full_4m-conditional-targets.json=mega-ctx1-alpha30.json \
...
timeout 1800s python3 jobs/tools/fetch_result_files.py --prefix "$PURE_CTX2_ROOT" \
  --file artefacts/JASS_CONTROL_SUMMARY.json=pure-ctx2-summary.json \
  --file artefacts/current_2m-conditional-targets.json=current-pure-ctx2.json \
  --file artefacts/current_2m-ctx2-alpha100.npy.gz=current-pure-ctx2.npy.gz \
  --file artefacts/mega_full_4m-conditional-targets.json=mega-pure-ctx2.json \
  --file artefacts/mega_full_4m-ctx2-alpha100.npy.gz=mega-pure-ctx2.npy.gz \
  --out-dir "$IN" --report "$ART/verified-pure-ctx2.json" >"$W/fetch-pure-ctx2.log" 2>&1
timeout 1800s python3 jobs/tools/fetch_result_files.py --prefix "$CURRENT_CTX2_A30_ROOT" \
  --file artefacts/JASS_CONTROL_SUMMARY.json=current-ctx2-a30-summary.json \
  --file artefacts/ctx2-aligned-target.npy.gz=current-ctx2-a30-aligned.npy.gz \
  --file artefacts/ctx2-shuffled-target.npy.gz=current-ctx2-a30-shuffled.npy.gz \
  --out-dir "$IN" --report "$ART/verified-current-ctx2-a30.json" \
  >"$W/fetch-current-ctx2-a30.log" 2>&1

"$PY" - "$ART" "$IN/uniform-summary.json" "$IN/abc-summary.json" \
  "$IN/curriculum-summary.json" "$IN/pure-ctx2-summary.json" \
  "$IN/current-ctx2-a30-summary.json" <<'PY'
import json,sys
from pathlib import Path
art=Path(sys.argv[1]); uniform=json.load(open(sys.argv[2])); abc=json.load(open(sys.argv[3]))
curr=json.load(open(sys.argv[4])); pure=json.load(open(sys.argv[5]))
current_a30=json.load(open(sys.argv[6]))
expected={
...
expected.update({
 'verified-pure-ctx2.json':('cpx62-1384-l3-context2-curriculum-full-relabel-fit-v1','20260817T073156Z-05554755','05554755d4268388aafcb0062026391cedd9627e'),
 'verified-current-ctx2-a30.json':('home-1373-l3-context2-phase-tactical-fit-v1','20260816T214312Z-9e224d6e','9e224d6ec7583d3c041755a35559bf559d380f8f')})
...
if pure.get('verdict')!='JASS_CONTEXT2_CURRICULUM_FULL_RELABEL_MODEL_READY':
 raise SystemExit('pure CTX2 certificate drift')
if current_a30.get('verdict')!='JASS_CONTEXT2_PHASE_TACTICAL_MODELS_READY':
 raise SystemExit('current CTX2 alpha30 certificate drift')
PY
...
gunzip -c "$IN/current-pure-ctx2.npy.gz" >"$W/current-pure-ctx2.npy"
gunzip -c "$IN/mega-pure-ctx2.npy.gz" >"$W/mega-pure-ctx2.npy"
gunzip -c "$IN/current-ctx1-alpha30.npy.gz" >"$W/current-ctx1-alpha30.npy"
gunzip -c "$IN/mega-ctx1-alpha30.npy.gz" >"$W/mega-ctx1-alpha30.npy"
gunzip -c "$IN/current-ctx2-a30-aligned.npy.gz" >"$W/current-certified-aligned.npy"
gunzip -c "$IN/current-ctx2-a30-shuffled.npy.gz" >"$W/current-certified-shuffled.npy"
...
stage dump-production-features
...
PY
  ...
  [ "$K" -eq "$EXPECTED_EXTRAS" ] || die "$arm production feature width drift: $K"
done
...
stage recompose-aligned-and-shuffled-ctx2-alpha30-targets
for arm in mega_full_4m current_2m; do
  read -r records train holdout < <(arm_counts "$arm")
  short=${arm%_*}
  [ "$arm" = mega_full_4m ] && short=mega
  /usr/bin/time -f '%e' -o "$W/targets-$arm.seconds" \
    "$PY" jobs/tools/l3_context2_recompose_targets.py \
      --data "$W/$arm.jnnw" --meta "$W/$arm.jsm" \
      --pure-context-target "$W/$short-pure-ctx2.npy" \
      --pure-context-report "$IN/$short-pure-ctx2.json" \
      --reference-target "$W/$short-ctx1-alpha30.npy" \
      --reference-report "$IN/$short-ctx1-alpha30.json" \
      --train-count "$train" --aligned-out "$W/$arm-aligned.npy" \
      --shuffled-out "$W/$arm-shuffled.npy" --report "$ART/$arm-targets.json" \
      --alpha 0.30 --fold-count 5 --fold-seed 20260811 \
      --shuffle-seed 20260812 --shuffle-phase-bins 4 \
      >"$W/targets-$arm.log" 2>&1
  gzip -n -c "$W/$arm-aligned.npy" >"$ART/$arm-ctx2-alpha30-aligned.npy.gz"
  gzip -n -c "$W/$arm-shuffled.npy" >"$ART/$arm-ctx2-alpha30-shuffled.npy.gz"
  "$PY" - "$ART/$arm-targets.json" "$records" "$train" "$holdout" <<'PY'
import json,sys
r=json.load(open(sys.argv[1])); p=r['strict_protocol']; ref=r['reference_ctx1_alpha30']
if (r.get('schema')!='jass.l3_context2_recomposed_targets.v1'
    or r.get('target',{}).get('alpha')!=0.30): raise SystemExit('CTX2 alpha30 target drift')
...
if (p.get('fold_group')!='opening_id' or p.get('fixed_point_count')!=0
    or p.get('shuffle_stratification')!='terminal_wdl_black_x_tempo_phase_4_bins'
    or not p.get('all_final_target_marginals_preserved')):
 raise SystemExit('strict CTX2 shuffle contract failed')
for key in ('aligned_std_ratio','shuffled_std_ratio'):
 if not 0.95 <= ref[key] <= 1.05:
  raise SystemExit(f'alpha30 target scale drift: {key}={ref[key]}')
PY
done

fit_stage(){
  local label="$1" corpus="$2" target="$3" prior="$4" records train holdout
  read -r records train holdout < <(arm_counts "$corpus")
  stage "fit-$label"
  /usr/bin/time -f '%e' -o "$W/fit-$label.seconds" timeout "$FIT_TIMEOUT" \
    env JASS_PATTERNS_DIR="$GEOM" PYTHONPATH="$GEOM:pattern_jass/tools" PYTHONUNBUFFERED=1 \
    "$PY" pattern_jass/tools/train_stream.py \
      --data "$W/$corpus.jnnw" --feat "$W/$corpus.feat" --out "$W/$label.pjtw" \
      --target external --target-values "$target" \
      --targets-report "$ART/$label-target-consumption.json" \
      ...
      --optimizer-report "$ART/$label-optimizer.json" >"$W/fit-$label.log" 2>&1
  [ -s "$W/$label.pjtw" ] || die "$label produced no model"
  "$PY" jobs/tools/verify_optimizer_convergence.py \
    --report "$ART/$label-optimizer.json" --label "$label" \
    --expected-max-iterations "$MAXIT" --expected-maxcor 20 --expected-gtol 1e-4 \
    --receipt "$ART/$label-convergence.json"
  gzip -n -c "$W/$label.pjtw" >"$ART/$label.pjtw.gz"
}

stage validate-current-target-recomposition-against-original-alpha30-fit
"$PY" - "$W" <<'PY'
import numpy as np,sys
from pathlib import Path
w=Path(sys.argv[1])
for kind in ('aligned','shuffled'):
 got=np.load(w/f'current_2m-{kind}.npy')
 old=np.load(w/f'current-certified-{kind}.npy')
 delta=float(np.max(np.abs(got.astype(np.float64)-old.astype(np.float64))))
 if delta>2e-7: raise SystemExit(f'current {kind} recomposition drift: {delta}')
 print(f'current_{kind}_recomposition_max_abs_delta={delta:.9g}')
PY

fit_stage B-mega mega_full_4m "$W/mega_full_4m-aligned.npy" "$W/l2low.pjtw"
fit_stage C-mega mega_full_4m "$W/mega_full_4m-shuffled.npy" "$W/l2low.pjtw"
fit_stage B-current current_2m "$W/current_2m-aligned.npy" "$W/B-mega.pjtw"
fit_stage C-current current_2m "$W/current_2m-shuffled.npy" "$W/C-mega.pjtw"

stage publish-alpha30-abc-certificate
...
specs={
 'B-mega':('mega_full_4m','aligned'),
 'C-mega':('mega_full_4m','shuffled'),
 'B-current':('current_2m','aligned'),
 'C-current':('current_2m','shuffled')}
stages={}
for label,(corpus,kind) in specs.items():
 targets=json.load(open(art/f'{corpus}-targets.json'))
 consumed=json.load(open(art/f'{label}-target-consumption.json'))
 target_sha=targets['outputs'][f'{kind}_sha256']
 if consumed['source']['sha256']!=target_sha:
  raise SystemExit(f'{label}: target consumption drift')
 model=w/f'{label}.pjtw'; model_rms=rms(model)
 stages[label]={'corpus':corpus,'target_kind':kind,
  'model_raw_sha256':sha(model),'model_gz_sha256':sha(art/f'{label}.pjtw.gz'),
  'structure':structure(model),'optimizer':json.load(open(art/f'{label}-optimizer.json')),
  'convergence':json.load(open(art/f'{label}-convergence.json')),
  'target_sha256':target_sha,'model_rms':model_rms,
  'model_rms_ratio_vs_curriculum':model_rms/champion_rms,
  'fit_seconds':float((w/f'fit-{label}.seconds').read_text())}
if len({champion_sha,stages['B-current']['model_raw_sha256'],stages['C-current']['model_raw_sha256']})!=3:
 raise SystemExit('A/B/C final models are not distinct')
for label in ('B-current','C-current'):
 ratio=stages[label]['model_rms_ratio_vs_curriculum']
 if not 0.80 <= ratio <= 1.20:
  raise SystemExit(f'{label}: model RMS scale guard failed: {ratio}')
target_reports={}
for corpus in ('mega_full_4m','current_2m'):
 report=json.load(open(art/f'{corpus}-targets.json'))
 ref=report['reference_ctx1_alpha30']
 target_reports[corpus]={
  'aligned_sha256':report['outputs']['aligned_sha256'],
  'shuffled_sha256':report['outputs']['shuffled_sha256'],
  'pure_ctx2_sha256':report['source']['pure_context_target_sha256'],
  'aligned_std_ratio_vs_ctx1_alpha30':ref['aligned_std_ratio'],
  'shuffled_std_ratio_vs_ctx1_alpha30':ref['shuffled_std_ratio'],
  'aligned_mean_delta_vs_ctx1_alpha30':ref['aligned_mean_delta'],
  'shuffle':report['strict_protocol']}
payload={'schema':'jass.l3_context2_curriculum_alpha30_abc_fit.v1',
 'verdict':'JASS_CONTEXT2_CURRICULUM_ALPHA30_ABC_MODELS_READY','code_sha':code,
 'baseline':{'label':'CURRICULUM','model_raw_sha256':champion_sha,
  'model_rms':champion_rms,'source_job':'cpx62-1341-jass-megacorpus-arm-d-fit-v1',
  'baseline_reused_without_refit':True},
 'arms':{
  'A':{'label':'CURRICULUM_CTX1_ALPHA30','reused_without_refit':True,
       'model_raw_sha256':champion_sha},
  'B':{'label':'CURRICULUM_CTX2_ALPHA30_ALIGNED',
       'model_raw_sha256':stages['B-current']['model_raw_sha256'],
       'artifact':'B-current.pjtw.gz'},
  'C':{'label':'CURRICULUM_CTX2_ALPHA30_SHUFFLED',
       'model_raw_sha256':stages['C-current']['model_raw_sha256'],
       'artifact':'C-current.pjtw.gz'}},
 'recipe':{'sequence':['MEGA_FULL_4M','CURRENT_2M'],
  'targets':{'B':'CTX2_PHASE_TACTICAL_ALIGNED','C':'CTX2_PHASE_TACTICAL_SHUFFLED'},
  'alpha':0.30,'architecture':'8cf_exact_fold_tempo_120_extras',
  'first_prior_mean':'L2LOW','second_prior_mean':'same_arm_refitted_MEGA_FULL_4M',
  'prior_decay':0,'l2':1e-5,
  'gtol':1e-4,'max_iterations':2000,'lbfgs_maxcor':20},
 'teacher_reuse':{'source_job':'cpx62-1384-l3-context2-curriculum-full-relabel-fit-v1',
  'pure_alpha100_predictions_reused':True,'teacher_refits':0},
 'target_reports':target_reports,'stages':stages,
...
(art/'VERDICT__JASS_CONTEXT2_CURRICULUM_ALPHA30_ABC_MODELS_READY').touch()
(art/'PROMOTION_AUTHORIZED__FALSE').touch(); (art/'AUTOMATIC_NEXT_JOB__NULL').touch()
print(json.dumps(payload,sort_keys=True))
PY
say "JASS_CONTEXT2_CURRICULUM_ALPHA30_ABC_MODELS_READY alpha=0.30 fits=4 games=0 promotion=false"
