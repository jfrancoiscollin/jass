...
say "D1 v2 start job=$JASS_JOB_ID code=$EXPECTED_CODE_SHA science=two_fits_no_games"
...
[ "$RECORDS" -eq "$RECORDS_EXPECTED" ] && [ "$TRAIN" -eq "$TRAIN_EXPECTED" ] && [ "$HOLD" -eq "$HOLDOUT" ] || die "CURRENT_2M historical cardinality drift"
...
for arm in WDL_CONTROL WDL_LISTWISE; do
  timeout "$FIT_TIMEOUT" env JASS_PATTERNS_DIR="$GEOM" PYTHONPATH="$GEOM:pattern_jass/tools:." PYTHONUNBUFFERED=1 \
    "$PY" jobs/tools/d1_listwise_fit_historical_split.py --arm "$arm" \
...
env JASS_PATTERNS_DIR="$GEOM" PYTHONPATH="$GEOM:pattern_jass/tools:." \
  "$PY" jobs/tools/d1_postfit_readout_historical_split.py \
