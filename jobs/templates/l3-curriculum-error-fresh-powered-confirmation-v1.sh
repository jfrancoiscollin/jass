...
CONFIRMATION_PAIRS="${CONFIRMATION_PAIRS:-300}"
CONFIRMATION_MODULE="${CONFIRMATION_MODULE:-jobs.tools.l3_curriculum_error_fresh_powered_confirmation}"
CONFIRMATION_TOOL="${CONFIRMATION_TOOL:-jobs/tools/l3_curriculum_error_fresh_powered_confirmation.py}"
CONFIRMATION_READY_VERDICT="${CONFIRMATION_READY_VERDICT:-JASS_CURRICULUM_ERROR_FRESH_POWERED_CONFIRMATION_READY}"
CONFIRMATION_NOT_ESTABLISHED_VERDICT="${CONFIRMATION_NOT_ESTABLISHED_VERDICT:-JASS_CURRICULUM_ERROR_FRESH_POWERED_CONFIRMATION_NOT_ESTABLISHED}"
CONFIRMATION_TERMINAL_SCHEMA="${CONFIRMATION_TERMINAL_SCHEMA:-jass.curriculum_error_fresh_powered_confirmation_terminal.v1}"
CONFIRMATION_FRESH_LABELS_KEY="${CONFIRMATION_FRESH_LABELS_KEY:-fresh_extension_labels_used_for_fit}"
CONFIRMATION_EXPERIMENT_NAME="${CONFIRMATION_EXPERIMENT_NAME:-CURRICULUM_ERROR_FRESH_POWERED_CONFIRMATION}"
NSH=16; MAX_ROUNDS="${CONFIRMATION_MAX_ROUNDS:-32}"; TARGET_STATES_PER_ROUND=256; CACHE_MB=128
...
[[ "$JASS_JOB_ID" =~ ^cpx62-[0-9]+-l3-curriculum-error-(fresh-powered|endgame-abstention)-confirmation-v1$ ]] || die "invalid job nomenclature"
...
[ "${EXACT_FRESH_CONFIRMATION_ONLY:-0}" = 1 ] || die "fresh confirmation guard missing"
[ "${FIRST_VALID_FROZEN_ONLY:-${FIRST_VALID_300_ONLY:-0}}" = 1 ] || die "first-valid frozen-order guard missing"
...
say "experiment=$CONFIRMATION_EXPERIMENT_NAME pairs=$CONFIRMATION_PAIRS alpha=300 cap=100 mode=strict_both_change threshold=10"
monitor

stage repository-contract-tests
python3 -m py_compile jobs/tools/fetch_result_subset.py "$CONFIRMATION_TOOL"
test_modules=(
...
[ -z "${CONFIRMATION_EXTRA_TEST_MODULES:-}" ] || read -r -a extra_test_modules <<<"$CONFIRMATION_EXTRA_TEST_MODULES"
[ -z "${CONFIRMATION_EXTRA_TEST_MODULES:-}" ] || test_modules+=("${extra_test_modules[@]}")
...
python3 -m "$CONFIRMATION_MODULE" prepare \
...
python3 -m "$CONFIRMATION_MODULE" normalize \
...
stage exact-label-first-${CONFIRMATION_PAIRS}-valid-pairs-in-frozen-order
CACHE=""
COMPLETE=0
for round in $(seq 0 $((MAX_ROUNDS-1))); do
  ...
  python3 -m "$CONFIRMATION_MODULE" plan "${args[@]}" >"$W/plan-$round.log" 2>&1
  ...
  python3 -m "$CONFIRMATION_MODULE" ingest \
...
[ "$COMPLETE" -eq 1 ] || die "first-$CONFIRMATION_PAIRS exact pair selection exceeded $MAX_ROUNDS batches"

stage finalize-repacked-authenticated-fresh-atlas
python3 -m "$CONFIRMATION_MODULE" finalize \
...
python3 -m "$CONFIRMATION_MODULE" confirm \
...
python3 - "$ART" "$EXPECTED_CODE_SHA" "$AVAILABILITY_SOURCE_JOB" "$AVAILABILITY_SOURCE_ATTEMPT" "$AVAILABILITY_SOURCE_CODE" \
  "$PREREG_SOURCE_JOB" "$PREREG_SOURCE_ATTEMPT" "$PREREG_SOURCE_CODE" \
  "$TRAINING_SOURCE_JOB" "$TRAINING_SOURCE_ATTEMPT" "$TRAINING_SOURCE_CODE" \
  "$CONFIRMATION_READY_VERDICT" "$CONFIRMATION_NOT_ESTABLISHED_VERDICT" \
  "$CONFIRMATION_TERMINAL_SCHEMA" "$CONFIRMATION_FRESH_LABELS_KEY" "$CONFIRMATION_PAIRS" <<'PY_FINAL'
import json,re,sys
from pathlib import Path
art=Path(sys.argv[1]); code=sys.argv[2]; availability=tuple(sys.argv[3:6]); prereg=tuple(sys.argv[6:9]); training=tuple(sys.argv[9:12]); ready=sys.argv[12]; not_established=sys.argv[13]; terminal_schema=sys.argv[14]; labels_key=sys.argv[15]; expected_pairs=int(sys.argv[16])
report=json.load(open(art/'fresh-powered-confirmation.json'))
if report.get('verdict') not in {ready,not_established}: raise SystemExit('fresh confirmation verdict drift')
if report.get(labels_key) is not False or report.get('fresh_pairs')!=expected_pairs: raise SystemExit('fresh isolation/cardinality drift')
...
payload={**report,'schema':terminal_schema,'code_sha':code,
...
(art/f"PAIRS__{expected_pairs}__POOL1_{report['fresh_pairs_by_pool'].get('pool1',0)}__POOL2_{report['fresh_pairs_by_pool'].get('pool2',0)}").touch()
...
for name in ('FRESH_LABELS_USED_FOR_FIT__FALSE','FRESH_EXTENSION_LABELS_USED_FOR_FIT__FALSE','PATTERNEVAL_FITS__0','PRODUCTION_MODEL_FITS__0','STRENGTH_GAMES__0','NEW_SELFPLAY__0','FROZEN_READS__0','PROMOTION_AUTHORIZED__FALSE','AUTOMATIC_CONTINUATION__FALSE'): (art/name).touch()
proof=report.get('rule_proof')
if proof:
 (art/f"ENDGAME_RULE__STATES_{proof['endgame_states']}__WOULD_INTERVENE_{proof['would_intervene_without_abstention']}__ACTUAL_INTERVENTIONS_{proof['endgame_interventions']}").touch()
 (art/f"RULE_IDENTITY__ENDGAME_{str(proof['endgame_decisions_bit_identical_to_anchor']).upper()}__NONENDGAME_{str(proof['non_endgame_decisions_bit_identical_to_frozen_residual']).upper()}").touch()
PY_FINAL
VERDICT=$(python3 -c 'import json,sys;print(json.load(open(sys.argv[1]))["verdict"])' "$ART/JASS_CONTROL_SUMMARY.json")
stage completed
say "$VERDICT pairs=$CONFIRMATION_PAIRS fit_source=1508 fresh_fit=0 strength=0 frozen=0 promotion=false"
