...
# Two independent fixed-pool W/D/L equivalence checks: candidate self-play vs Scan self-play.
set -Eeuo pipefail
...
for pool in a b; do
  ...
  for shard in $(seq 0 $((NSHARDS-1))); do
    ...
    python3 jobs/tools/imbalance2_scan_gate.py run --jass "$J" --scan "$SCAN_BIN" \
      ...
      --scan-bb-size 0 --shard "$shard" --nshards "$NSHARDS" --out "$out" \
      > "$W/${pool}.s${shard}.log" 2>&1 &
    pids+=("$!")
...
out={'schema':2,'lineage':'L3-IMBALANCE2','protocol':'candidate-selfplay_vs_scan-selfplay',
