...
[[ "$FORCE_DEPTH" =~ ^[0-9]+$ ]] && [ "$FORCE_DEPTH" -ge 1 ] && [ "$FORCE_DEPTH" -le 64 ] ||
  die "FORCE_DEPTH must be an integer in [1,64]"
python3 - "$MOVETIME" <<'PYB' || die "MOVETIME must be finite and in (0,30] seconds"
import math, sys
v = float(sys.argv[1])
raise SystemExit(0 if math.isfinite(v) and 0.0 < v <= 30.0 else 1)
PYB
