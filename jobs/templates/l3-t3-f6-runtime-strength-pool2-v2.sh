#!/usr/bin/env bash
# R1-v2 Pool2: exactly one unchanged native replication after positive Pool1.
set -Eeuo pipefail
...
GEN_SEED="${T3_F6_POOL2_GEN_SEED:-2026091901}"
SELECT_SEED="${T3_F6_POOL2_SELECT_SEED:-2026091902}"
NATIVE_BOOTSTRAP_SEED="${T3_F6_POOL2_BOOTSTRAP_SEED:-2026091903}"
CHAINED_NATIVE_SEED="${T3_F6_CHAINED_BOOTSTRAP_SEED:-2026092001}"
CANDIDATES=30000
OPENINGS=3000
GAMES=6000
BOOTSTRAP=200000
...
for pass in a b; do "$J" --gen-opening-pool "$CANDIDATES" "$W/pool2-candidates-$pass.fen" 8 32 20 "$GEN_SEED" >"$W/pool2-generate-$pass.log" 2>&1; done
...
python3 jobs/tools/t3_f6_force_pool_select.py --candidates "$W/pool2-candidates-a.fen" \
  ...
  --out "$ART/pool2-openings.fen" --report "$ART/pool2-provenance.json" >"$W/pool2-select.log" 2>&1
...
timeout -k 120s 43200s "$PY" jobs/tools/run_jass_gate_bounded.py \
  ...
  --search-params-a "$Q00" --search-params-b "$Q00" --openings-file "$ART/pool2-openings.fen" \
  ...
  --work-dir "$W/gate-pool2-native" --out "$FORCE/pool2-native.json" >"$W/force-pool2-native.log" 2>&1 || die "Pool2 native gate failed"
python3 - "$W/gate-pool2-native" "$ART/pool2-native-games.jsonl.gz" "$GAMES" <<'PY_GAMES'
...
"$PY" jobs/tools/t3_f6_strength_readout_v2.py --pool1-native "$IN/pool1-native.json" \
  --pool2-native "$FORCE/pool2-native.json" --model "$IN/t3-a-f6-only.json" \
...
: >"$ART/POOL3_AUTHORIZED__FALSE"; : >"$ART/PROMOTION_AUTHORIZED__FALSE"; : >"$ART/BAKE__FALSE"
say "$VERDICT total_native_games=12000 total_q00_games=0 promotion=false bake=false"
