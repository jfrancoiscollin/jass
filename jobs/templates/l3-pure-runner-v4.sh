...
# L3-PURE C1-Q1 lineage-native search fork.
# Four matched variants form the 2x2 threat-extension x selective-sac screen.
# Every variant starts from the same material seed, uses terminal WDL only,
# skips the unused score-label search, and disables the moving frontier.
set -Eeuo pipefail

: "${JASS_CODE_DIR:?runner v4 must provide JASS_CODE_DIR}"
: "${JASS_RESULT_DIR:?runner v4 must provide JASS_RESULT_DIR}"
: "${JASS_ARTEFACT_DIR:?runner v4 must provide JASS_ARTEFACT_DIR}"
: "${JASS_JOB_ID:?runner v4 must provide JASS_JOB_ID}"
: "${ARM:?set ARM=A}"
: "${L3_VARIANT:?set the pre-registered C1-Q1 variant}"
[ "$ARM" = A ] || { echo "ABORT: C1-Q1 requires ARM=A" >&2; exit 2; }
: "${FRONTIER_FRAC:=0}"
[ "$FRONTIER_FRAC" = 0 ] || { echo "ABORT: C1-Q1 forbids frontier seeding" >&2; exit 2; }
...
case "$L3_VARIANT" in
  ...
  Q10_THREAT)
    EXPECTED_SEARCH_OVERRIDES="qs_threat_ext=1,qs_sacs=0,qs_sacs_depth0_only=1,qs_forcing_depth=0,qs_promo_depth=0" ;;
  Q01_SACS)
    EXPECTED_SEARCH_OVERRIDES="qs_threat_ext=0,qs_sacs=1,qs_sacs_depth0_only=1,qs_forcing_depth=0,qs_promo_depth=0" ;;
  Q11_THREAT_SACS)
    EXPECTED_SEARCH_OVERRIDES="qs_threat_ext=1,qs_sacs=1,qs_sacs_depth0_only=1,qs_forcing_depth=0,qs_promo_depth=0" ;;
  *) echo "ABORT: unknown C1-Q1 variant: $L3_VARIANT" >&2; exit 2 ;;
esac
L3_SEARCH_OVERRIDES="${L3_SEARCH_OVERRIDES:-$EXPECTED_SEARCH_OVERRIDES}"
[ "$L3_SEARCH_OVERRIDES" = "$EXPECTED_SEARCH_OVERRIDES" ] || {
  echo "ABORT: overrides do not match pre-registration for $L3_VARIANT" >&2
  exit 2
}
...
# Overrides are applied only to the five pre-registered quiescence keys.
...
[ "$NGEN" -eq 2 ] || die "C1-Q1 is pre-registered for exactly 2 generations"
[ "$FRESH" -eq 150000 ] || die "C1-Q1 is pre-registered for exactly 150000 records/generation"
[ "$NSHARDS" -eq 8 ] || die "C1-Q1 requires NSHARDS=8"
[ "$PAR_GEN" -eq 8 ] || die "C1-Q1 requires PAR_GEN=8"
[ "$MAXPLIES" -eq 260 ] || die "C1-Q1 requires MAXPLIES=260"
[ "$LABEL_DEPTH" -eq 4 ] || die "C1-Q1 keeps the ignored label-depth argument at 4"
[ "$RANDOM_OPEN_PLIES" -eq 8 ] || die "C1-Q1 requires RANDOM_OPEN_PLIES=8"
[ "$EXPLORE_EPS" -eq 8 ] || die "C1-Q1 requires EXPLORE_EPS=8"
[ "$EXPLORE_DECAY_PLIES" -eq 60 ] || die "C1-Q1 requires EXPLORE_DECAY_PLIES=60"
[ "$HOLDOUT_MOD" -eq 10 ] || die "C1-Q1 requires HOLDOUT_MOD=10"
{ [ "$BASE_SEED" -eq 271828 ] || [ "$BASE_SEED" -eq 161803 ]; } || die "C1-Q1 requires a pre-registered seed (271828 primary or 161803 replication)"
[ "$MAXIT" -eq 25 ] || die "C1-Q1 requires MAXIT=25"
[ "$L2" = 3e-5 ] || die "C1-Q1 requires L2=3e-5"
[ "$CHUNK" -eq 500000 ] || die "C1-Q1 requires CHUNK=500000"
NPROC="$(nproc)"
...
python3 jobs/tests/test_l3_doe_q1_prepared.py > "$W/test-c1-q1.log" 2>&1 \
  || die "C1-Q1 contract tests failed"
...
# Runner-v4 pins one immutable SHA. Verify the performance-critical sources in
