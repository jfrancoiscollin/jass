...
: "${EXPECTED_P1_JOB_ID:?}"
: "${W0_PREFIX:?immutable completed 0877 W0 prefix}"
: "${EXPECTED_W0_JOB_ID:?}"
...
RESAMPLE_SEED="${RESAMPLE_SEED:-271832}"
L2="${L2:-3e-5}"
MAXIT="${MAXIT:-25}"
CHUNK="${CHUNK:-500000}"
...
finalize(){
  ...
  rm -rf "$W/build" "$INPUTS" "$GEOM" "$W"/*.feat "$W"/*.jnnw "$W"/*.jsm 2>/dev/null || true
  exit "$rc"
}
trap finalize EXIT
...
[ "${W1_ADAPTIVE_GO:-0}" = 1 ] || die "W1_ADAPTIVE_GO=1 missing"
[ "$POOL_SEED" -eq 141421 ] || die "W1 E64/F64 seed must remain 141421"
[ "$PLATEAU_PER_STRATUM" -eq 64 ] || die "W1 requires 64 positions per stratum/pool"
[ "$DEPTH" -eq 10 ] && [ "$MAXPLIES" -eq 400 ] || die "W1 conversion gate requires d10/maxplies400"
[ "$NSHARDS" -eq 8 ] && [ "$PAR" -eq 8 ] || die "W1 requires 8 shards / 8 parallel"
[ "$BOOTSTRAP" -ge 10000 ] || die "W1 requires >=10000 bootstrap replicates"
[ "$HOLDOUT_MOD" -eq 10 ] && [ "$BASE_SEED" -eq 271828 ] && [ "$RESAMPLE_SEED" -eq 271832 ] || die "G4 split/resample contract mismatch"
...
python3 -m py_compile \
  ...
  jobs/tools/imbalance2_adaptive_reweight.py \
  jobs/tools/make_imbalance2_pools.py \
  jobs/tools/imbalance2_scan_gate.py \
  jobs/tools/imbalance2_weight_generalist.py \
  jobs/tools/imbalance2_w1_screen_report.py \
...
python3 jobs/tools/fetch_result_files.py --prefix "$P1_PREFIX" \
  ...
  --file artefacts/g4.pjtw.gz=published-g4.pjtw.gz \
  --file artefacts/g4-source.jnnw.gz=g4-source.jnnw.gz \
  --file artefacts/g4-source.jsm.gz=g4-source.jsm.gz \
  --file artefacts/g4-reweight.json=published-g4-reweight.json \
  --file artefacts/g4-split.json=published-g4-split.json \
...
python3 jobs/tools/fetch_result_files.py --prefix "$W0_PREFIX" \
  ...
  --expected-state completed --out-dir "$INPUTS/w0" --report "$ART/verified-w0-source.json" \
  > "$W/fetch-w0.log" 2>&1 || die "W0 source fetch failed"
...
for name in ('g3.pjtw.gz','published-g4.pjtw.gz'):
 if not (root/'p1'/name).is_file(): raise SystemExit(f'missing {name}')
...
(root/'g3.pjtw').write_bytes(gzip.decompress((root/'p1/g3.pjtw.gz').read_bytes()))
w0=json.loads((root/'w0/w0-policy.json').read_text())
if w0.get('decision') != 'W0_ORACLE_WEIGHT_CALIBRATION_READY' or w0.get('classification') != 'STRATUM_ORACLE_WEIGHTING_SUPPORTED_DENSITY_ONLY_NOT_SUPPORTED':
 raise SystemExit('W0 verdict mismatch')
if w0.get('diagnostics',{}).get('pool_stability_pass') is not True or w0.get('diagnostics',{}).get('density_only_hypothesis_pass') is not False:
 raise SystemExit('W0 stability/density contract mismatch')
(root/'search-params.txt').write_text(search+'\n')
proof={'schema':1,'protocol':'w1-adaptive-source-contract','p1_job_id':p1id,'w0_job_id':w0id,
 'p1_prefix':json.loads((art/'verified-p1-source.json').read_text()).get('prefix'),
 'w0_prefix':json.loads((art/'verified-w0-source.json').read_text()).get('prefix'),
 ...
 'w0_policy_sha256':hashlib.sha256((root/'w0/w0-policy.json').read_bytes()).hexdigest(),
 'search_params_sha256':hashlib.sha256(search.encode()).hexdigest(),
 'same_source_bytes_both_arms':True,'same_warm_start_both_arms':True,
 'scan_used_for_training_labels':False,'oracle_used_for_sampling_weights':True,'new_training_selfplay_games':0}
...
python3 tools/selfplay_frontier.py split \
  ...
  --holdout-mod "$HOLDOUT_MOD" --seed "$BASE_SEED" --manifest "$ART/w1-split.json" \
  > "$W/split.log" 2>&1
HOLDOUT_COUNT="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["holdout_records"])' "$ART/w1-split.json")"
...
env IMBALANCE2_REWEIGHT_POLICY=role-aware-v2 python3 jobs/tools/prepare_imbalance2_training.py reweight \
  ...
  --win-weight 1 --draw-weight 2 --loss-weight 4 --seed "$RESAMPLE_SEED" --report "$ART/w1-control-reweight.json" \
  > "$W/control-reweight.log" 2>&1
python3 jobs/tools/imbalance2_adaptive_reweight.py \
  --input "$W/g4.fit.jnnw" --output "$W/adaptive.weighted.jnnw" \
  --policy "$INPUTS/w0/w0-policy.json" --holdout-count "$HOLDOUT_COUNT" \
  --seed "$RESAMPLE_SEED" --report "$ART/w1-adaptive-reweight.json" \
  > "$W/adaptive-reweight.log" 2>&1
python3 - "$W/control.weighted.jnnw" "$W/adaptive.weighted.jnnw" "$HOLDOUT_COUNT" "$ART/w1-training-bytes-contract.json" <<'PY'
import hashlib,json,struct,sys
from pathlib import Path
c=Path(sys.argv[1]); a=Path(sys.argv[2]); hold=int(sys.argv[3]); out=Path(sys.argv[4]); rec=38
cr=c.read_bytes(); ar=a.read_bytes()
for name,raw in (('control',cr),('adaptive',ar)):
 if raw[:4] != b'JNNW' or struct.unpack_from('<I',raw,4)[0] != 500000: raise SystemExit(f'{name}: invalid weighted corpus')
if cr[-hold*rec:] != ar[-hold*rec:]: raise SystemExit('control/adaptive holdout bytes differ')
p={'schema':1,'same_total_records':True,'same_holdout_bytes':True,'holdout_records':hold,
 'holdout_sha256':hashlib.sha256(cr[-hold*rec:]).hexdigest(),
 'control_training_sha256':hashlib.sha256(cr[8:-hold*rec]).hexdigest(),
 'adaptive_training_sha256':hashlib.sha256(ar[8:-hold*rec]).hexdigest(),
 'only_intended_difference':'training_row_resampling_policy'}
...
"$J" --dump-eval-features "$W/control.weighted.jnnw" "$W/control.feat" > "$W/control-features.log" 2>&1
"$J" --dump-eval-features "$W/adaptive.weighted.jnnw" "$W/adaptive.feat" > "$W/adaptive-features.log" 2>&1
for arm in control adaptive; do
  env JASS_PATTERNS_DIR="$GEOM" PYTHONPATH="$GEOM:pattern_jass/tools" python3 pattern_jass/tools/train_stream.py \
    --data "$W/${arm}.weighted.jnnw" --feat "$W/${arm}.feat" --out "$W/${arm}.pjtw" \
    --target wdl --loss logistic --color-fold --tempo-stage --warm-start "$INPUTS/g3.pjtw" \
...
c=header(sys.argv[1]); a=header(sys.argv[2])
if c['n_pat'] != a['n_pat'] or c['n_ext'] != 120 or a['n_ext'] != 120: raise SystemExit(f'layout mismatch: {c} {a}')
Path(sys.argv[3]).write_text(json.dumps({'schema':1,'protocol':'w1-same-corpus-weight-only-refit','control':c,'adaptive':a,
 'same_source':True,'same_split':True,'same_holdout':True,'same_feature_extractor':True,'same_optimizer':True,
 'same_warm_start_g3':True,'only_intended_difference':'resampling_weights'},indent=2,sort_keys=True)+'\n')
PY
gzip -n -c "$W/control.pjtw" > "$ART/w1-control-refit.pjtw.gz"
gzip -n -c "$W/adaptive.pjtw" > "$ART/w1-adaptive.pjtw.gz"
...
mv "$POOLS/raw/plateau-a.jnnw" "$POOLS/plateau-e.jnnw"
mv "$POOLS/raw/plateau-a.json" "$POOLS/plateau-e.json"
mv "$POOLS/raw/plateau-b.jnnw" "$POOLS/plateau-f.jnnw"
mv "$POOLS/raw/plateau-b.json" "$POOLS/plateau-f.json"
python3 - "$POOLS" "$ART/w1-e64-f64-manifest.json" "$POOL_SEED" "$PLATEAU_PER_STRATUM" <<'PY'
import hashlib,json,struct,sys
from pathlib import Path
root=Path(sys.argv[1]); out=Path(sys.argv[2]); seed=int(sys.argv[3]); per=int(sys.argv[4]); files={}
for pool in ('e','f'):
...
out.write_text(json.dumps({'schema':1,'protocol':'w1-independent-e64-f64','seed':seed,'per_stratum':per,
 'records_per_pool':18*per,'historical_a64_b64_reused':False,'historical_c64_d64_reused':False,'files':files,
 'oracle_used_for_selection':False},indent=2,sort_keys=True)+'\n')
PY
...
run_arm_pool(){
  local arm="$1" pattern="$2" pool="$3" dir="$REPORTS/$arm/$pool"
  ...
  for shard in $(seq 0 $((NSHARDS-1))); do
    ...
    timeout "$SHARD_TIMEOUT" python3 jobs/tools/imbalance2_scan_gate.py run \
      --engine candidate --jass "$J" --pattern "$pattern" \
...
run_arm_pool control "$W/control.pjtw" e
run_arm_pool control "$W/control.pjtw" f
run_arm_pool adaptive "$W/adaptive.pjtw" e
run_arm_pool adaptive "$W/adaptive.pjtw" f

echo "stage=generalist_guard" > "$PROG"
python3 jobs/tools/imbalance2_weight_generalist.py \
  --jass "$J" --control-pattern "$W/control.pjtw" --adaptive-pattern "$W/adaptive.pjtw" \
  ...
  --out "$ART/w1-generalist-guard.json" > "$W/generalist.log" 2>&1
...
for arm in ('control','adaptive'):
 paths=[]
 for pool in ('e','f'): paths.extend((root/arm/pool/'inputs.list').read_text().splitlines())
 sets[arm]=paths
out.write_text(json.dumps({'schema':1,'same_pools':True,'same_search_budget':True,
 'pool_labels':{'plateau-e.jnnw':'E64','plateau-f.jnnw':'F64'},'report_sets':sets},indent=2,sort_keys=True)+'\n')
PY

echo "stage=aggregate_verdict" > "$PROG"
python3 jobs/tools/imbalance2_w1_screen_report.py --manifest "$W/w1-manifest.json" \
  --generalist "$ART/w1-generalist-guard.json" --policy-report "$ART/w1-adaptive-reweight.json" \
  --out "$ART/w1-adaptive-screen-decision.json" --summary-out "$ART/JASS_CONTROL_SUMMARY.json" \
...
import json,sys
from pathlib import Path
p=json.load(open(sys.argv[1])); art=Path(sys.argv[2]); res=Path(sys.argv[3]); macro=p['paired']['macro_equal_stratum']; gen=p['generalist_gate']
def safe(v): return ('P' if v>=0 else 'M')+f'{abs(v):.4f}'.replace('.','_')
markers=[f"VERDICT__{p['decision']}",f"MACRO_DELTA_ADAPTIVE_MINUS_CONTROL__{safe(macro['adaptive_minus_control_failure_cost'])}",
 f"NONWORSE_STRATA__{macro['nonworse_strata']}_OF_18",f"GENERALIST_PASS__{str(bool(gen['pass'])).upper()}",
 'CONFIRMATION_REQUIRES_FRESH_C512_CROSSFIT__TRUE','TRAINING_CONTINUATION_AUTHORIZED__FALSE','PROMOTION_AUTHORIZED__FALSE']
for name in markers: (art/name).write_text(name+'\n')
with res.open('a') as f:
 f.write(f"decision={p['decision']}\n")
 f.write(f"macro_delta={macro['adaptive_minus_control_failure_cost']:.6f} ci95={macro['stratified_bootstrap_95']} nonworse={macro['nonworse_strata']}/18\n")
 f.write(f"generalist_score={gen['adaptive_score_rate']:.4f} ci95={gen['paired_bootstrap_95']} pass={gen['pass']}\n")
 f.write('training_continuation_authorized=false promotion_authorized=false automatic_next_job=null\n')
PY
echo "stage=completed" > "$PROG"
say "=== W1 adaptive screen complete; confirmation and promotion remain unauthorized ==="
