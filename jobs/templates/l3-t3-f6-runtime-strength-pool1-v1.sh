#!/usr/bin/env bash
# R1 Pool1: frozen T3-A/F6 vs CURRICULUM, native primary + Q00 diagnostic.
set -Eeuo pipefail
...
GEN_SEED=2026091001
SELECT_SEED=2026091002
NATIVE_BOOTSTRAP_SEED=2026091003
Q00_BOOTSTRAP_SEED=2026091004
CANDIDATES=30000
OPENINGS=3000
GAMES=6000
BOOTSTRAP=200000
  ...
  (t0=$(date +%s); while true; do
    {
      ...
      printf 'force_views_ready=%s\n' "$(find "$FORCE" -maxdepth 1 -name 'pool1-*.json' | wc -l)"
...
[[ "$JASS_JOB_ID" =~ ^cpx62-[0-9]+-l3-t3-f6-runtime-strength-pool1-v1$ ]] || die "job nomenclature drift"
...
python3 jobs/tools/fetch_result_files.py --prefix "$R0_RESULT_PREFIX" \
  ...
  --file artefacts/r0-runtime-profile.json=r0-runtime-profile.json \
  --file artefacts/runtime-contract.json=runtime-contract.json \
...
if s.get('verdict')!='R0_PRODUCTION_LEAF_CONTRACT_PASS' or s.get('passed') is not True or s.get('pool1_authorized') is not True: raise SystemExit('R0 authorization drift')
if s.get('code_sha')!=code or receipt.get('code_sha')!=code: raise SystemExit('R0 code drift')
if sha(root/'t3-a-f6-only.json')!=model or s.get('artifact_sha256')!=model: raise SystemExit('R0 model drift')
if sha(root/'curriculum.pjtw')!=curr or s.get('curriculum_sha256')!=curr: raise SystemExit('R0 curriculum drift')
PY_R0
...
grep -q '^ready' "$W/load-off.log" && grep -q '^ready' "$W/load-on.log" || die "R0 binary loader smoke failed"
EGDIR=""
...
IDENTITY_ARGS=(); FORCE_ARGS=(--exclude-fen "$IN/r0-corpus.fen"); IDENTITY_COUNT=0; FORCE_COUNT=0
...
for pass in a b; do
  "$J" --gen-opening-pool "$CANDIDATES" "$W/pool1-candidates-$pass.fen" 8 32 20 "$GEN_SEED" \
    >"$W/pool1-generate-$pass.log" 2>&1
done
...
python3 jobs/tools/t3_f6_force_pool_select.py --candidates "$W/pool1-candidates-a.fen" \
  ...
  --generator-seed "$GEN_SEED" --out "$ART/pool1-openings.fen" \
  --report "$ART/pool1-provenance.json" >"$W/pool1-select.log" 2>&1
...
run_gate(){
  ...
  timeout -k 120s 43200s "$PY" jobs/tools/run_jass_gate_bounded.py \
    ...
    --openings-file "$ART/pool1-openings.fen" "${budget[@]}" --pairs 1 --max-plies 160 \
    --nshards "$NSH" --max-parallel "$PAR" --timeout 40000 \
    --paired-bootstrap-samples "$BOOTSTRAP" --paired-bootstrap-seed "$seed" \
    --work-dir "$W/gate-pool1-$view" --out "$FORCE/pool1-$view.json" \
    >"$W/force-pool1-$view.log" 2>&1
  python3 - "$W/gate-pool1-$view" "$ART/pool1-$view-games.jsonl.gz" "$GAMES" <<'PY_GAMES'
...
run_gate native "$NATIVE_BOOTSTRAP_SEED" || die "Pool1 native gate failed"
stage force-pool1-q00-diagnostic
run_gate q00 "$Q00_BOOTSTRAP_SEED" || die "Pool1 Q00 gate failed"

stage pool1-readout-and-decision
"$PY" jobs/tools/t3_f6_strength_readout.py --pool1-native "$FORCE/pool1-native.json" \
  --pool1-q00 "$FORCE/pool1-q00.json" --model "$IN/t3-a-f6-only.json" \
  ...
  --r0-summary "$IN/r0-summary.json" \
  --code-sha "$EXPECTED_CODE_SHA" --search-params "$Q00" \
...
cp "$IN/r0-corpus.fen" "$ART/r0-corpus.fen"
: >"$ART/VERDICT__$VERDICT"
: >"$ART/GAMES_TOTAL__12000"
: >"$ART/POOL2_AUTHORIZED__${AUTHORIZED^^}"
