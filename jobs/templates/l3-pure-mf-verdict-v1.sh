#!/usr/bin/env bash
# template: L3-PURE C3-MF L2-triage screen verdict v1
# description: evaluation only from three immutable G2 artefacts (control 0817 +
#              MF_L2LO 0829 + MF_L2HI 0830); common-search + native equal-time +
#              paired P1-P4 conversion vs the L2=3e-5 control. Reuses the generic
#              l3_screen_verdict. Cell URIs come from the wrapper env.
# expected_duration: ~40-55 min on ccx33 (3 cells; anchored on the 4-cell 0812).
set -Eeuo pipefail
...
: "${MF_CONTROL_URI:?}"; : "${MF_L2LO_URI:?}"; : "${MF_L2HI_URI:?}"
: "${MF_CONTROL_JOB:?}"; : "${MF_L2LO_JOB:?}"; : "${MF_L2HI_JOB:?}"

declare -A PREFIX=( [CONTROL]="$MF_CONTROL_URI" [MF_L2LO]="$MF_L2LO_URI" [MF_L2HI]="$MF_L2HI_URI" )
declare -A SRCJOB=( [CONTROL]="$MF_CONTROL_JOB" [MF_L2LO]="$MF_L2LO_JOB" [MF_L2HI]="$MF_L2HI_JOB" )
# CONTROL is the published X1 X_HHH_CONTROL cell; the challengers are C3-MF cells.
declare -A EXPERIMENT=( [CONTROL]="C2-X1" [MF_L2LO]="C3-MF" [MF_L2HI]="C3-MF" )
declare -A VARIANT=( [CONTROL]="X_HHH_CONTROL" [MF_L2LO]="MF_L2LO" [MF_L2HI]="MF_L2HI" )
declare -A STATUS=( [CONTROL]="complete_exploration_screen_cell" [MF_L2LO]="complete_mf_screen_cell" [MF_L2HI]="complete_mf_screen_cell" )
CELLS=(CONTROL MF_L2LO MF_L2HI)
CHALLENGERS=(MF_L2LO MF_L2HI)

NOPEN="${NOPEN:-300}"; NSH_GATE="${NSH_GATE:-8}"; PAR_GATE="${PAR_GATE:-4}"
...
monitor(){ ( while true; do { TZ=Europe/Paris date '+time_fr=%Y-%m-%dT%H:%M:%S%z'; printf 'stage=%s\n' "$(cat "$STAGE" 2>/dev/null||echo ?)";
  ...
  printf 'conv_shards=%s\n' "$(find "$W" -maxdepth 1 -name '*.conv.*.json' 2>/dev/null|wc -l)"; } > "$PROG.tmp"; mv "$PROG.tmp" "$PROG"; sleep 300; done ) & MONITOR_PID="$!"; }
...
say "=== $JASS_JOB_ID — L3-PURE C3-MF L2-triage screen verdict v1 ==="
...
[ "$NOPEN" -eq 300 ] || die "requires NOPEN=300"
[ "$DEPTH" -eq 9 ] || die "common-search requires depth 9"
[ "$CONV_DEPTH" -eq 10 ] || die "conversion requires depth 10"
[ "$NATIVE_MOVETIME" = 0.1 ] || die "native-search requires 0.1 s/move"
[ "$SCREEN_DELTA" = 0.02 ] || die "conversion threshold is pre-registered at 0.02"
[ "$BOOTSTRAP_REPLICATES" -ge 10000 ] || die "paired bootstrap requires >=10000 replicates"
NPROC="$(nproc)"; [ "$NPROC" -ge 8 ] || die "need >=8 CPUs, got $NPROC"
FREE_MB="$(df -Pm "$JASS_RESULT_DIR"|awk 'NR==2{print $4}')"; [ "${FREE_MB:-0}" -ge 5000 ] || die "<5GiB free"
say "preflight: nproc=$NPROC free_mb=$FREE_MB common_gates=2x$((NOPEN*2)) native_gates=2x$((NOPEN*2)) conv=3cells-P1-P4 timeout=${SHARD_TIMEOUT}s"
monitor
python3 -m py_compile jobs/tools/fetch_result_files.py jobs/tools/fetch_t1bis_inputs.py \
  ...
  jobs/tools/aggregate_conv_shards.py jobs/tools/l3_screen_verdict.py \
...
python3 jobs/tests/test_l3_screen_verdict.py > "$W/t-verdict.log" 2>&1 || die "verdict tests red"
...
for c in "${CELLS[@]}"; do
  ...
  python3 - "$d/manifest.json" "$ART/verified-$c.json" "$c" "${SRCJOB[$c]}" "$d/g2.pjtw.gz" "${EXPERIMENT[$c]}" "${VARIANT[$c]}" "${STATUS[$c]}" <<'PY'
import hashlib,json,sys
man=json.loads(open(sys.argv[1]).read()); ver=json.loads(open(sys.argv[2]).read())
cell,job,g2,exp,variant,status=sys.argv[3],sys.argv[4],sys.argv[5],sys.argv[6],sys.argv[7],sys.argv[8]
if man.get('experiment')!=exp or man.get('variant')!=variant:
    raise SystemExit(f'{cell}: manifest experiment/variant mismatch {man.get("experiment")}/{man.get("variant")}')
if man.get('generations')!=2 or man.get('scientific_status')!=status:
    raise SystemExit(f'{cell}: manifest not a complete 2-gen cell ({man.get("scientific_status")})')
...
for c in "${CHALLENGERS[@]}"; do
  [ "${SEARCH[$c]}" = "$COMMON_SEARCH" ] || die "$c fingerprint diverges from CONTROL (screen shares one search)"
done
say "provenance OK: 3 cells (control + L2 lo/hi) + shared fingerprint + gen2 defender + gauge"
...
python3 - "$ART/mf-verdict-spec.json" "$BOOTSTRAP_REPLICATES" "$COMMON_SEARCH" <<'PY'
import json,sys
from pathlib import Path
out=Path(sys.argv[1]); replicates=int(sys.argv[2]); common=sys.argv[3]
cells=('CONTROL','MF_L2LO','MF_L2HI'); strata=('p1_net','p2_moyen','p3_mince','p4_egal')
meta={'CONTROL':{'l2':'3e-5'},'MF_L2LO':{'l2':'1e-5'},'MF_L2HI':{'l2':'1e-4'}}
cell_specs={}
for cell in cells:
    item={'search_params':common,'meta':meta[cell],'conversion':{s:f'conversion/{cell}-{s}.json' for s in strata}}
    if cell!='CONTROL':
...
payload={'schema':2,'experiment':'L3-PURE-C3-MF','baseline':'CONTROL','common_search_params':common,
    'conversion_delta_threshold':0.02,'bootstrap':{'method':'paired_position','replicates':replicates,'seed':271828},
    'cells':cell_specs,'automatic_next_job':None}
out.write_text(json.dumps(payload,indent=2,sort_keys=True)+'\n',encoding='utf-8')
PY
python3 jobs/tools/l3_screen_verdict.py --spec "$ART/mf-verdict-spec.json" \
  --out "$ART/mf-verdict.json" > "$W/verdict.log" 2>&1 \
  || die "contract-complete C3-MF verdict invalid"
cat "$ART/mf-verdict.json" | tee -a "$RES"
set_stage complete
say "=== C3-MF verdict complete; no confirmation job was launched ==="
