...
[ "$(sha256sum "$W/M2.jnnw"|awk '{print $1}')" = "$M2_CORPUS_SHA" ] || die "M2 corpus hash drift"
[ "$(jnnw_count "$W/M2.jnnw")" -eq 2000000 ] || die "M2 corpus count drift"
...
python3 - "$IN/m2-training.json" "$IN/champion-benchmark.json" \
  ...
  "$ART/verified-champion.json" "$ART/verified-m1-matrix.json" <<'PY'
import json,sys
...
if training.get("model_sha256")!="75ace3c0ad2ffa2b71a9b9073c3c1d1545164e3a5a048e411e91adba23ec3b45":
...
run_gate(){
  ...
  timeout 21600 python3 jobs/tools/run_jass_gate_bounded.py \
    --jass-a "$J8" --jass-b "$jb" \
    --pattern-a "$W/M2.pjtw" --pattern-b "$pattern" \
    --search-params-a "$Q00" --search-params-b "$Q00" \
    --openings-file "$W/open-m2.fen" "${args[@]}" --pairs 1 \
    --max-plies 160 --nshards "$NSH_GATE" --max-parallel "$PAR_GATE" \
    --timeout 10800 --game-timeout 180 \
    --work-dir "$W/gate-$view-M2-$opponent" \
    --out "$ART/force/force-$view-M2-vs-$opponent.json" \
    > "$W/force-$view-M2-$opponent.log" 2>&1
}
stage force-q00
...
run_conv(){
  ...
  for shard in $(seq 0 $((NSH_CONV-1))); do
    out="$W/M2-$stratum-$shard.json"; inputs+=("$out")
    timeout 14400 python3 jobs/tools/conv_fixed_wdl.py \
      --jass "$J8" --defender-jass "$J32FIXED" \
      --pattern "$W/M2.pjtw" --defender-pattern "$W/GEN2.pjtw" \
      ...
      > "$W/M2-$stratum-$shard.log" 2>&1 & pids+=("$!")
  done
  wait_all "M2/$stratum conversion" "${pids[@]}"
  python3 jobs/tools/aggregate_conv_shards.py --inputs "${inputs[@]}" \
    ...
    --out "$ART/conversion/M2-$stratum.json" > "$W/M2-$stratum-aggregate.log" 2>&1
}
...
python3 jobs/tools/l3_m2_evaluation.py \
  ...
  > "$W/aggregate.log" 2>&1
VERDICT="$(python3 - "$ART/m2-evaluation.json" <<'PY'
import json,sys
