...
if [ "$EVAL_VARIANT" = D10_CAUSAL ]; then
  ...
  wait_all "Q00 force wave" "$p_f2m" "$p_gen2" "$p_m2"
else
...
if [ "$EVAL_VARIANT" = D10_CAUSAL ]; then
  ...
  wait_all "native force wave" "$p_f2m" "$p_gen2" "$p_m2"
else
