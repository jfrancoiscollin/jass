#!/usr/bin/env bash
# id: ccx33-l3-imbalance2-role-v2-p1
# description: ccx33 P1 G1-G4 d8; exact TB; per-position conversion/resilience matrix
# expected_duration: pending ccx33 probe calibration; do not queue without explicit go
set -Eeuo pipefail
: "${EXPECTED_CODE_SHA:?set merged jass SHA in jass-control}"
export FULL_RUN_APPROVED=1 SCIENTIFIC_GO=1 PHASE=P1
