#!/usr/bin/env bash
# id: ccx33-l3-imbalance2-role-v2-probe
# description: ccx33 memory/rate probe; one reduced non-promotable P1 generation
# expected_duration: calibration only; inspect RAM, throughput and EGDB before full P1
set -Eeuo pipefail
: "${EXPECTED_CODE_SHA:?set merged jass SHA in jass-control}"
export FULL_RUN_APPROVED=1 SCIENTIFIC_GO=1 PHASE=P1 PROBE=1
export FRESH=54000 NSHARDS=18 PAR_GEN=8 MAXPLIES=260 LABEL_DEPTH=4
...
export TRAIN_SEEDS_PER_SIDE=2048 BENCH_PER_STRATUM=24 PLATEAU_PER_STRATUM=8 FRONTIER_FRAC=0
