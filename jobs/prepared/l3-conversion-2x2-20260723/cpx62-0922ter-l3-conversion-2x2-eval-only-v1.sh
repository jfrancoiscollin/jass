#!/usr/bin/env bash
# id: cpx62-0922ter-l3-conversion-2x2-eval-only-v1
# description: evaluation-only recovery of 0922bis with one pinned ply-cap adjudication
# expected_duration: 12-18 min on measured cpx62 nproc=16; hard cap 30 min
set -Eeuo pipefail
: "${EXPECTED_CODE_SHA:?set reviewed develop SHA containing 0922ter}"
: "${EXPECTED_SCAN_SHA256:?set pinned Scan executable SHA256}"
: "${EXPECTED_SCAN_RUNTIME_SHA256:?set pinned Scan runtime fingerprint}"
export EXPECTED_JOB_ID="cpx62-0922ter-l3-conversion-2x2-eval-only-v1"
