...
from jobs.tools import cls_hier_l2_2052_stage_log_diagnostic as diag

ROOT = Path(__file__).resolve().parents[2]
PROFILE = ROOT / "jobs" / "launch_profiles" / "cls-hier-l2-2052-stage-log-diagnostic-v1.json"


class CLSHierL22052StageLogDiagnosticV1Tests(unittest.TestCase):
    def test_failed_source_identity_and_terminal_are_frozen(self):
        ...
        self.assertEqual(diag.TERMINAL, "CLS_HIER_L2_2052_STAGE_LOG_DIAGNOSTIC_COMPLETE_V1")
        self.assertEqual(diag.PHASE, "execute-cls-hier-l2-2052-stage-log-diagnostic")
    ...
    def test_stage_stderr_surfaces_exact_mechanical_error(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            path.write_text(
                ...
                "RuntimeError: deterministic 2052 mechanical witness\n",
                encoding="utf-8",
            )
    ...
    def test_profile_is_zero_effect_and_cannot_read_scientific_payloads(self):
        ...
        self.assertEqual(
            profile["command"],
            ["/usr/bin/python3", "jobs/tools/cls_hier_l2_2052_stage_log_diagnostic.py"],
        )
