...
class Context4SourceContractTests(unittest.TestCase):
    ...
    def test_certified_summary_copy_passes_exact_scientific_contract(self):
        validate_1428_force_summary(self._scientific_readout())

    def test_fictional_nested_runner_wrapper_is_rejected(self):
        runner = {
            "verdict": "JASS_CONTEXT3_ALIGNED_VS_SHUFFLED_NOT_ESTABLISHED",
            "protocol": {
                "fits": {"count": 0},
        ...
        with self.assertRaisesRegex(ValueError, "scientific readout schema drift"):
            validate_1428_force_summary(runner)

    def test_summary_copy_fails_closed_on_execution_scope_drift(self):
        protocol_cases = (
        ...
        for key, value, message in top_level_cases:
            payload = self._scientific_readout()
            payload[key] = value
            with self.subTest(key=key):
                with self.assertRaisesRegex(ValueError, message):
