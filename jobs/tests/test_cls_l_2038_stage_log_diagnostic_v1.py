...
from jobs.tools import cls_l_2038_stage_log_diagnostic as diag

ROOT = Path(__file__).resolve().parents[2]


class CLSL2038StageLogDiagnosticTests(unittest.TestCase):
    def test_failed_source_identity_and_terminal_are_frozen(self):
        ...
        self.assertEqual(diag.TERMINAL, "CLS_L_2038_STAGE_LOG_DIAGNOSTIC_COMPLETE_V1")
        self.assertEqual(diag.PHASE, "execute-cls-l-2038-stage-log-diagnostic")
    ...
    def test_profile_is_zero_effect_and_cannot_read_fit_payloads(self):
        ...
        profile = json.loads(
            (ROOT / "jobs/launch_profiles/cls-l-2038-stage-log-diagnostic-v1.json").read_text()
        )
        self.assertEqual(profile["required_phases"], [diag.PHASE])
        self.assertEqual(
            profile["command"],
            ["/usr/bin/python3", "jobs/tools/cls_l_2038_stage_log_diagnostic.py"],
        )
