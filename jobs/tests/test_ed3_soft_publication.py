...
from jobs.tools.ed3_soft_value_fit import PHASES,OUTPUTS
...
class FittedModelPublicationTests(unittest.TestCase):
    def test_actual_fit_stage_publisher_and_authenticated_model_readback(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            job='cpx62-ed3-soft-fixture';attempt='20260908T000000Z-'+head[:8]
            ...
            with patch.dict(os.environ,{'FIXTURE_STORE':str(store)}):
                ...
                self.assertEqual(g.read(dest/'candidate-seal.json')['model_sha256'],g.sha(dest/'SOFT.pjtw'))
                with (store/job/attempt/'artefacts/SOFT.pjtw').open('ab') as f:f.write(b'corrupt')
                with self.assertRaises(Exception):fetch_files(rclone=str(fake),prefix=f'r2:jass-data/runs/{job}/{attempt}',
                    out_dir=root/'bad',selections=[('artefacts/SOFT.pjtw','SOFT.pjtw')])
