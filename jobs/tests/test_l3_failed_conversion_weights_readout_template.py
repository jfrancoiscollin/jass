...
TEMPLATE = (
    ...
    / "jobs"
    / "templates"
    / "l3-pure-failed-conversion-weights-readout-v1.sh"
)


class FailedConversionWeightsReadoutTemplateTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.text = TEMPLATE.read_text(encoding="utf-8")

    def test_fit_arms_and_weight_paths_are_authenticated(self) -> None:
        for literal in (
    ...
    def test_force_is_paired_two_view_and_preregistered_power(self) -> None:
        for literal in (
            ...
            '--depth "$FORCE_DEPTH"',
            '--movetime "$MOVETIME"',
            "FAILED_X2-vs-UNWEIGHTED",
        ):
            self.assertIn(literal, self.text)

    def test_openings_are_disjoint_from_every_prior_signal_readout(self) -> None:
        for literal in (
