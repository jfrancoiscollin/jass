#!/usr/bin/env bash
# id: ccx33-0631-mmto-scan-mixed-v2
# description: MMTO v6-bis COMPLÉMENTAIRE de 0630 (remplir la nuit ccx33). Même recette mixte (équilibrées fort-vs-fort +
# déséquilibrées mt0.3/0.03, prof Scan) mais OUVERTURES DISJOINTES (seeds différents) → ~100-120k parents INDÉPENDANTS de 0630.
# But : COUPLER avec 0630 si le volume paie → concat des parents (0630+0631 = ~200-240k) pour un gros fit MMTO haut-volume
# (job combine séparé, gaté sur les A/B). Committe parents+moves (couplables) + fait son propre fit standalone (candidat +
# delta indépendants, réplication du +47 à volume comparable). AUCUN NNUE.
set -uo pipefail
cd /root/jass
NCPU=$(nproc); export TMPDIR=/root/jass/.compile-tmp; mkdir -p "$TMPDIR"
ART="/root/jass/jobs/results/ccx33-0631-mmto-scan-mixed-v2/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/ccx33-0631-mmto-scan-mixed-v2/artefacts"
RES="$ART/RESULTS.txt"; : > "$RES"; say(){ echo "$@" | tee -a "$RES"; }
W=/root/cw-mmtomix2; rm -rf "$W"; mkdir -p "$W"; GEOM=/root/jass-geom32-mmtomix2
...
for s in $(seq 0 $((NCPU-1))); do
  python3 tools/scan_selfplay_gen.py --scan "$SCAN_BIN" --jass "$J" --seeds "$W/seeds.jnnw" \
    ...
    --skip-book "$SKIP" --keep-draw-frac 1.0 --seed 31631 --nshards "$NCPU" --shard "$s" >"$W/.a-$s.log" 2>&1 &
done; wait
...
for s in $(seq 0 $((NCPU-1))); do
  python3 tools/scan_selfplay_gen.py --scan "$SCAN_BIN" --jass "$J" --seeds "$W/seeds.jnnw" \
    ...
    --skip-book "$SKIP" --keep-draw-frac 0.2 --seed 41631 --nshards "$NCPU" --shard "$s" >"$W/.b-$s.log" 2>&1 &
done; wait
...
gzip -c "$W/parents.jnnw" > "$ART/scan-mixed2-parents.jnnw.gz"; gzip -c "$W/moves.bin" > "$ART/scan-mixed2-moves.bin.gz"
commit_to_main "$ART/scan-mixed2-parents.jnnw.gz" "$ARTREL/scan-mixed2-parents.jnnw.gz" "0631 parents Scan mixte v2 ($NPAR, équi+déséqui)" >/dev/null 2>&1 || true
commit_to_main "$ART/scan-mixed2-moves.bin.gz" "$ARTREL/scan-mixed2-moves.bin.gz" "0631 moves Scan mixte v2" >/dev/null 2>&1 || true
...
for A in 0.05 0.1; do
  ...
  env JASS_PATTERNS_DIR="$GEOM" PYTHONPATH="$GEOM:pattern_jass/tools" python3 pattern_jass/tools/rank_finetune.py \
      --champion "$W/gen1.pjtw" --pairs "$W/pairs.jnnw" --feat "$W/feat" --out "$W/mixmmto2_$A.pjtw" \
  ...
  if [ $? = 0 ]; then grep -E 'pairwise-acc|delta' "$W/ft_$A.log" | sed "s/^/  [$A] /" | tee -a "$RES"; OKA="$OKA $A"
    gzip -c "$W/mixmmto2_$A.pjtw" > "$ART/mixmmto2_$A.pjtw.gz"
    commit_to_main "$ART/mixmmto2_$A.pjtw.gz" "$ARTREL/mixmmto2_$A.pjtw.gz" "0631 candidat MMTO mixte v2 anchor=$A ($NPAR parents)" \
      && say "  [$A] committé" || say "  [$A] ⚠ commit echoue"
  else say "  [$A] ABORT (gate) : $(tail -2 "$W/ft_$A.log"|tr '\n' ' ')"; fi
done
git checkout -- src/main.cpp pattern_jass/tools/rank_finetune.py tools/scan_selfplay_gen.py 2>/dev/null || true
say ""; say "  => next : COUPLER parents 0630+0631 (~200-240k) pour un gros fit si volume paie ; + A/B standalone mixmmto2."
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0631 MMTO Scan mixte v2 (équi+déséqui) haut-volume : candidats prets" \
