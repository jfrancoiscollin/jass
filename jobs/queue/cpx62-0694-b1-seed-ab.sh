...
ART="/root/jass/jobs/results/cpx62-0694-b1-seed-ab/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/cpx62-0694-b1-seed-ab/artefacts"
...
restore_src(){ git checkout -- src/main.cpp tools/scan_selfplay_gen.py tools/calibrate_vs_scan.py tools/jass_vs_jass_arch.py pattern_jass/tools/train_stream.py pattern_jass/tools/wdl_finetune.py 2>/dev/null||true; }
cmake -S . -B "$W/build" -DCMAKE_BUILD_TYPE=Release $FLAGS >"$W/cmake.log" 2>&1
cmake --build "$W/build" -j"$NCPU" --target jass >"$W/build.log" 2>&1 || { say "BUILD FAIL"; tail -15 "$W/build.log"|sed 's/^/  /'|tee -a "$RES"; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0694 BUILD FAIL"; restore_src; exit 6; }
J="$W/build/jass"
...
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0694 gen bras A fini" >/dev/null 2>&1 || true
NW_B=$(gen_arm B "$W/poolB.jnnw" "$W/wdlB.jnnw" | tail -1)
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0694 gen bras B fini" >/dev/null 2>&1 || true
...
fit_arm(){ local arm="$1" wdl="$2" out="$3"
  ...
  if [ $? = 0 ] && [ -s "$out" ]; then
    ...
    gzip -c "$out" > "$ART/cand_$arm.pjtw.gz"; commit_to_main "$ART/cand_$arm.pjtw.gz" "$ARTREL/cand_$arm.pjtw.gz" "0694 candidat $arm" >/dev/null 2>&1 || true
  else say "  [$arm] FIT FAIL : $(tail -4 "$W/fit_$arm.log"|tr '\n' ' ')"; return 1; fi
}
fit_arm A "$W/wdlA.jnnw" "$W/candA.pjtw" || { say "ABORT fit A"; restore_src; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0694 ABORT fit A"; exit 8; }
fit_arm B "$W/wdlB.jnnw" "$W/candB.pjtw" || { say "ABORT fit B"; restore_src; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0694 ABORT fit B"; exit 8; }
restore_src
...
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0694 A/B candB-vs-candA mt0.2" >/dev/null 2>&1 || true
...
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0694 A/B compose ensemencement" >/dev/null 2>&1 || true
...
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0694 B1 étape-0 ensemencement : gen+fit+A/B (+thermo candB) — verdict compose" \
  && say "  ✓ RESULTS committé" || say "  ⚠ commit RESULTS échoué"
say "=== 0694 B1 étape-0 FINI ==="
