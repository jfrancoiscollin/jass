...
# description: ÉTAGE 1 de l'orchestration distribuée — committe dans git la GEN DATA + les évals de 0374 (boucle d12,
# ccx33) pour que cpx62 puisse les pooler (les 2 boxes ne partagent QUE git). Copie cumulative.jnnw (tronqué au cap
# artefact) + tous les gen*.pjtw de 0374 dans artefacts/ (committé ≤95Mo). DÉPLOYER après la fin de 0374. La data WDL
# est arch-indépendante → actif durable réutilisable. NB : dépend de la persistance d'artefacts.src de 0374 (lancer vite).
...
[ -d "$SRC" ] || { echo "ABORT: artefacts.src de 0374 introuvable (container recyclé ? relancer 0367)"; exit 4; }
[ -f "$SRC/cumulative.jnnw" ] || { echo "ABORT: cumulative.jnnw de 0374 absent"; ls "$SRC" | head; exit 4; }

echo "=== évals de 0374 (gen*.pjtw) ==="
n=0; for p in "$SRC"/gen*.pjtw; do [ -f "$p" ] || continue; cp "$p" "$ART/0374-$(basename "$p")"; n=$((n+1)); done
echo "  $n évals copiées"; ls -la "$ART"/0374-gen*.pjtw 2>/dev/null | tail -3
...
python3 - "$SRC/cumulative.jnnw" "$ART/0374-cumulative.jnnw" 1500000 <<'PY'
import struct,sys
