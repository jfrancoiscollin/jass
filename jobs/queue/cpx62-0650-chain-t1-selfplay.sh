#!/usr/bin/env bash
# id: cpx62-0650-chain-t1-selfplay
# description: CHAÎNE ITÉRATIVE LONGUE (mémo JFC) — TOUR 1, SELF-PLAY. Pilote = champion(0)=gen2-mmto qui DRIVE la
# distribution (mobile, cœur du mémo : PAS Scan-as-player=statique). jass self-asym par DEPTH (fort d8 vs faible d3,
# randomisé/partie) => WDL (outcomes) + prefs MMTO côté-FORT (recherche d8 > feuille d5 = teacher autonome). asym pur
...
ART="/root/jass/jobs/results/cpx62-0650-chain-t1-selfplay/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/cpx62-0650-chain-t1-selfplay/artefacts"
RES="$ART/RESULTS.txt"; : > "$RES"; say(){ echo "$@" | tee -a "$RES"; }
W=/root/cw-chain-t1; rm -rf "$W"; mkdir -p "$W"
...
PERG=250; MAXPLIES=160; MINPIECES=32; SKIP=8; DRAWFRAC=0.2
STRONG_D=8; WEAK_D=3; JITTER=1; SEED=50650
...
PY
    [ -s "$W/.prog" ] && { cat "$W/.prog"|tee -a "$RES"; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0650 progress ($(cat "$W/.prog"|grep -oE '[0-9]+/[0-9]+'|head -1))" >/dev/null 2>&1||true; }
    [ "$alive" = 0 ] && break
  done ) & MON=$!
...
gzip -c "$W/wdl.jnnw" > "$ART/chain-t1-wdl.jnnw.gz"
gzip -c "$W/parents.jnnw" > "$ART/chain-t1-prefs-parents.jnnw.gz"
gzip -c "$W/moves.bin" > "$ART/chain-t1-prefs-moves.bin.gz"
commit_to_main "$ART/chain-t1-wdl.jnnw.gz" "$ARTREL/chain-t1-wdl.jnnw.gz" "0650 tour1 WDL ($N_WDL)" >/dev/null 2>&1||true
commit_to_main "$ART/chain-t1-prefs-parents.jnnw.gz" "$ARTREL/chain-t1-prefs-parents.jnnw.gz" "0650 tour1 prefs ($N_PAR)" >/dev/null 2>&1||true
commit_to_main "$ART/chain-t1-prefs-moves.bin.gz" "$ARTREL/chain-t1-prefs-moves.bin.gz" "0650 tour1 moves" >/dev/null 2>&1||true
...
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0650 tour1 self-play FINI : WDL=$N_WDL prefs=$N_PAR (ancrage timing jass-self-play)" && say "  RESULTS committé ✓" || say "  ⚠ commit"
