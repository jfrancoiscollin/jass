#!/usr/bin/env bash
# id: cpx62-0665-scratch-tour0
# description: DÉCOLLAGE FROM-SCRATCH — TOUR 0 (re-size de 0664 tué : sur-dimensionné d14+qs, dark 89 min). Éval de ZÉRO, aucun
# prof, aucune distillation, aucun Scan. eval(0)=zero.pjtw (corps tout-à-0, matériel-aveugle) → self-play jass : play d10
# (le signal matériel vient de l'ADJUD-MATERIAL, pas de la profondeur → d10 suffit pour la 1re marche, bien + rapide), qs
# PLEINE (co-adaptation), eps 30% décroissant (decay 30) + drop-post-eps, ADJUD-MATERIAL 2/10 (⭐ issue corrélée au matériel
# malgré l'éval aveugle), quiet-only, random-open 8 + seed-file min-pieces 32, SYMÉTRIQUE. Volume léger (45k×NCPU≈720k). Fit
# WDL 100% STREAMÉ (train_stream --chunk). ⭐ MONITOR PROGRESS committé toutes les ~10 min (PROGRESS.txt) — plus jamais dark.
# Gate PUR : eval(1) vs eval(0)=zero (jass_vs_jass_arch, généraliste, prof fixe d9, qs pleine des 2 côtés) — ZÉRO Scan.
# GATE : eval(1) ≫ zero hors-IC => matériel + 1re structure appris PAR NOUS => tour 1. AUCUN NNUE.
set -uo pipefail
cd /root/jass
NCPU=$(nproc); export TMPDIR=/root/jass/.compile-tmp; mkdir -p "$TMPDIR"
ART="/root/jass/jobs/results/cpx62-0665-scratch-tour0/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/cpx62-0665-scratch-tour0/artefacts"
...
PERG=45000    # positions/shard (×NCPU ≈ 720k, léger)
...
# ⭐ moniteur de progression : committe PROGRESS.txt toutes les ~10 min (élapsé + octets shards + dernière ligne sp-0.log)
START=$(date +%s)
monitor_loop(){ local phase="$1"
  while [ ! -f "$W/.stopmon" ]; do
    ...
    printf '[+%dmin] %s : ~%d octets shards (~%d pos brutes), sp-0: %s\n' "$el" "$phase" "$by" "$approx" "$last" >> "$PROG"
    commit_to_main "$PROG" "$ARTREL/PROGRESS.txt" "0665 progress +${el}min ($phase, ~${approx} pos)" >/dev/null 2>&1 || true
  done
}

say "=== FROM-SCRATCH tour 0 (eval=0, zéro prof/Scan) re-size d$PLAYD léger + monitor — HEAD $(git log --oneline -1|cat) ==="
...
cmake --build "$W/build" -j"$NCPU" --target jass >"$W/build.log" 2>&1 || { say "BUILD FAIL"; tail -12 "$W/build.log"|sed 's/^/  /'|tee -a "$RES"; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0665 BUILD FAIL"; restore_src; exit 6; }
J="$W/build/jass"
...
# ---- self-play champion(0)=zero, paramétrage vérifié + MONITOR ----
...
[ "$N_WDL" -gt 60000 ] 2>/dev/null || { say "ABORT WDL faible ($N_WDL) — voir sp-0.log"; tail -8 "$W/sp-0.log"|sed 's/^/    /'|tee -a "$RES"; restore_src; exit 8; }
...
PY
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0665 self-play fini (N=$N_WDL)" >/dev/null 2>&1 || true
...
commit_to_main "$ART/eval1-scratch.pjtw.gz" "$ARTREL/eval1-scratch.pjtw.gz" "0665 eval(1) from-scratch tour0" >/dev/null 2>&1 || true
...
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0665 FIN tour0 from-scratch : eval(1) décolle-t-il de zero (zéro prof, zéro Scan)" \
