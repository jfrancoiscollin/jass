#!/usr/bin/env bash
# id: cpx62-0655-chain-t1c-selfplay
# description: CHAÎNE ITÉRATIVE LONGUE (mémo JFC) — TOUR 1, SELF-PLAY. Pilote = champion(0)=gen2-mmto qui DRIVE la
# distribution (mobile, cœur du mémo : PAS Scan-as-player=statique). jass self-asym par DEPTH (fort d14 vs faible d8 = teacher FORT + positions DISPUTÉES,
...
ART="/root/jass/jobs/results/cpx62-0655-chain-t1c-selfplay/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/cpx62-0655-chain-t1c-selfplay/artefacts"
RES="$ART/RESULTS.txt"; : > "$RES"; say(){ echo "$@" | tee -a "$RES"; }
W=/root/cw-chain-t1c; rm -rf "$W"; mkdir -p "$W"
...
STRONG_D=14; WEAK_D=8; JITTER=1; SEED=50655
...
PY
    [ -s "$W/.prog" ] && { cat "$W/.prog"|tee -a "$RES"; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0655 progress ($(cat "$W/.prog"|grep -oE '[0-9]+/[0-9]+'|head -1))" >/dev/null 2>&1||true; }
    [ "$alive" = 0 ] && break
  done ) & MON=$!
...
gzip -c "$W/wdl.jnnw" > "$ART/chain-t1c-wdl.jnnw.gz"
gzip -c "$W/parents.jnnw" > "$ART/chain-t1c-prefs-parents.jnnw.gz"
gzip -c "$W/moves.bin" > "$ART/chain-t1c-prefs-moves.bin.gz"
commit_to_main "$ART/chain-t1c-wdl.jnnw.gz" "$ARTREL/chain-t1c-wdl.jnnw.gz" "0655 tour1c WDL ($N_WDL)" >/dev/null 2>&1||true
commit_to_main "$ART/chain-t1c-prefs-parents.jnnw.gz" "$ARTREL/chain-t1c-prefs-parents.jnnw.gz" "0655 tour1c prefs ($N_PAR)" >/dev/null 2>&1||true
commit_to_main "$ART/chain-t1c-prefs-moves.bin.gz" "$ARTREL/chain-t1c-prefs-moves.bin.gz" "0655 tour1c moves" >/dev/null 2>&1||true
...
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0655 tour1c self-play FINI : WDL=$N_WDL prefs=$N_PAR (ancrage timing jass-self-play)" && say "  RESULTS committé ✓" || say "  ⚠ commit"
