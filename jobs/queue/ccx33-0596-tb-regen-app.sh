...
# eps5, open8) + --tb-relabel (labels finale EXACTS <=6 pieces via EGDB). Fit prior gen1 + combos. GARDE-FOU :
# fail-fast si tb_relabel=0 sur pd8 (pas de pre-probe combos : 0595 faux-negatif). JUGE moteur COIN par
...
ART="/root/jass/jobs/results/ccx33-0596-tb-regen-app/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/ccx33-0596-tb-regen-app/artefacts"
RES="$ART/RESULTS.txt"; : > "$RES"; say(){ echo "$@" | tee -a "$RES"; }
W=/root/cw-tbregenapp6; rm -rf "$W"; mkdir -p "$W"; GEOM32=/root/jass-geom32-tbregenapp6
...
# EGDB deja prouve fonctionnel sur /app par 0594 (42673 resolved). Pas de pre-probe sur combos (pas de finales <=6p =>
# faux negatif, cf 0595). Garde-fou = fail-fast pd8 : la self-play atteint des finales <=6p => tb_relabel>0 si egdb OK.
