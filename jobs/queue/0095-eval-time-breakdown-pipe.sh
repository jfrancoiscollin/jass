#!/usr/bin/env bash
# id: 0095-eval-time-breakdown-pipe
# description: Retry breakdown measurement with -pipe + TMPDIR explicit.
#
# 0094 a échoué non pas sur du code mais sur "Assembler messages: can't
# open /tmp/cc*.s" — gcc a écrit le .s temporaire dans /tmp puis
# l'assembler n'a pas pu le relire. Probablement systemd PrivateTmp
# ou pression disque sur /tmp pendant le build parallèle.
#
# Fix : -pipe (gcc pipe cc1 → as en mémoire, plus de fichier .s temp)
# + TMPDIR=/root/jass/tmp/ (sortir de /tmp si toujours nécessaire).
#
# expected_duration: ~5-8 min wall
set -uo pipefail
cd /root/jass

OUT_BASE="/root/jass/jobs/results/0095-eval-time-breakdown-pipe"
...
g++ --version | head -1
df -h /tmp /root | head -3

# --- workaround : private tmpdir + gcc -pipe ---
...
echo
echo "=== rebuild with -DJASS_TIME_BREAKDOWN=ON + -pipe ==="
rm -rf build-bd
cmake -S . -B build-bd \
...
if ! cmake --build build-bd -j"$(nproc)" --target jass \
    ...
    echo "BUILD FAILED with -j$(nproc). Last 40 lines :"
    tail -40 "$ART/cmake-build.log"
    echo
    echo "=== retry single-threaded ==="
    if ! cmake --build build-bd -j1 --target jass \
            > "$ART/cmake-build-j1.log" 2>&1; then
        echo "ALSO FAILED -j1. Last 40 lines :"
        tail -40 "$ART/cmake-build-j1.log"
        exit 5
    fi
    echo "build OK with -j1"
fi
...
echo
echo "=========================================================="
echo "       0095 EVAL / MOVEGEN / APPLY / OTHER BREAKDOWN"
echo "=========================================================="
python3 - <<EOF
import re
from pathlib import Path
