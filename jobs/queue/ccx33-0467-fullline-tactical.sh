#!/usr/bin/env bash
# id: ccx33-0467-fullline-tactical
# description: SUPERVISION TACTIQUE v4 = FULL-LINE (leve la DERNIERE reserve de validite avant tout verdict plafond). 0464/0466
# ont sur-pondere la seule position-RACINE pre-combinaison (etiquetee gagnante) => 0440 PLAT (0.304 a 5.4%, 0.308 a 35%). MAIS
# convertir un sacrifice exige que l'eval voie AU-DELA du sac : juger gagnants les noeuds INTERMEDIAIRES ou le materiel est
# temporairement EN BAS. Ces noeuds n'etaient PAS sur-ponderes. Ici on mine les MEMES combinaisons (vraies parties, sac->regain
# net) mais on emet TOUTE la ligne forcante [racine .. resolution] — chaque position etiquetee par le RESULTAT REEL, POV du
# trait (noeud trait-gagnant => +1 meme materiel-en-bas ; trait-perdant => -1). Meme base (pool+egdb) et MEME POIDS ~35% que
# 0466 => A/B propre "racine seule (0466=0.308) vs ligne complete". Si full-line >> 0.31 => on supervisait les mauvaises
# positions, le levier vit. Si ~0.31 => 5e echec, mecanisme epuise => le plafond FEATURE lineaire est solide. AUCUN moteur dans
# le label (ni auto-supervision, ni distillation). AUCUN NNUE.
set -uo pipefail
cd /root/jass
NCPU=$(nproc); export TMPDIR=/root/jass/.compile-tmp; mkdir -p "$TMPDIR"
ART="/root/jass/jobs/results/ccx33-0467-fullline-tactical/artefacts"; mkdir -p "$ART"
RES="$ART/RESULTS.txt"; : > "$RES"; say(){ echo "$@" | tee -a "$RES"; }
W=/root/cw-fullline; rm -rf "$W"; mkdir -p "$W"
...
GEOM32=/root/jass-geom32-fullline
POOL_TRIM=5000000; NEGDB=4000000; TARGET_FRAC=35; MID_LO=12; MID_HI=44
