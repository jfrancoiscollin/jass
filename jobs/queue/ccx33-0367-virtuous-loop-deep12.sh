#!/usr/bin/env bash
# id: ccx33-0367-virtuous-loop-deep12
# description: BRACKET QUALITÉ (parallèle de 0366 qui est à d10) — même boucle virtuelle mais self-play à play_depth=12.
...
ART="/root/jass/jobs/results/ccx33-0367-virtuous-loop-deep12/artefacts.src"; mkdir -p "$ART"
NCPU=$(nproc); export TMPDIR=/root/jass/.compile-tmp; mkdir -p "$TMPDIR"
PLAY_DEPTH=12; EVAL_DEPTH=4; SCALE=1000; NGEN=6; TARGET_GEN_MIN=18
...
NPER=$(( RATE*TARGET_GEN_MIN )); [ "$NPER" -lt 20000 ] && NPER=20000; [ "$NPER" -gt 160000 ] && NPER=160000
echo "  débit ≈ ${RATE} pos/min @ d${PLAY_DEPTH} → NPER=${NPER}/gen"
: > "$CUM"   # reset (la sonde a écrit dedans)
...
echo; echo "=========================================================="
echo "   cpx62-0366 — BOUCLE VIRTUELLE PROFONDE (d${PLAY_DEPTH}), jugée SELF (aucun Scan)"
