#!/usr/bin/env bash
# id: cpx62-0241-kingloop-scaled
# description: LE PUSH — pousser la classe LINÉAIRE à son max. Loop self-play full-fold,
# KING-AWARE (binaire -DJASS_KING_PATTERNS=ON + train --king-patterns : les rois comptent
# comme pièce, base-3 Scan-style, valider par 0240 = +37 Elo) ET SCALÉ (600k/gen cumulé vs
# 300k de 0227 → couverture ~2× plus dense de la table 17M→1M sous full-fold) pour attaquer
# la famine de données. 8 gens depth4, Elo RÉEL vs hc par gen. À comparer aux baselines
# men-only : 0227 +175 / 0231 +142. King-awareness = occupation (pas géométrie) → patterns.py
# reste v4, reset-proof sans épinglage. gen8 sauvé pour autopsie king-aware vs Scan (avant/après).
set -uo pipefail
cd /root/jass
ART="/root/jass/jobs/results/cpx62-0241-kingloop-scaled/artefacts.src"; mkdir -p "$ART"
...
CFOLD="--full-fold --king-patterns"     # full-fold + ROIS dans les patterns (cohérent avec le binaire)
declare -A ELO PRX
...
echo "=== gen0 : seed (300k self-play embarqué depth4, l2=3e-2) KING-AWARE $CFOLD ==="
...
echo; echo "=========================================================="
echo "   cpx62-0241 — LOOP FULL-FOLD KING-AWARE SCALÉ (600k/gen) — TRAJECTOIRE Elo"
for g in $(seq 0 "$NGEN"); do echo "  gen$g  Elo_vs_hc=${ELO[$g]}  proxy=${PRX[$g]}"; done
echo "  gen${NGEN} (60p) : $ELOF"
echo "  → comparer aux baselines MEN-ONLY : 0227 +175 / 0231 +142 (60p)."
echo "     king-aware + scalé monte plus haut ? = la classe linéaire se pousse (sans pivoter)."
echo "  gen8.pjtw conservé pour l'autopsie king-aware vs Scan (avant/après)."
echo "=========================================================="
