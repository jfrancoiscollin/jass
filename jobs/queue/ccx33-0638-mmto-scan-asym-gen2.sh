#!/usr/bin/env bash
# id: ccx33-0638-mmto-scan-asym-gen2
# description: RONDE MMTO #2 — ancrée sur le NOUVEAU champion gen2-mmto (bake 2026-07-07), sur self-play Scan ASYMÉTRIQUE
# PUR (conversion) car (a) les maîtres plafonnent/pauvres, (b) l'équilibré-Scan régresse (0633 : −30 dilf), (c) le pur-asym
# évite le bug de starvation du mix (un seul régime). Prof = coup Scan côté FORT (mt0.3 vs faible mt0.03). MMTO --leaf-mode
# --leaf-pov ancré gen2-mmto + WORKING-SET ON (n'entraîner que là où gen2-mmto désaccorde de Scan = sous-ensemble informatif).
# PROGRESS committé toutes les 10 min (parties/parents/ETA). A/B généraliste+dilf = job cpx62 suivant. AUCUN NNUE.
set -uo pipefail
cd /root/jass
NCPU=$(nproc); export TMPDIR=/root/jass/.compile-tmp; mkdir -p "$TMPDIR"
ART="/root/jass/jobs/results/ccx33-0638-mmto-scan-asym-gen2/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/ccx33-0638-mmto-scan-asym-gen2/artefacts"
RES="$ART/RESULTS.txt"; : > "$RES"; say(){ echo "$@" | tee -a "$RES"; }
W=/root/cw-mmto-r2; rm -rf "$W"; mkdir -p "$W"; GEOM=/root/jass-geom32-r2
...
say "=== RONDE MMTO #2 (ancre gen2-mmto, Scan asym conversion) — HEAD $(git log --oneline -1|cat) ==="
...
for s in $(seq 0 $((NCPU-1))); do
  python3 tools/scan_selfplay_gen.py --scan "$SCAN_BIN" --jass "$J" --seeds "$W/seeds.jnnw" \
    ...
    --skip-book "$SKIP" --keep-draw-frac "$DRAWFRAC" --seed 20638 --nshards "$NCPU" --shard "$s" >"$W/.sp-$s.log" 2>&1 &
  pids+=($!)
done
TOT=$((PERG*NCPU))
...
PY
    [ -s "$W/.prog" ] && { cat "$W/.prog" | tee -a "$RES"; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0638 progress gen ($(cat "$W/.prog"|grep -oE '[0-9]+/[0-9]+' ))" >/dev/null 2>&1 || true; }
    [ "$alive" = 0 ] && break
  done; }
...
gzip -c "$W/parents.jnnw" > "$ART/conv-parents.jnnw.gz"; gzip -c "$W/moves.bin" > "$ART/conv-moves.bin.gz"
commit_to_main "$ART/conv-parents.jnnw.gz" "$ARTREL/conv-parents.jnnw.gz" "0638 parents conversion Scan asym ($NPAR)" >/dev/null 2>&1 || true
commit_to_main "$ART/conv-moves.bin.gz" "$ARTREL/conv-moves.bin.gz" "0638 moves conversion" >/dev/null 2>&1 || true
...
env JASS_PATTERNS_DIR="$GEOM" PYTHONPATH="$GEOM:pattern_jass/tools" python3 pattern_jass/tools/rank_finetune.py \
    ...
    --full-fold --tempo-stage --leaf-pov --verify-jass "$J" --verify-n 60 >"$W/ft.log" 2>&1
if [ $? = 0 ]; then grep -E 'pairwise-acc|delta' "$W/ft.log" | sed 's/^/  /' | tee -a "$RES"
  gzip -c "$W/gen3.pjtw" > "$ART/gen3-candidate.pjtw.gz"
  commit_to_main "$ART/gen3-candidate.pjtw.gz" "$ARTREL/gen3-candidate.pjtw.gz" "0638 candidat gen3 (MMTO ronde 2, ancre gen2-mmto, conversion Scan, WS ON)" \
...
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0638 ronde MMTO 2 (gen2-mmto->gen3, conversion Scan, WS ON) : candidat pret pour A/B" \
