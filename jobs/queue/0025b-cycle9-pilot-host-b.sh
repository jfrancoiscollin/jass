#!/usr/bin/env bash
# id: 0025b-cycle9-pilot-host-b
# description: Cycle 9 pilot, host B's half. Generates 500K WDL records
...
#
#              Total target: 1M records over 2× CCX23 (0025a does 500K
#              on host A; this script does 500K on host B). At ~5 CPU-sec
#              per record on 4 vCPU CCX23, 500K = ~175 hours total CPU
#              time = ~44 hours wall on 4 cores ÷ 4 shards = ~67-72h.
#              Call it 3 days per host.
#
#              Requires: JASS_HOST_FILTER='0025b-' on host B's systemd
#              env so host A doesn't pick this job.
#
#              Reads:
...
#                $ART/host-b.bin                  (500K records, merged)
#
#              Pipeline continues at:
#                0025a — host A counterpart (500K, seeds 3001-3004)
...
OUT_BASE="/root/jass/jobs/results/0025b-cycle9-pilot-host-b"
...
PLAY_DEPTH=4
MAX_PLIES=200
# Seed range distinct from 0020a/b (1001-1004 / 2001-2004) and
# 0025a (3001-3004): 4001-4004 guarantees no overlap with any prior
# corpus, so the 1M dataset is fully independent.
SEED_BASE=4000
...
echo "=== host facts ==="
echo "host:    $(hostname)"
echo "filter:  ${JASS_HOST_FILTER:-(unset, expected '0025b-')}"
echo "nproc:   $(nproc)"
...
echo
echo "=========================================================="
echo "       0025b CYCLE-9 PILOT (host B) SUMMARY"
echo "=========================================================="
echo "  host:           $(hostname)"
...
echo "=========================================================="
echo
echo "Next: wait for 0025a (host A) to finish on its host, then queue"
