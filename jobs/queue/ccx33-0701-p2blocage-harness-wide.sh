#!/usr/bin/env bash
# id: ccx33-0701-p2blocage-harness-wide
...
ART="/root/jass/jobs/results/ccx33-0701-p2blocage-harness-wide/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/ccx33-0701-p2blocage-harness-wide/artefacts"
W=/root/cw-p2bloc
...
DILF=/root/dilf-src
PERG=8000; MAXPLIES=160; MINPIECES=12; STRONG_D=8; WEAK_D=3; JITTER=1; SKIP=8; DRAWFRAC=1.0; SEED=57000
ARB_MT=1.0; ARB_PAIRS=3; SHARD_TIMEOUT=3600
...
[ -n "$EGDIR" ] || { say "ABORT egdb"; restore_src; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0701 ABORT egdb"; exit 4; }
cmake -S . -B "$W/build" $FLAGS_EGDB >"$W/cmake.log" 2>&1 && grep -q "EXTERNAL EGDB ENABLED" "$W/cmake.log" \
  || { say "ABORT cmake"; tail -8 "$W/cmake.log"|sed 's/^/  /'; restore_src; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0701 ABORT cmake"; exit 6; }
cmake --build "$W/build" -j"$NCPU" --target jass >"$W/build.log" 2>&1 || { say "BUILD FAIL"; tail -10 "$W/build.log"|sed 's/^/  /'; restore_src; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0701 ABORT build"; exit 6; }
J="$W/build/jass"
...
PYTHONPATH="$DILF" python3 -c "from pedagogy.features.blocage import blocage_structurel, mutual_blocked; from pedagogy.game import GameState; from scripts.pcblues.rules import RulesEngine; print('dilf import OK')" >"$W/imp.log" 2>&1 \
  || { say "ABORT import dilf : $(cat "$W/imp.log"|tail -2)"; restore_src; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0701 ABORT dilf import"; exit 7; }
...
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0701 fires : 7p=$N7 big=$NBIG" >/dev/null 2>&1 || true
...
PY
say ""; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0701 FIN harnais P2-blocage : precision P(NULLE|fire) du prédicat C1" \
