#!/usr/bin/env bash
# id: ccx33-0207-wdl-loop-mt100
# description: NUIT (CCX33) — boucle WDL ITÉRÉE propre (mt100, SANS replay buffer),
# mesurée au PROXY DÉTERMINISTE (eval_proxy : accord Spearman avec Scan-d10 sur
# set fixe) en plus du bench v15 final. Remplace 0207 (tué par la course) avec
# la bonne mesure. Question Nœud 1 : la courbe proxy gen0→gen4 MONTE-t-elle ?
# (Le proxy n'a pas le bruit binomial des parties → on lit enfin le signal.)
set -uo pipefail
cd /root/jass
ART="/root/jass/jobs/results/ccx33-0207-wdl-loop-mt100/artefacts.src"; mkdir -p "$ART"
...
N=300000; SH=$NCPU; PER=$(( (N + SH - 1) / SH )); MT=100; PREV="$ART/gen0"
for g in 1 2 3 4; do
...
echo; echo "=========================================================="
echo "   ccx33-0207b BOUCLE WDL PROXY-MESURÉE — VERDICT"
echo "  COURBE proxy (Spearman des scores eval vs Scan-d10, set fixe 50k) :"
for g in 0 1 2 3 4; do echo "    gen$g = $(proxy "$ART/gen$g")"; done
echo "  gen4 v15 d9 (54 parties, continuité) = $(rate "$ART/gen4-v15d9.log")"
echo "  → proxy MONTE proprement gen0→gen4 = la boucle WDL COMPOUNDE (Nœud 1 = OUI)."
echo "  → proxy plat = ne grimpe pas à mt100 → profondeur (0206/0207) ou features."
echo "=========================================================="
