...
# lmp_max_depth=5,rfp7 vs Scan. mt égal 1.0, 72 parties/bras. B>A → l'ajout aide vs Scan → baker.
...
ART="/root/jass/jobs/results/cpx62-0341-vs-scan-robust/artefacts.src"; mkdir -p "$ART"
...
preflight_match 72 1.0 150; preflight_match 72 1.0 150
preflight_check
...
echo "=== ARM A : combo baké vs Scan (mt1.0, 72 parties) ==="
python3 tools/calibrate_vs_scan.py --jass "$JASS" --scan "$SCAN_BIN" --jass-pattern "$ART/eval.pjtw" \
    --scan-bb-size 0 --movetime 1.0 --pairs 4 --max-plies 150 --allow-long-movetime >"$ART/vs-combo.log" 2>&1 || true
echo "=== ARM B : combo + ${ADD} vs Scan (même build, --jass-search-params) ==="
python3 tools/calibrate_vs_scan.py --jass "$JASS" --scan "$SCAN_BIN" --jass-pattern "$ART/eval.pjtw" \
    --jass-search-params "$ADD" \
    --scan-bb-size 0 --movetime 1.0 --pairs 4 --max-plies 150 --allow-long-movetime >"$ART/vs-add.log" 2>&1 || true

echo; echo "=========================================================="
echo "   cpx62-0341 — combo (+lmp5/rfp7 ?) vs Scan, mt1.0, 72 parties/bras"
echo "----------------------------------------------------------"
