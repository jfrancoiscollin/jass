#!/usr/bin/env bash
# id: ccx33-0524-scan-search-eval-control
# description: BRAS CONTRÔLE du localiseur éval-vs-recherche (jumeau de cpx62-0523). MÊME corpus, MÊME pipeline, MÊME jauge
# 0440 — seule variable : le relabel vise le SCORE DE RECHERCHE de Scan (depth 12) au lieu de son éval STATIQUE (depth 0).
# C'est la reproduction propre de nos distillations historiques (0147 = Scan-d10) sur le corpus/pipeline courant, pour un
# static-vs-search apples-to-apples. LECTURE (croisée avec 0523) : static ~ search => la profondeur du label n'est PAS le
# levier ; static >> search => la contamination-recherche plombait le fit historique (le vrai signal était l'éval statique) ;
# search >> static => (peu probable) la recherche AIDE le label. AUCUN NNUE (règle gravée). expected_duration: ~3-6 h.
set -uo pipefail
cd /root/jass
NCPU=$(nproc); export TMPDIR=/root/jass/.compile-tmp; mkdir -p "$TMPDIR"
ART="/root/jass/jobs/results/ccx33-0524-scan-search-eval-control/artefacts"; mkdir -p "$ART"
...
RELABEL_DEPTH=12       # 12 = score de RECHERCHE de Scan (contrôle ; contamination combinatoire)
MAXREC=400000          # identique à 0523
PLAY_DEPTH=11
...
say "=== build jass (mêmes flags que 0523 : ENDGAME+KING_MOBILITY+SCAN_PARITY+TEMPO) ==="
...
say "=== Phase 1 : relabel corpus au SCORE DE RECHERCHE de Scan (depth ${RELABEL_DEPTH}, ${MAXREC} pos, ${NCPU} threads) ==="
say "    (depth 12 => relabel LENT : ~horizon combinatoire ; c'est le point : montrer ce que la contamination donne)"
...
sc=[struct.unpack('<i',body[i*38+33:i*38+37])[0] for i in range(n)]
nz=sum(1 for s in sc if s!=0)
print(f"  relabel : {n} pos ; non-nuls {nz}/{n} ({100*nz/max(n,1):.1f}%) ; min {min(sc)} max {max(sc)} mediane {int(st.median(sc))}")
PY
...
say ""
say "=== Phase 4 : JOUE jass-search + Scan-SEARCH-eval vs Scan sur la jauge 0440 (depth ${PLAY_DEPTH}, no-DB) ==="
...
if ja_n and sc_n:
    print(f"  ECART jass - scan : {ja_w/ja_n - sc_w/sc_n:+.3f}")
PY

say ""
say "  LECTURE : à croiser avec cpx62-0523 (bras statique). Voir le bloc LECTURE de 0523."
say "  (parties dumpées dans artefacts/games/ ; v3 recherche = artefacts/scan_search_eval_v3.pjtw)"
