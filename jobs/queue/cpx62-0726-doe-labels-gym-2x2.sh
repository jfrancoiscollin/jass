#!/usr/bin/env bash
# id: cpx62-0726-doe-labels-gym-2x2
...
JOB_ID=cpx62-0726-doe-labels-gym-2x2
exec 9>/root/.jass-0726.lock
if ! flock -n 9; then echo "ABORT 0726 : instance deja active"; exit 0; fi
...
relabel_strict(){ # input output prefix timeout
  ...
  for s in $(seq 0 $((NSH-1))); do
    ...
    timeout "$timeout_s" "$J" --deep-relabel "$W/${prefix}_src.$tag.jnnw" "$W/${prefix}_rel.$tag.jnnw" \
      "$ARB_DEPTH" --egdb "$EGDIR" --cache-mb 512 >"$W/${prefix}_rel.$tag.log" 2>&1 &
    pids+=($!)
  done
  local failed=0
...
agg={"n_pos":0,"n_win":0,"n_draw":0,"n_loss":0,"n_errors":0,"n_restarts":0,"n_skipped_draw_label":0}
...
_tol=max(10,int(0.08*(agg["n_pos"]+agg["n_errors"])))
...
commit_to_main "$ART/corpus_manifest.json" "$ARTREL/corpus_manifest.json" "0726 DOE corpus manifest strict matched" >/dev/null 2>&1 || say "  ⚠ commit corpus manifest"
commit_to_main "$ART/doe_manifest.json" "$ARTREL/doe_manifest.json" "0726 DOE metrics and interaction manifest" >/dev/null 2>&1 || say "  ⚠ commit DOE manifest"
restore_src
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0726 FIN DOE 2x2 labels x gym G1/G$GYM_W" && say "  ✓ RESULTS committé" || say "  ⚠ commit RESULTS"
say "=== 0726 FINI ==="
rm -rf "$W" "$GEOM"
