#!/usr/bin/env bash
# id: ccx33-0704-p1-runaway-harness
...
set -uo pipefail
cd /root/jass
exec 9>/root/.jass-0704.lock
if ! flock -n 9; then echo "ABORT 0704 : instance deja active"; exit 0; fi
NCPU=$(nproc); export TMPDIR=/root/jass/.compile-tmp; mkdir -p "$TMPDIR"
ART="/root/jass/jobs/results/ccx33-0704-p1-runaway-harness/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/ccx33-0704-p1-runaway-harness/artefacts"
W=/root/cw-p1run
...
[ -n "$EGDIR" ] || { say "ABORT egdb"; restore_src; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0704 ABORT egdb"; exit 4; }
cmake -S . -B "$W/build" $FLAGS_EGDB >"$W/cmake.log" 2>&1 && grep -q "EXTERNAL EGDB ENABLED" "$W/cmake.log" \
  || { say "ABORT cmake"; tail -8 "$W/cmake.log"|sed 's/^/  /'; restore_src; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0704 ABORT cmake"; exit 6; }
cmake --build "$W/build" -j"$NCPU" --target jass >"$W/build.log" 2>&1 || { say "BUILD FAIL"; tail -10 "$W/build.log"|sed 's/^/  /'; restore_src; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0704 ABORT build"; exit 6; }
J="$W/build/jass"
...
PYTHONPATH="$DILF:pattern_jass/tools" python3 -c "from adjud.predicates import p1_runaway_win, ahead_side; from pedagogy.game import GameState; from scripts.pcblues.rules import RulesEngine; print('p1 import OK')" >"$W/imp.log" 2>&1 \
  || { say "ABORT import p1/dilf : $(cat "$W/imp.log"|tail -2)"; restore_src; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0704 ABORT p1 import"; exit 7; }
...
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0704 fires : 7p=$N7 big=$NBIG" >/dev/null 2>&1 || true
...
commit_to_main "$ART/p1_fires7_relabeled.jnnw" "$ARTREL/p1_fires7_relabeled.jnnw" "0704 P1 fires ≤7p TB-relabeled" >/dev/null 2>&1 || true
restore_src
say ""; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0704 FIN harnais P1-percée : precision P(WIN|fire) — $([ "$N7" -ge 30 ] 2>/dev/null && echo 'gate ≤7p' || echo 'sous-puissant')" \
