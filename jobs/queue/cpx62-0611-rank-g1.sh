...
ART="/root/jass/jobs/results/cpx62-0611-rank-g1/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/cpx62-0611-rank-g1/artefacts"
RES="$ART/RESULTS.txt"; : > "$RES"; say(){ echo "$@" | tee -a "$RES"; }
W=/root/cw-rankg1e; rm -rf "$W"; mkdir -p "$W"; GEOM=/root/jass-geom32-rank
...
# ---- FINE-TUNE rank-loss (avec les 3 gates auto) ----
say ""; say "=== rank_finetune λ=$LAM (POV gate + grad-check + pairwise-acc) ==="
env JASS_PATTERNS_DIR="$GEOM" PYTHONPATH="$GEOM:pattern_jass/tools" python3 pattern_jass/tools/rank_finetune.py \
    --champion "$W/gen1.pjtw" --pairs "$W/pairs.jnnw" --feat "$W/feat" --out "$W/cand.pjtw" \
    --tools pattern_jass/tools --lam "$LAM" --anchor 1e-3 --rank-scale 1.0 --max-iter 60 \
    --full-fold --tempo-stage --verify-jass "$J" --verify-n 80 >"$W/ft.log" 2>&1
FTRC=$?
sed 's/^/  /' "$W/ft.log" | tee -a "$RES"
git checkout -- pattern_jass/tools/rank_finetune.py 2>/dev/null || true
[ "$FTRC" = 0 ] || { say "  => rank_finetune a AVORTÉ (rc=$FTRC) — un gate a échoué (voir ci-dessus). Pas de G1."; \
  commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0606 G1 : rank_finetune abort (gate) — diagnostic"; exit 0; }
...
survie(){ local pat="$1" tag="$2"
  ...
  cat "$W"/${tag}.* > "$W/${tag}.all" 2>/dev/null
...
say ""; say "=== G1 : survie-1er-choix (d1==d11) champion vs candidat ==="
survie "$W/gen1.pjtw" "champ"
survie "$W/cand.pjtw"  "cand"
say ""
...
gzip -c "$W/cand.pjtw" > "$ART/cand-rankft.pjtw.gz" 2>/dev/null || true
commit_to_main "$ART/cand-rankft.pjtw.gz" "$ARTREL/cand-rankft.pjtw.gz" "0606 G1 : candidat rank-finetuné (λ=$LAM)" || true
