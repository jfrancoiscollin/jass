#!/usr/bin/env bash
# id: ccx33-0283-egdb-innoextract-selfcheck
...
set -uo pipefail
cd /root/jass
ART="/root/jass/jobs/results/ccx33-0283-egdb-innoextract-selfcheck/artefacts.src"; mkdir -p "$ART"
...
IE=$(find /root/ie -name innoextract -type f 2>/dev/null | head -1); chmod +x "$IE" 2>/dev/null || true
cd /root/jass
