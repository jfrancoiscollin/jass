#!/usr/bin/env bash
# id: 0062-h1-h3-h4-combined
# description: H1+H3+H4 combined. The kitchen sink : v3 geometry +
#              master games bootstrap + king mobility, all together.
#
# Activation rationale : 0061 may have shown that bootstrap-alone has
# pearson 0.7+ vs v7 (signal qualitatif) mais win-rate flat. This job
# tests if adding the LAST missing Scan feature (king mobility) to the
# bootstrap+self-play loop unlocks the calibration that was missing.
#
# Phase A : supervised bootstrap on master games WITH --mobility →
#           pattern-bootstrap-h4.jpat (v6 with mobility weights learned
#           from master WDL).
# Phase B : self-play loop from this starter, 10 iter × 20K records,
#           --mobility throughout.
#
# Decision gate :
#   rate vs v6 d10 ≥ 0.10 → COMBO unlock pattern axis
#   ∈ (0, 0.10)           → marginal signal, considérer H2 volume
#   = 0 (mais pearson > 0.75) → calibration limite intrinsèque archi v3
#   < 0.05 + pearson plat → toutes hypothèses cheap testées
#
# expected_duration: ~3-4h sur 8 vCPU CCX33 :
#                    * bootstrap supervised v3+mobility sur 4.7M
#                      master records (~1-1.5h)
#                    * 10 iter × (gen + train + bench) ~10-15 min each
set -uo pipefail
cd /root/jass

OUT_BASE="/root/jass/jobs/results/0062-h1-h3-h4-combined"
...
# Phase A : bootstrap supervised on master games WITH --mobility (H3
# + H4). Produces v6 starter with all features (skeleton, PST, balance,
# mobility, phase split) trained on real WDL signal.
MASTER="/root/jass/jobs/results/0014-fetch-master-games/artefacts.src/master-1600.jnnw"
[ -f "$MASTER" ] || MASTER="/root/jass/jobs/results/0014-fetch-master-games/artefacts.src/master-2000.jnnw"
[ -f "$MASTER" ] || { echo "ABORT: master corpus not found"; exit 3; }

INIT_JPAT="$ART/h134-v0.jpat"
START_BOOT=$(date +%s)
...
# Sanity-bench the bootstrap itself before self-play
[ -n "$V6" ] && ./build/jass --benchmark-nnue-vs-nnue "$INIT_JPAT" "$V6" 10 3 2>&1 | tee "$ART/bootstrap-vs-v6.log" | tail -2
BOOT_RATE_V6=""
[ -f "$ART/bootstrap-vs-v6.log" ] && BOOT_RATE_V6=$(grep -oE 'score rate: [0-9.]+' "$ART/bootstrap-vs-v6.log" | head -1 | awk '{print $3}')
...
echo
echo "=========================================================="
echo "       0062 H1+H3+H4 COMBINED VERDICT"
echo "=========================================================="
