#!/usr/bin/env bash
# id: ccx33-0362-lean8-distill-loop-colorfold
# description: BOUCLE distillation Scan, archi LEAN 8 — CORRIGÉE (0361 tué : --full-fold REPLIAIT les 8 bandes verticales
# (toutes des TRANSLATES) en 66 977 poids = ~32× sous la capacité pattern de Scan (~2.1M) ET imposait une invariance
# par translation/réflexion que Scan N'UTILISE PAS (il garde ses 4 colonnes distinctes) → fit plafonné SOUS Scan par
# construction). Fix : --color-fold = 2 125 768 poids ≈ EXACTEMENT la capacité de Scan, 8 patterns DISTINCTS, seule
# l'antisymétrie COULEUR repliée (correcte). Reste : gen0 = distill old-scan (8-pat, color-fold) ; chaque gen,
# self-play(gen_{k-1}) → relabel Scan d9 → pool → refit ; juge gen_k vs gen0 EN DIRECT (8-pat même binaire, bande
# ~0.5 = SENSIBLE) + vs Scan (gen0 et dernier). gen0 vs Scan doit décoller du plancher 0.028 de 0359 (= preuve que le
# fold était bien le bug). DIRECT croît > 0.55 → itérer améliore le fit (covariate shift) → SCALER. Plat → fit déjà
# convergé. NB : 0359 "8=32" est CONFONDU (son arm 8 était plié à 67k ; à re-juger color-fold un jour).
...
ART="/root/jass/jobs/results/ccx33-0362-lean8-distill-loop-colorfold/artefacts.src"; mkdir -p "$ART"
...
fit(){ # <pool.jnnw> <out-prefix> — COLOR-FOLD (Scan-faithful 2.1M), PAS full-fold (qui replie les bandes en 67k).
  ...
  grep -E "color-fold|distinct weights|nnz" "$2-train.log" | head -3 | sed 's/^/    [fit] /'
}
selfplay(){ # <pilot.pjtw> <out.jnnw> <n> — positions que jass visite (labels jetés, relabel Scan ensuite)
...
declare -A DIR VS
echo "=== gen0 : distill old-scan (8-pat, COLOR-FOLD ~2.1M) = baseline corrigé ==="
fit "$POOL" "$ART/gen0"
DIR[0]="0.500"; VS[0]=$(vs_scan "$ART/gen0")
echo "  gen0 vsScan=${VS[0]}  (doit décoller du plancher 0.028 de 0359 si le fold était le bug)"
...
echo; echo "=========================================================="
echo "   ccx33-0362 — BOUCLE distillation Scan, LEAN 8, COLOR-FOLD (Scan-faithful 2.1M)"
echo "   gen :  DIRECT(vs gen0)   vsScan-d9"
for g in $(seq 0 "$NGEN"); do echo "    $g :   ${DIR[$g]:-NA}            ${VS[$g]:-NA}"; done
echo "----------------------------------------------------------"
echo "   gen0 vsScan >> 0.028 → le fold ÉTAIT le bug (capacité Scan rétablie)."
echo "   DIRECT croît > 0.55 → itérer améliore le fit (covariate shift = le manque) → SCALER la boucle."
echo "   DIRECT plat ~0.5 → fit one-shot déjà convergé → le levier est ailleurs."
echo "=========================================================="
