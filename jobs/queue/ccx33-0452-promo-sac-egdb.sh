...
ART="/root/jass/jobs/results/ccx33-0452-promo-sac-egdb/artefacts"; mkdir -p "$ART"
...
# auto-detection du repertoire egdb (path different selon la box : ccx33=/root/egdb_db, cpx62=/root/egdb_extracted)
EGDIR=""; for d in /root/egdb_db /root/egdb_extracted/app /root/egdb_extracted; do ls "$d"/db*.idx1 >/dev/null 2>&1 && { EGDIR="$d"; break; }; done
[ -n "$EGDIR" ] || { say "ABORT: egdb introuvable (db*.idx1 dans /root/egdb_db, /root/egdb_extracted/app, /root/egdb_extracted)"; exit 4; }
say "  egdb detecte : $EGDIR"
