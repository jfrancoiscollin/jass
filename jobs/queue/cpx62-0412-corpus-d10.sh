#!/usr/bin/env bash
# id: cpx62-0412-corpus-d10
# description: CORPUS continu cpx62 (volume d10, vers les 30M du gros fit train_stream). Génère 2800000 pos
# piloté PILOTE FIGE w32_full (0401, 32cf@29M) — gen pure vers doublement 60M, committe UN shard gzippé à la finalisation. Maillon de chaîne. Hors-tree, gzip. Aucun Scan.
# expected_duration: ~150 min
set -uo pipefail
cd /root/jass
export PREFLIGHT_CAP_MIN="${PREFLIGHT_CAP_MIN:-110}"
source jobs/lib/preflight.sh
ART="/root/jass/jobs/results/cpx62-0412-corpus-d10/artefacts"; mkdir -p "$ART"
W=/root/cw-cpx62-0412-corpus-d10; rm -rf "$W"; mkdir -p "$W"
