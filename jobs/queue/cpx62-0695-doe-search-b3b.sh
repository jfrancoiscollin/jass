...
ART="/root/jass/jobs/results/cpx62-0695-doe-search-b3b/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/cpx62-0695-doe-search-b3b/artefacts"
W=/root/cw-doe-b3b
...
git show origin/develop:src/movegen.cpp > src/movegen.cpp
git show origin/develop:tools/jass_vs_jass_arch.py > tools/jass_vs_jass_arch.py
restore_src(){ git checkout -- src/main.cpp src/search.cpp src/scan_eval.cpp src/movegen.cpp tools/jass_vs_jass_arch.py 2>/dev/null||true; }
arch_assert(){
...
[ "$build_ok" = 1 ] || { say "BUILD FAIL :"; grep -iE "error:|Error [0-9]|fatal" "$W/build.2.log"|head -12|sed 's/^/  /'|tee -a "$RES"; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0695 BUILD FAIL"; restore_src; exit 6; }
J="$W/build/jass"
...
for cell in "${CELLS[@]}"; do
  ...
  docell "$name" "$params"
  commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0695 DOE cellule $name : $(tail -1 "$RES")" >/dev/null 2>&1 || true
done
restore_src
...
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0695 FIN B3b DOE search : quel cut agressif convertit éval->movetime sur gen2-mmto" \
