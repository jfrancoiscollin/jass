#!/usr/bin/env bash
# id: ccx33-0607-siblings-corpus-400k
...
ART="/root/jass/jobs/results/ccx33-0607-siblings-corpus-400k/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/ccx33-0607-siblings-corpus-400k/artefacts"
RES="$ART/RESULTS.txt"; : > "$RES"; say(){ echo "$@" | tee -a "$RES"; }
W=/root/cw-sib7; rm -rf "$W"; mkdir -p "$W"
...
python3 - "$W/siblings-400k.jnnw" "$W"/pairs.*.jnnw <<'PY' 2>&1 | tee -a "$RES"
...
PY
gzip -c "$W/siblings-400k.jnnw" > "$ART/siblings-400k.jnnw.gz"
commit_to_main "$ART/siblings-400k.jnnw.gz" "$ARTREL/siblings-400k.jnnw.gz" "siblings-corpus: 50k parents, paires d9 m_min15 (piste-a etape 1)" \
  && say "  corpus committe job-side ($(du -h "$ART/siblings-400k.jnnw.gz"|cut -f1))" || say "  ⚠ commit corpus echoue"
