#!/usr/bin/env bash
# id: ccx33-0649-wdlft-full-retry
...
ART="/root/jass/jobs/results/ccx33-0649-wdlft-full-retry/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/ccx33-0649-wdlft-full-retry/artefacts"
...
JB=$(( NCPU>4 ? 4 : NCPU )); cmake --build "$W/build" -j"$JB" --target jass >"$W/build.log" 2>&1 || { say "BUILD FAIL (j=$JB)"; tail -20 "$W/build.log"|sed 's/^/  /'|tee -a "$RES"; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0649 BUILD FAIL"; restore_src; exit 6; }
J="$W/build/jass"
