#!/usr/bin/env bash
# id: cpx62-0232-rfe-prune24
# description: RFE GÉOMÉTRIE — BRAS ÉLAGUÉ (24 patterns = v4 moins les 8 de plus basse importance: anti_0/1/2, diag_5, sq_0/3, v_top_0, v_bot_0). Réplique EXACTE
# de la boucle full-fold de 0227 (8 gens, depth4, 300k/gen, l2 3e-2→3e-4, Elo réel vs hc
# par gen) pour mesurer la trajectoire 24-pat élaguée et la VITESSE d'eval (knps pattern / knps
# hc, indépendant de la machine). À comparer au bras TÉMOIN ccx33-0231 (32 pat).
# La géométrie est REGÉNÉRÉE au démarrage (gen_patterns --variant v4 --drop anti_0,anti_1,anti_2,diag_5,sq_0,sq_3,v_top_0,v_bot_0 --emit) : pas de
# dépendance au géo sur main. Question : élaguer 8 pat coûte-t-il de l'Elo, gagne-t-on en vitesse ?
set -uo pipefail
cd /root/jass
ART="/root/jass/jobs/results/cpx62-0232-rfe-prune24/artefacts.src"; mkdir -p "$ART"
...
python3 pattern_jass/tools/gen_patterns.py --variant v4 --drop anti_0,anti_1,anti_2,diag_5,sq_0,sq_3,v_top_0,v_bot_0 --emit > "$ART/geom.log" 2>&1 || { echo GEOM FAIL; cat "$ART/geom.log"; exit 4; }
...
echo; echo "=========================================================="
echo "   ccx33-0231 — RFE BRAS ÉLAGUÉ 24-PAT — TRAJECTOIRE Elo + VITESSE"
