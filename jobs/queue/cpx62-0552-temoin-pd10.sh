#!/usr/bin/env bash
# id: cpx62-0552-temoin-pd10
# description: BRAS TEMOIN pd10 (briefing JFC) — leve l ambiguite pd6 : gen2(pd6) vs gen1 = NEUTRE (-5). Meme pilote+prior gen1, meme asym/qs_sacs, PLAY_DEPTH 10 (vs 6), ~600k positions. Juge cand-T vs gen1 : si cand-T COMPOSE (borne basse IC>0.50) alors que gen2(pd6) ne composait pas => pd6 est le coupable (le pilote pd6 punit trop faiblement). Sinon plateau robuste a la profondeur. Champion committe JOB-SIDE (le runner rate les champions des jobs rapides). AUCUN NNUE. ~4h.
...
ART="/root/jass/jobs/results/cpx62-0552-temoin-pd10/artefacts"; mkdir -p "$ART"
RES="$ART/RESULTS.txt"; : > "$RES"; say(){ echo "$@" | tee -a "$RES"; }
W=/root/cw-temoin; rm -rf "$W"; mkdir -p "$W"; GEOM32=/root/jass-geom32-temoin
FRESH=600000; PLAY_DEPTH=10; LABEL_DEPTH=4; OPEN_PLIES=8; EXPLORE_EPS=5; MAXPLIES=200
...
fit "$W/corpus.jnnw" "$W/feat" "$W/temoin.pjtw"; rm -f "$W/feat"
grep -iE 'prior|iter|loss' "$W/temoin.log" | tail -5 | sed 's/^/  /' | tee -a "$RES"
gzip -c "$W/temoin.pjtw" > "$ART/champion-temoin.pjtw.gz"
if commit_to_main "$ART/champion-temoin.pjtw.gz" "$ARTREL/champion-temoin.pjtw.gz" "temoin: commit champion-temoin JOB-SIDE (robuste au runner)"; then
  say "  champion-temoin committé JOB-SIDE"; else say "  ⚠ commit job-side champion echoué"; fi

# ---------- JUGE : cand-T vs gen1 (LE diagnostic) + vs egdbmix (cumulé) ----------
say ""; say "=== JUGE cand-T (pd10) @ d$JUDGE_DEPTH, dilf x${JUDGE_PAIRS}pair ==="
pjudge "$W/temoin.pjtw" "$PILOT" "temoin-vs-gen1" | tee -a "$RES"
[ -s "$W/egdbmix.pjtw" ] && pjudge "$W/temoin.pjtw" "$W/egdbmix.pjtw" "temoin-vs-egdbmix" | tee -a "$RES"
say ""
say "  => DIAGNOSTIC : cand-T(pd10)-vs-gen1 borne basse IC >0.50 (COMPOSE) alors que gen2(pd6)-vs-gen1 = -5 NEUTRE"
say "     => pd6 coupable (pilote trop superficiel) => chaine a pd8/pd10. Sinon => plateau robuste a la profondeur."
say "=== fin temoin pd10 ==="
