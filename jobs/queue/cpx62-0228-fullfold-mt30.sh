...
set -uo pipefail
cd /root/jass
ART="/root/jass/jobs/results/cpx62-0228-fullfold-mt30/artefacts.src"; mkdir -p "$ART"
...
EVAL_DEPTH=8; PLAY_DEPTH=8; NPER=300000; NGEN=8; BD=9; EPAIRS=15
CFOLD="--full-fold"     # bras FULL-FOLD (couleur+rot180+translation+reflexion ~ echelle Scan)
declare -A ELO PRX
...
gen_and_append(){ local NN=$1; local OUTP=$2; local CUM=$3; shift 3; local PER=$(( (NN + NCPU - 1) / NCPU ))
  for s in $(seq 1 "$NCPU"); do
    $JASS --gen-data-wdl "$PER" "${OUTP}-$s.jnnw" "$EVAL_DEPTH" "$PLAY_DEPTH" 200 $((RANDOM)) "$@" --movetime 30 >"${OUTP}-$s.log" 2>&1 &
  done; wait
  python3 - "$OUTP" "$CUM" <<'PY'
...
echo; echo "=========================================================="
echo "   cpx62-0219 — SYM FULL-FOLD — TRAJECTOIRE Elo"
