#!/usr/bin/env bash
# id: cpx62-0556-gen2M-mixdepth
# description: GEN NUIT (JFC) — 2M positions self-play sur l'ARCHITECTURE COMPLETE (qs_sacs + qs_threat_ext bakes + F1
# NMP sound), MIX de profondeurs : 60% pd8 (1.2M) + 20% pd9 (400k) + 20% pd10 (400k). Pilote=gen1 (champion), asym
# conserve, quiet-only, combo-seed. GEN-ONLY : le fit se fera demain matin sur cette base (decision JFC au reveil,
# eclairee par le verdict F2 0554). Infra : progress/checkpoints NUMEROTES (fichiers neufs => committes aux heartbeats,
# contrairement aux fichiers modifies), corpus final committe JOB-SIDE. AUCUN NNUE. expected_duration: ~4-6 h (nuit).
set -uo pipefail
cd /root/jass
NCPU=$(nproc); export TMPDIR=/root/jass/.compile-tmp; mkdir -p "$TMPDIR"
ART="/root/jass/jobs/results/cpx62-0556-gen2M-mixdepth/artefacts"; mkdir -p "$ART"
RES="$ART/RESULTS.txt"; : > "$RES"; say(){ echo "$@" | tee -a "$RES"; }
W=/root/cw-mix2m; rm -rf "$W"; mkdir -p "$W"
...
ARTREL="jobs/results/cpx62-0556-gen2M-mixdepth/artefacts"

say "=== build jass depuis main (ARCHI COMPLETE : qs_sacs + qs_threat_ext + F1 NMP sound) ==="
...
say "  sanity qs_sacs : $(head -1 "$DILF" | sed 's/#.*//' | "$J" --dump-sacs 2>/dev/null | head -1)"
...
gen_phase(){ local pd="$1" nn="$2" tag="$3"; local per=$(( (nn+NCPU-1)/NCPU )); local pids=()
  say ""; say "=== PHASE $tag : ${nn} pos @ pd${pd} (x$NCPU shards) ==="
  ...
  # checkpoint de phase committe JOB-SIDE (un kill ne perd que la phase en cours)
  gzip -c "$W/${tag}.jnnw" > "$ART/phase-${tag}.jnnw.gz"
  commit_to_main "$ART/phase-${tag}.jnnw.gz" "$ARTREL/phase-${tag}.jnnw.gz" "gen2M: phase $tag committee (${NP} pos)" \
    && say "  phase $tag committee job-side" || say "  (commit phase $tag echoue, restera au finalize)"
}

# seed-file (dilf + lidraughts)
...
# ---- MIX 60/20/20 : pd8 1.2M, pd9 400k, pd10 400k ----
gen_phase 8 1200000 "pd8"
gen_phase 9 400000  "pd9"
gen_phase 10 400000 "pd10"

say ""; say "=== MERGE final : corpus-mix2M (60% pd8 / 20% pd9 / 20% pd10, archi complete, pilote gen1) ==="
NTOT=$(merge_into "$W/mix2m.jnnw" "$W/pd8.jnnw" "$W/pd9.jnnw" "$W/pd10.jnnw")
say "  total : ${NTOT} pos"
gzip -c "$W/mix2m.jnnw" > "$ART/corpus-mix2M.jnnw.gz"
commit_to_main "$ART/corpus-mix2M.jnnw.gz" "$ARTREL/corpus-mix2M.jnnw.gz" "gen2M: corpus mix 60/20/20 pd8/9/10 complet (${NTOT} pos, archi complete)" \
  && say "  corpus-mix2M.jnnw.gz committe JOB-SIDE ($(du -h "$ART/corpus-mix2M.jnnw.gz"|cut -f1))" || say "  (commit final echoue, restera au finalize)"
say "  => PRET pour le fit de demain matin (archi complete : qs_sacs + threat_ext + F1)."
say "=== fin gen2M mixdepth ==="
