#!/usr/bin/env bash
# id: ccx33-0206-wdl-loop-mt60
# description: NUIT (CCX33) — boucle WDL ITÉRÉE propre (mt60, SANS replay buffer),
# mesurée au PROXY DÉTERMINISTE (eval_proxy : accord Spearman avec Scan-d10 sur
# set fixe) en plus du bench v15 final. Remplace 0206 (tué par la course) avec
# la bonne mesure. Question Nœud 1 : la courbe proxy gen0→gen5 MONTE-t-elle ?
# (Le proxy n'a pas le bruit binomial des parties → on lit enfin le signal.)
set -uo pipefail
cd /root/jass
ART="/root/jass/jobs/results/ccx33-0206-wdl-loop-mt60/artefacts.src"; mkdir -p "$ART"
...
N=300000; SH=$NCPU; PER=$(( (N + SH - 1) / SH )); MT=60; PREV="$ART/gen0"
...
echo; echo "=========================================================="
echo "   ccx33-0206b BOUCLE WDL PROXY-MESURÉE — VERDICT"
...
echo "  → proxy MONTE proprement gen0→gen5 = la boucle WDL COMPOUNDE (Nœud 1 = OUI)."
echo "  → proxy plat = ne grimpe pas à mt60 → profondeur (0206/0207) ou features."
echo "=========================================================="
