#!/usr/bin/env bash
# id: ccx33-0417-corpus-d10
# description: CORPUS continu ccx33 (chaîne alternée d12/d10, nourrit le fit au fur et à mesure). Génère 350000 pos
# @ d10 piloté PILOTE FIGE w32_full (0401) — gen pure vers doublement 60M, committe UN shard gzippé. Le runner committe à la finalisation → shard
# dispo pour le futur gros fit train_stream. Maillon d'une chaîne ; le suivant démarre après. Aucun Scan. Hors-tree, gzip.
# expected_duration: ~60 min
set -uo pipefail
cd /root/jass
export PREFLIGHT_CAP_MIN="${PREFLIGHT_CAP_MIN:-110}"
source jobs/lib/preflight.sh
ART="/root/jass/jobs/results/ccx33-0417-corpus-d10/artefacts"; mkdir -p "$ART"
W=/root/cw-ccx33-0417-corpus-d10; rm -rf "$W"; mkdir -p "$W"
NCPU=$(nproc); export TMPDIR=/root/jass/.compile-tmp; mkdir -p "$TMPDIR"
PLAY_DEPTH=10; EVAL_DEPTH=4; TARGET=1400000; BATCH_MIN=25
SEED_CH=jobs/results/cpx62-0401-gate-matrix-2x2/artefacts/w32_full.pjtw.gz
preflight_build 1; preflight_note "corpus d10 350000" 90; preflight_check
...
while [ "$N" -lt "$TARGET" ]; do gen 200000 "$W/b" >/dev/null 2>&1; N=$(app "$W/b" "$ACC"); echo "  accumulé $N"; done
gzip -c "$ACC" > "$ART/corpus-d10.jnnw.gz"
echo "=== shard d10 : $N pos, $(( $(stat -c%s "$ART/corpus-d10.jnnw.gz")/1000000 )) Mo gz → committé (maillon chaîne corpus) ==="
