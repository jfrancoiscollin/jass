...
set -uo pipefail
cd /root/jass; ART="/root/jass/jobs/results/0192-freq-reg-stability-v2/artefacts.src"; mkdir -p "$ART"
NCPU=$(nproc); export TMPDIR=/root/jass/.compile-tmp; mkdir -p "$TMPDIR"
export CCACHE_DISABLE=1   # geometry emit changes NUM_PATTERNS → avoid stale cached objects (0191 rc=5)
...
cmake --build build-prod -j"$NCPU" --target jass jass_tests >"$ART/build.log" 2>&1 || { echo "build retry (clean)"; rm -rf build-prod; cmake -S . -B build-prod -DCMAKE_BUILD_TYPE=Release >>"$ART/cmake.log" 2>&1; cmake --build build-prod -j"$NCPU" --target jass jass_tests >"$ART/build.log" 2>&1 || { echo BUILD FAIL; tail -20 "$ART/build.log"; exit 5; }; }
