#!/usr/bin/env bash
# id: cpx62-0671-scratch-tour1-wdl
...
ART="/root/jass/jobs/results/cpx62-0671-scratch-tour1-wdl/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/cpx62-0671-scratch-tour1-wdl/artefacts"
RES="$ART/RESULTS.txt"; : > "$RES"; say(){ echo "$@" | tee -a "$RES"; }
PROG="$ART/PROGRESS.txt"; : > "$PROG"
W=/root/cw-scratch-t1; rm -rf "$W"; mkdir -p "$W"; GEOM=/root/jass-geom32-scratch
...
monitor_loop(){ while [ ! -f "$W/.stopmon" ]; do sleep 600; [ -f "$W/.stopmon" ] && break
  ...
  commit_to_main "$PROG" "$ARTREL/PROGRESS.txt" "0671 self-play +${el}min (~${approx} pos)" >/dev/null 2>&1||true; done; }
...
cmake --build "$W/build" -j"$NCPU" --target jass >"$W/build.log" 2>&1 || { say "BUILD FAIL"; tail -12 "$W/build.log"|sed 's/^/  /'|tee -a "$RES"; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0671 BUILD FAIL"; restore_src; exit 6; }
J="$W/build/jass"
...
PY
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0671 self-play fini (N=$N_WDL) + census" >/dev/null 2>&1||true
...
commit_to_main "$ART/eval2-scratch.pjtw.gz" "$ARTREL/eval2-scratch.pjtw.gz" "0671 eval(2) tour1 WDL ancre" >/dev/null 2>&1||true
...
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0671 FIN tour1 WDL : eval(2) compose-t-il sur eval(1) (zéro prof/Scan)" \
