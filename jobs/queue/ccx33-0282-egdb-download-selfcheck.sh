#!/usr/bin/env bash
# id: ccx33-0282-egdb-download-selfcheck
...
set -uo pipefail
cd /root/jass
ART="/root/jass/jobs/results/ccx33-0282-egdb-download-selfcheck/artefacts.src"; mkdir -p "$ART"
...
echo "  liveness (dl --help) :"; "$MT" dl --help >/dev/null 2>&1 && echo "  megatools OK" || { echo "ABORT: megatools inutilisable (libs manquantes ci-dessus)"; ldd "$MT" 2>&1 | grep -i 'not found'; exit 0; }
