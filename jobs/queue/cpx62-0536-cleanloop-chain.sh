#!/usr/bin/env bash
# id: cpx62-0536-cleanloop-chain
# description: CHAINE ITERATIVE from-scratch (JFC "boucle sur cpx62"). Self-play PUR (pas d expert_games.db sur cpx62 => masters skip). Seed materiel -> KGEN=15 gen chainees, recette forte (asym ext_forcing + quiet-only + ballots-fallback + egdb-mix), fit 32cf, juge 0440/tour, AUTO-STOP au point fixe. 3M/boucle. Commit + courbe par tour. Reprise-safe. AUCUN NNUE. ~1.5-3j.
...
ART="/root/jass/jobs/results/cpx62-0536-cleanloop-chain/artefacts"; mkdir -p "$ART"
...
KGEN=15
SEED_CORPUS=500000; SEED_FRAC=25
...
# ---------- mix STATIQUES (maîtres + egdb) générés UNE fois, réutilisés chaque génération ----------
MASTERS=""
if [ -f "$EXPERT_DB" ]; then
  python3 tools/master_games_to_jnnw.py --db "$EXPERT_DB" --jass "$J" --out "$W/masters.jnnw" \
     --min-plies 24 --skip-open 8 --skip-endgame-pieces 8 --max-games "$MASTER_MAX" >"$W/masters.log" 2>&1 \
     && [ -s "$W/masters.jnnw" ] && MASTERS="$W/masters.jnnw"
  [ -n "$MASTERS" ] && say "  maîtres (une fois) : $(python3 -c "import struct;print(struct.unpack('<I',open('$MASTERS','rb').read(8)[4:8])[0])" 2>/dev/null||echo 0)" || say "  (maîtres sautés)"
else say "  (pas d'expert_games.db sur cette box -> SELF-PLAY PUR, sans maîtres — fidèle à Scan)"; fi
EGMIX=""
if [ -n "$EGDIR" ]; then
  "$J" --gen-egdb-wld "$EGDB_POOL" "$W/egdb.jnnw" "$EGDIR" 7 2048 12345 >"$W/egdb.log" 2>&1 && [ -s "$W/egdb.jnnw" ] && EGMIX="$W/egdb.jnnw"
  [ -n "$EGMIX" ] && say "  egdb-finale (une fois) : $(python3 -c "import struct;print(struct.unpack('<I',open('$EGMIX','rb').read(8)[4:8])[0])" 2>/dev/null||echo 0)" || say "  (egdb sauté)"
else say "  (egdb absent -> mix egdb sauté)"; fi

# ---------- CHAÎNE : KGEN générations, pilote chaîné, auto-stop au point fixe ----------
CURVE="$ART/curve.txt"; [ -f "$CURVE" ] || : > "$CURVE"
PILOT="$SEED_MAT"; PREV=""; flat=0
for k in $(seq 1 "$KGEN"); do
  CHGZ="$ART/champion-gen$k.pjtw.gz"
  if [ -f "$CHGZ" ]; then gunzip -c "$CHGZ" > "$W/champ$k.pjtw"; PILOT="$W/champ$k.pjtw"; PREV="$W/champ$k.pjtw"; say "(reprise) gen $k déjà committée"; continue; fi
  PN=$([ "$PILOT" = "$SEED_MAT" ] && echo "seed-matériel" || echo "champ-gen$((k-1))")
  say ""; say "================= GÉNÉRATION $k / $KGEN (pilote=$PN) ================="
  ...
  say "  self-play (quiet-only) : ${NSP} pos"
  [ "${NSP:-0}" -ge 200000 ] || { say "ABORT gen $k : corpus trop petit ($NSP)"; exit 7; }
  concat "$W/corpus.jnnw" "$W/selfplay.jnnw" $MASTERS $EGMIX | tee -a "$RES"
  "$J" --dump-eval-features "$W/corpus.jnnw" "$W/feat" >"$W/feat.log" 2>&1 || { say "ABORT dump feat g$k"; exit 8; }
  fit "$W/corpus.jnnw" "$W/feat" "$W/champ$k.pjtw"
  gzip -c "$W/champ$k.pjtw" > "$CHGZ"
  VB=$(pjudge "$W/champ$k.pjtw" "$W/egdbmix.pjtw")
  VP="NA"; [ -n "$PREV" ] && VP=$(pjudge "$W/champ$k.pjtw" "$PREV")
  M="NA"
  if [ "$HAVE_SCAN" = 1 ]; then
    ( unset JASS_EGDB_PATH; python3 tools/calibrate_vs_scan.py --jass "$J" --scan "$SCAN_BIN" --jass-pattern "$W/champ$k.pjtw" \
        --scan-bb-size 0 --depth "$D" --pairs 1 --openings-file "$DILF_FEN" --dump-games-dir "$ART/conv-gen$k" >"$W/cv$k.log" 2>&1 ) || say "  (juge 0440 g$k échoué)"
    read JA JN SA SN < <(conv_rate "$ART/conv-gen$k"); M="$JA"
  fi
  say "gen $k  0440=$M  vs_egdbmix=$VB  vs_prev=$VP  pilote=$PN" | tee -a "$CURVE"
  rm -f "$W/feat" "$W/corpus.jnnw" "$W/selfplay.jnnw"
  if [ "$VP" != "NA" ] && awk "BEGIN{exit !(${VP:-1} <= 0.53)}"; then flat=$((flat+1)); else flat=0; fi
  PREV="$W/champ$k.pjtw"; PILOT="$W/champ$k.pjtw"
  [ "$flat" -ge 3 ] && { say ""; say "AUTO-STOP gen $k : vs_prev<=0.53 sur 3 tours (point fixe atteint)."; break; }
done

say ""; say "================= COURBE CHAÎNE (self-play pur from-scratch, cpx62) ================="
cat "$CURVE" | sed 's/^/  /' | tee -a "$RES"
say "  LECTURE : 0440 MONTE tour par tour => la recette forte casse le point fixe => on continue au plateau (champion)."
say "  0440 PLAT ~0,28-0,30 malgré la chaîne => plateau de CLASSE (from-scratch + recette forte) => preuve forte pour la gate NNUE."
