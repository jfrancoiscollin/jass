...
#   RATIO FINALE: --phase-weight endgame=3,deep-eg=3 (le fit PRIORISE les lignes finale ;
#                 le phase-split mg/eg protège l'ouverture STRUCTURELLEMENT (banc EG ≠ banc MG)
#                 → 3/3 visible sans pénaliser l'ouverture ; pas 4 pour attribution propre
#                 (ce run change déjà labels+lowmem). Plafond ~3-4 ; escalader si val/phase tient).
...
PHASE_W="endgame=3,deep-eg=3"            # RATIO FINALE : sur-pondère les lignes finale (0251).
                                         # 3/3 = visible mais PAS au plafond : le phase-split mg/eg
                                         # protège déjà l'ouverture (banc EG ≠ banc MG) ; deep-eg pas
                                         # à 4 car ce run change déjà 2 choses (labels profonds+lowmem)
                                         # → attribution propre. Plafond ~3-4 (cf ROADMAP) ; escalader
                                         # SEULEMENT si val/phase montre que l'ouverture tient.
