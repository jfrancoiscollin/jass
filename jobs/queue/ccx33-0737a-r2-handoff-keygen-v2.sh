#!/usr/bin/env bash
# id: ccx33-0737a-r2-handoff-keygen-v2
# description: generate one-time certificate for encrypted R2 handoff using tracked status paths
# expected_duration: <5 min
set -euo pipefail
cd /root/jass

JOB_ID="ccx33-0737a-r2-handoff-keygen-v2"
EXPECTED_HOST="ubuntu-16gb-hel1-2"
OUT="jobs/results/$JOB_ID/artefacts.src"
KEY="/root/.jass-r2-handoff-0737-private.pem"
CERT="/root/.jass-r2-handoff-0737-cert.pem"
...
cp "$CERT" status/ccx33-r2-0737-cert.pem
{
  echo "handoff=ccx33-r2-0737"
  echo "created_at=$(date -Is)"
  echo "host=$(hostname)"
  echo "certificate_sha256=$(sha256sum "$CERT" | awk '{print $1}')"
  echo "private_key_published=false"
} > status/ccx33-r2-0737-meta.txt

git add status/ccx33-r2-0737-cert.pem status/ccx33-r2-0737-meta.txt
git commit -m "secure handoff: publish ccx33 R2 certificate v2" >/dev/null
