#!/usr/bin/env bash
# id: cpx62-0465-freshmix-12m
# description: REPRISE du plan de base (= 0463 re-dimensionne 25M->12M, 0463 tue : TVR dit que le volume au-dela de la
# saturation ~10M ne deplace PAS le point fixe, il ne reduit que la variance => 12M screene les 5 recettes en ~2x moins
# de mur, on refit la gagnante a plein volume ensuite). PILOTE = champion-egdbmix (+58 Elo, le meilleur connu), apres
# l'echec prouve des deux supervisions tactiques auto-supervisees (0460 relabel-all 0.259, 0462 shot-filter 0.285 < 0.302)
# et le verdict 0461 = FIT-pas-FEATURE. Self-play 100% EPURE a chaque boucle (aucune reutilisation), un MIX de mu DIFFERENT
# par boucle (blend profondeur de JEU d8/d10/d12 + SEEDING hors-ouverture milieux-lidraughts/combinaisons-dilf + plies
# d'ouverture aleatoires + exploration epsilon + profondeur de label). Pilote = champion-egdbmix (montee gloutonne :
# une recette qui le bat devient le nouveau pilote). BASE/ancre du juge = egdbmix lui-meme => vs_base>0.55 = la composition
# de mu a CASSE le point fixe du MEILLEUR champion actuel. AUCUN NNUE, AUCUNE distillation Scan (regles gravees). Corpus
# frais box-local (regenerable) ; seuls champions + trajectoire committes.
set -uo pipefail
cd /root/jass
# ----- params -----
FRESH=12000000               # positions FRAICHES/boucle. 12M = au-dela de la saturation (~10M) sans gaspiller : TVR => le volume au-dela ne deplace
...
ART="/root/jass/jobs/results/cpx62-0465-freshmix-12m/artefacts"; mkdir -p "$ART"
...
BEST_CH="$W/champ0.pjtw"; BEST_VB="0.5000"; BEST_NAME="egdbmix"
say "iter 0  base=champion-egdbmix  (NREC=${NREC} recettes, FRESH=${FRESH}/boucle, 100% epure)" | tee -a "$TRAJ"
