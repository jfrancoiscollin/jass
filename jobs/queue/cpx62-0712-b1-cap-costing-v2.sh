#!/usr/bin/env bash
# id: cpx62-0712-b1-cap-costing-v2
# description: CHIFFRAGE B1 v2 — v1 (0710) INCONCLUANT : 0 caps car ouvertures=dilf_combinations (tactiques, résolvent
...
set -uo pipefail
cd /root/jass
exec 9>/root/.jass-0712.lock
if ! flock -n 9; then echo "ABORT 0712 : instance deja active"; exit 0; fi
NCPU=$(nproc); export TMPDIR=/root/jass/.compile-tmp; mkdir -p "$TMPDIR"
ART="/root/jass/jobs/results/cpx62-0712-b1-cap-costing-v2/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/cpx62-0712-b1-cap-costing-v2/artefacts"
W=/root/cw-0712
...
[ -n "$EGDIR" ] || { say "ABORT egdb"; restore_src; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0712 ABORT egdb"; exit 4; }
...
cmake --build "$W/build" -j"$NCPU" --target jass >"$W/build.log" 2>&1 || { say "BUILD FAIL"; tail -10 "$W/build.log"|sed 's/^/  /'; restore_src; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0712 BUILD FAIL"; exit 6; }
J="$W/build/jass"
...
say "  parties=$NG  cappées=$NCAP  plycap_rate=$PLYCAP  coût/partie=${SPG}s"
...
PY
restore_src
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0712 FIN chiffrage B1 v2 : plycap=$PLYCAP coutd14=$COSTD14 spg=$SPG" && say "  ✓ RESULTS committé" || say "  ⚠ commit"
say "=== 0712 FINI ==="
rm -rf "$W"
