#!/usr/bin/env bash
# id: cpx62-0578-famine-learningcurve
...
ART="/root/jass/jobs/results/cpx62-0578-famine-learningcurve/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/cpx62-0578-famine-learningcurve/artefacts"
...
python3 pattern_jass/tools/gen_patterns.py --emit --variant v3 >"$W/emit_v3.log" 2>&1 || true
cp pattern_jass/tools/patterns.py "$GEOM8/patterns.py"
python3 pattern_jass/tools/gen_patterns.py --emit --variant v4 >"$W/emit_v4.log" 2>&1 || true
cp pattern_jass/tools/patterns.py "$GEOM32/patterns.py"
# check via PYTHONPATH (un python nu N'utilise PAS JASS_PATTERNS_DIR — seul train_stream fait le sys.path.insert)
TOOLS="$(pwd)/pattern_jass/tools"
N8=$(PYTHONPATH="$GEOM8:$TOOLS" python3 -c "import patterns;print(patterns.NUM_PATTERNS)" 2>"$W/imp8.log")
N32=$(PYTHONPATH="$GEOM32:$TOOLS" python3 -c "import patterns;print(patterns.NUM_PATTERNS)" 2>"$W/imp32.log")
note "  geom : 8cf NUM_PATTERNS=$N8 ; 32cf NUM_PATTERNS=$N32"
if [ "$N8" != 8 ] || [ "$N32" != 32 ]; then
  note "  ABORT: geom counts $N8/$N32 != 8/32"
  note "  emit_v3: $(tail -2 "$W/emit_v3.log"|tr '\n' ' ')"; note "  imp8: $(tail -2 "$W/imp8.log"|tr '\n' ' ')"
  exit 7
fi
