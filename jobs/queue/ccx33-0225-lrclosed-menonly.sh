#!/usr/bin/env bash
# id: ccx33-0225-lrclosed-menonly
# description: PHASE 1 symmetry-sharing — A/B (bras MEN-ONLY témoin). Boucle WDL cumulée,
...
set -uo pipefail
cd /root/jass
ART="/root/jass/jobs/results/ccx33-0225-lrclosed-menonly/artefacts.src"; mkdir -p "$ART"
...
CFOLD=""     # bras TÉMOIN men-only (pas de --color-fold) ; cpx62-0219 = --color-fold
declare -A ELO PRX
...
echo; echo "=========================================================="
echo "   cpx62-0219 — LR-CLOSED MEN-ONLY (54-pattern baseline) — TRAJECTOIRE Elo"
echo "  gen :  Elo_vs_hc  |  proxy"
for g in $(seq 0 "$NGEN"); do echo "   $g : ${ELO[$g]}   |  ${PRX[$g]}"; done
echo "  → compare cpx62-0219 (color-fold) : color-fold monte-t-il PLUS HAUT que ce témoin en Elo ?"
