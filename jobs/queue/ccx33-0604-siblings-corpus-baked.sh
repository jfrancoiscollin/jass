#!/usr/bin/env bash
# id: ccx33-0604-siblings-corpus-baked
# description: PISTE (a) — REGEN du corpus fratries AVEC LE BAKE ORDERING (main l'a promu 9422fc02 => oracle d9 = prob-pur, plus profond). 0602 avait l'oracle legacy. Meilleur oracle => meilleures paires de preference. Produit le mini-corpus
...
ART="/root/jass/jobs/results/ccx33-0604-siblings-corpus-baked/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/ccx33-0604-siblings-corpus-baked/artefacts"
RES="$ART/RESULTS.txt"; : > "$RES"; say(){ echo "$@" | tee -a "$RES"; }
W=/root/cw-sib4; rm -rf "$W"; mkdir -p "$W"
...
say "  bake ordering present (oracle d9 = prob-pur) : $(git show origin/main:src/search_params.hpp|grep -cE 'int hist_mode  = 1|int hist_pure  = 1')/2"
...
python3 - "$W/siblings-50k-baked.jnnw" "$W"/pairs.*.jnnw <<'PY' 2>&1 | tee -a "$RES"
...
PY
gzip -c "$W/siblings-50k-baked.jnnw" > "$ART/siblings-50k-baked.jnnw.gz"
commit_to_main "$ART/siblings-50k-baked.jnnw.gz" "$ARTREL/siblings-50k-baked.jnnw.gz" "siblings-corpus: 50k parents, paires d9 m_min15 (piste-a etape 1)" \
  && say "  corpus committe job-side ($(du -h "$ART/siblings-50k-baked.jnnw.gz"|cut -f1))" || say "  ⚠ commit corpus echoue"
