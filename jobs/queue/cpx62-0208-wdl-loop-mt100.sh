...
# Profondeur de jeu = 100 ms. Question : la courbe proxy monte-t-elle quand on
# joue plus profond (vs mt30 plat de 0205b) ?
set -uo pipefail
cd /root/jass
ART="/root/jass/jobs/results/cpx62-0208-wdl-loop-mt100/artefacts.src"; mkdir -p "$ART"
...
shard_selfplay(){ # <N> <out-prefix> <gen-flag...>  → merged $2.jnnw
  ...
  for s in $(seq 1 "$NCPU"); do
    $JASS --gen-data-wdl "$PER" "${OUT}-$s.jnnw" 4 64 200 $((RANDOM)) "$@" --movetime 100 >"${OUT}-$s.log" 2>&1 &
  done; wait
  python3 - "$OUT" <<'PY'
...
for g in 1 2 3 4 5; do
  echo "=== gen$g : self-play 300k @mt100 avec gen$((g-1)) → WDL → logistic ==="
...
echo; echo "=========================================================="
echo "   cpx62-0208-wdl-loop-mt100 — BOUCLE WDL @mt100 PROXY-MESURÉE — VERDICT"
