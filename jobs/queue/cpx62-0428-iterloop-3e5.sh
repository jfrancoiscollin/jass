...
ART="/root/jass/jobs/results/cpx62-0428-iterloop-3e5/artefacts"; mkdir -p "$ART"
W=/root/cw-iterloop-3e5; rm -rf "$W"; mkdir -p "$W"
NCPU=$(nproc); export TMPDIR=/root/jass/.compile-tmp; mkdir -p "$TMPDIR"
GEOM32=/root/jass-geom32-i3e5; TRAJ="$ART/trajectory.txt"; : > "$TRAJ"
...
for k in $(seq 1 "$MAX"); do
  ...
  echo "iter $k  vs_prev=${SP}  vs_3=${CUM}  (L2=3e-5)" > "$ART/iter${k}-score.txt"   # fichier NEUF -> committe en DIRECT (visibilite live)
  case "$SP" in
