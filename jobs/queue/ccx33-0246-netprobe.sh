...
set -uo pipefail
cd /root/jass
ART="/root/jass/jobs/results/ccx33-0246-netprobe/artefacts.src"; mkdir -p "$ART"
SELF=$(hostname); OTHER=ubuntu-32gb-hel1-1
echo "=== SELF = $SELF ==="
...
OIP=$(git show "origin/main:jobs/results/cpx62-0246-netprobe/artefacts/pub_ip.txt" 2>/dev/null | tr -d '[:space:]')
if [ -n "$OIP" ] && [ "$OIP" != "?" ]; then
