#!/usr/bin/env bash
# id: cpx62-0460-tactical-supervision
...
ART="/root/jass/jobs/results/cpx62-0460-tactical-supervision/artefacts"; mkdir -p "$ART"
RES="$ART/RESULTS.txt"; : > "$RES"; say(){ echo "$@" | tee -a "$RES"; }
W=/root/cw-tacsup60; rm -rf "$W"; mkdir -p "$W"
...
GEOM32=/root/jass-geom32-tacsup60
POOL_TRIM=20000000; NEGDB=4000000; NTAC=300000; RELABEL_D=13; MID_LO=14; MID_HI=40; OVERSAMPLE=4
...
HAVE_SCAN=0; [ -x "$SCAN_BIN" ] && HAVE_SCAN=1 || say "  (Scan absent sur cpx62 — gate 0440 a faire sur ccx33)"
...
PY
  rm -f "$W"/j.* ; }
say ""; say "=== self-direct (toujours) + GATE 0440 (si Scan) ==="
say "  self-direct : tactical vs egdbmix = $(pjudge "$W/champ_tac.pjtw" "$W/pilot.pjtw")   (>0.5 = la supervision tactique aide)"
if [ "${HAVE_SCAN:-0}" = 1 ]; then
  ...
  say "  GATE 0440 : Scan absent sur cpx62 => champion-tactical committe ; faire la conversion 0440 vs Scan sur ccx33 (job dedie)."
fi
say ""; say "================= LECTURE ================="
