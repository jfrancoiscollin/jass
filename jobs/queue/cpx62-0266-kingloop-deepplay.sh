#!/usr/bin/env bash
# id: cpx62-0266-kingloop-deepplay
# description: LA FORMULE — self-play joué FORT UNIFORMÉMENT (Scan-aligné). Apprentissages :
# Scan = WDL self-play + logistique itéré (= notre méthodo), mais joué FORT → WDL justes
# partout, finale comprise → la finale bootstrappe. Nous à depth-4 = finale jouée au hasard =
# labels-bruit. Densifier la finale SEULE nuit (0263 contrôle -30 : incohérence finale-forte /
# milieu-faible). Donc : play_depth 4→8 UNIFORME (pas de --play-depth-by-phase), binaire NMP-off
# (search plus forte → labels meilleurs). Sinon = 0241 à l'identique. Compare à 0241=+173.7 (même
# binaire) ET autopsie finale vs Scan (perte finale-rois < 3.6 de 0250 = la finale a grimpé ?).
set -uo pipefail
cd /root/jass
ART="/root/jass/jobs/results/cpx62-0266-kingloop-deepplay/artefacts.src"; mkdir -p "$ART"
...
EVAL_DEPTH=6; PLAY_DEPTH=8; NPER=600000; NGEN=8; BD=9; EPAIRS=15   # play UNIFORME depth-8
CFOLD="--full-fold --king-patterns"
declare -A ELO EGM
echo "PARAMS: play_depth=$PLAY_DEPTH UNIFORME (pas de phase-weight/label-depth/play-depth-by-phase)  $NPER/gen ×$NGEN"
...
for g in $(seq 1 "$NGEN"); do
  echo "=== gen$g : +${NPER} (play depth-8) avec gen$((g-1)) → cumulé → logistic ==="
...
echo; echo "=========================================================="
echo "   cpx62-0266 — FORMULE : self-play PLAY DEPTH-8 UNIFORME (king-aware, NMP-off)"
echo "----------------------------------------------------------"
for g in $(seq 0 "$NGEN"); do echo "  gen$g  Elo_vs_hc=${ELO[$g]}   val_endgame_mse=${EGM[$g]}"; done
echo "  gen${NGEN} (60p) vs hc : $ELOF   [0241 vanilla même binaire = +173.7]"
[ -n "$SCAN5" ] && echo "  gen${NGEN} vs Scan mt0.5 : $SCAN5"
echo "----------------------------------------------------------"
echo "  AUTOPSIE FINALE (perte par phase × rois vs Scan ; comparer 0250 : endgame-rois ~3.6) :"
echo "$AUTOP"
echo "----------------------------------------------------------"
echo "  Elo > +173.7 ET perte finale-rois < 3.6 → JOUER FORT UNIFORME densifie la finale = LA FORMULE."
echo "  Finale toujours ~3.6 → play depth-8 insuffisant (éval trop lente pour self-play assez profond)"
echo "     → la finale a besoin de labels EXTERNES (recherche profonde / bitbases comme TEACHER de l'éval)."
echo "=========================================================="
