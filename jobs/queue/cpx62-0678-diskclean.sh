#!/usr/bin/env bash
# id: cpx62-0678-diskclean
# description: MAINTENANCE disque (demande JFC apres cpx62 rempli a 100% => runner mort + 0670 boucle). Nettoie les scratch de
...
set -uo pipefail
cd /root/jass
ART="/root/jass/jobs/results/cpx62-0678-diskclean/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/cpx62-0678-diskclean/artefacts"
...
say "=== DISKCLEAN cpx62 — $(hostname) $(date -u +%FT%TZ) ==="
...
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0678 diskclean cpx62 : ${AVAIL_MB}Mo libres" && say "  RESULTS committé ✓" || say "  ⚠ commit"
say "=== fin diskclean ==="
