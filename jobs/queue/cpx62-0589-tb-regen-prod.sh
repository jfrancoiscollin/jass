#!/usr/bin/env bash
# id: cpx62-0589-tb-regen-prod
# description: MANCHE EVAL PROD (JFC "Je valide") — replie FIX#4 tb-relabel (0587 : +18 Elo hors-IC, seul levier label
# gagnant, cible le biais finale P2=31%) dans le PACKAGE GEN CHAMPION (mirror 0566/0568 : mix 60/20/20 pd8/9/10 2M, asym
# punisher, quiet-only, seed enrichment dilf+lidraughts, explore-eps5, open-plies8) + --tb-relabel (labels finale EXACTS
# via EGDB). Fit prior gen1 + combos (mirror 0568). Build EGDB (0587 cmake, "EXTERNAL EGDB ENABLED"). JUGE moteur COIN par
...
# Fit = train_stream de main (identique a champion-regen). AUCUN NNUE. ~8-10h cpx62.
set -uo pipefail
cd /root/jass
NCPU=$(nproc); export TMPDIR=/root/jass/.compile-tmp; mkdir -p "$TMPDIR"
ART="/root/jass/jobs/results/cpx62-0589-tb-regen-prod/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/cpx62-0589-tb-regen-prod/artefacts"
RES="$ART/RESULTS.txt"; : > "$RES"; say(){ echo "$@" | tee -a "$RES"; }
W=/root/cw-tbregen; rm -rf "$W"; mkdir -p "$W"; GEOM32=/root/jass-geom32-tbregen
...
[ -n "$EGDBP" ] || { say "ABORT egdb data /root/egdb_extracted introuvable (tb-relabel serait no-op)"; exit 5; }
...
gen_phase(){ local pd="$1" nn="$2" tag="$3"; local per=$(( (nn+NCPU-1)/NCPU )); local pids=()
  ...
  [ -n "${TBR:-}" ] && [ "$TBR" -gt 0 ] 2>/dev/null && say "  ✓ tb_relabel=$TBR>0 (FIX#4 actif)" || say "  ⚠ tb_relabel absent/0 sur shard1 (verifier egdb)"
