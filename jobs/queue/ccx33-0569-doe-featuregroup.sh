#!/usr/bin/env bash
# id: ccx33-0569-doe-featuregroup
...
ART="/root/jass/jobs/results/ccx33-0569-doe-featuregroup/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/ccx33-0569-doe-featuregroup/artefacts"
...
run_cfg(){ local A="$1" B="$2" C="$3" D="$4"; local tag="A${A}B${B}C${C}D${D}"; local bd="$W/b_$tag"
  ...
  cmake -S . -B "$bd" -DCMAKE_BUILD_TYPE=Release \
     ...
     -DJASS_SCAN_PARITY=$(onoff $C) -DJASS_TEMPO_STAGE=$(onoff $D) >"$bd/cmake.log" 2>&1
  if ! cmake --build "$bd" -j"$NCPU" --target jass >"$bd/build.log" 2>&1; then
    say "  BUILD FAIL (config invalide ?) : $(tail -2 "$bd/build.log"|tr '\n' ' ')"; echo -e "$A\t$B\t$C\t$D\tNA\tbuildfail" >>"$RANK"; return; fi
  local J="$bd/jass"
  ...
  rm -f "$bd/feat"; rm -rf "$bd"/CMakeFiles 2>/dev/null || true
}
