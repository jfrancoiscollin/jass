#!/usr/bin/env bash
# id: cpx62-0654-chain-t1b-fit
# description: CHAÎNE ITÉRATIVE LONGUE (mémo JFC) — TOUR 1b, FIT. Consomme le self-play champion-driven 0653 (318k WDL +
# 94k prefs, distribution MOBILE). Pipeline : wdl_finetune ANCRÉ gen2-mmto anchor=0.3 (tight au tour 1) => wdlbase' ;
# MMTO last (gen-siblings --leaf-mode WS-OFF ancré wdlbase' + rank_finetune --leaf-pov --chunk anchor 0.05) => champion(1)
# gen3-chain-t1b ; A/B vs gen2-mmto (généraliste ≥38p, mt0.2+mt0.3). GATE compose-vs-sature : compose (+ hors-IC) => promu,
...
ART="/root/jass/jobs/results/cpx62-0654-chain-t1b-fit/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/cpx62-0654-chain-t1b-fit/artefacts"
RES="$ART/RESULTS.txt"; : > "$RES"; say(){ echo "$@" | tee -a "$RES"; }
W=/root/cw-chain-t1b-fit; rm -rf "$W"; mkdir -p "$W"; GEOM=/root/jass-geom32-chaint1b
...
WDL_GZ=jobs/results/cpx62-0653-chain-t1b-selfplay/artefacts/chain-t1b-wdl.jnnw.gz
PP_GZ=jobs/results/cpx62-0653-chain-t1b-selfplay/artefacts/chain-t1b-prefs-parents.jnnw.gz
PM_GZ=jobs/results/cpx62-0653-chain-t1b-selfplay/artefacts/chain-t1b-prefs-moves.bin.gz
...
commit_to_main "$ART/wdlbase-a$(echo "$WDLFT_ANCHOR"|tr '.' 'p').pjtw.gz" "$ARTREL/wdlbase-a$(echo "$WDLFT_ANCHOR"|tr '.' 'p').pjtw.gz" "0654 wdlbase ancré gen2 anchor=$WDLFT_ANCHOR" >/dev/null 2>&1 || true
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0654 progress : wdlbase fitté" >/dev/null 2>&1 || true
...
gzip -c "$W/gen4.pjtw" > "$ART/gen3-chain-t1b.pjtw.gz"
commit_to_main "$ART/gen3-chain-t1b.pjtw.gz" "$ARTREL/gen3-chain-t1b.pjtw.gz" "0654 candidat gen3-chain-t1b (MMTO last ancré wdlbase')" >/dev/null 2>&1 || true
...
say ""; say "=== (3) A/B gen3-chain-t1b vs gen2-mmto (généraliste) ==="
...
open(outp,'w').write(f"  [gen3-chain-t1b vs gen2-mmto | generaliste mt{mt}] A={a} B={b} D={d} n={g} rate_A={r:.4f}+-{1.96*se:.4f} elo~{elo:+.0f} IC=[{lo:.3f},{hi:.3f}] => {vd}\n")
PY
  cat "$W/.about" | tee -a "$RES"; rm -f "${pref}".* ; }
abcell 0.2
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0654 A/B gen4 mt0.2" >/dev/null 2>&1 || true
abcell 0.3
restore_src
say ""; say "  GATE : gen3-chain-t1b >0.5 hors-IC => le pipeline MMTO-last sur base WDL-calibrée-ancrée AJOUTE sur gen2-mmto => confirm dilf + d9-vs-Scan + bake."
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0654 FIN : gen3-chain-t1b (MMTO last, anchor $WDLFT_ANCHOR) vs gen2-mmto" \
