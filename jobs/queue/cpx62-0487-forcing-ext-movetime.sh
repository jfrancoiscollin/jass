#!/usr/bin/env bash
# id: cpx62-0487-forcing-ext-movetime
...
ART="/root/jass/jobs/results/cpx62-0487-forcing-ext-movetime/artefacts"; mkdir -p "$ART"
W=/root/cw-extmt; mkdir -p "$W"
...
# A = ext_forcing ON, B = baseline. Sharded ; sum RESULT lines (A_wins draws B_wins).
say "=== ext_forcing ON vs OFF @ movetime ${MOVETIME}s (eval egdbmix des 2 cotes, ouvertures diverses, pairs=${PAIRS}) ==="
for s in $(seq 0 $((NCPU-1))); do
  python3 tools/jass_vs_jass_arch.py --jass-a "$J" --pattern-a "$W/egdbmix.pjtw" \
      ...
      --search-params-a "ext_forcing=1" --search-params-b "" >"$ART/shard-$s.txt" 2>"$W/sh-$s.log" & done
wait
