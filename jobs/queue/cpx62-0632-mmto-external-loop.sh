#!/usr/bin/env bash
# id: cpx62-0632-mmto-external-loop
# description: BOUCLE EXTERNE MMTO (le multiplicateur en attente, idée JFC). Le fit 0625/0628 est ONE-SHOT : poids bougés →
# PV bougées → feuilles du corpus PÉRIMÉES. Cœur de MMTO = re-générer les feuilles avec l'éval CANDIDATE puis re-fitter,
# 2-3 itérations, sur le MÊME corpus. Corpus = positions maîtres (0624) + prof Scan (0625) — celui qui a donné +47. Ancrage
# à gen1 constant ; éval-de-feuille (--nnue) = candidat courant. Convergence attendue : pré-fit pairwise-acc MONTE, delta
# RÉTRÉCIT. Puis A/B cand3 vs gen1 (généraliste=signal + dilf=garde-fou, mt0.2+0.3) : si > +47 => la boucle multiplie
# GRATUITEMENT sur le même corpus. AUCUN NNUE.
set -uo pipefail
cd /root/jass
NCPU=$(nproc); export TMPDIR=/root/jass/.compile-tmp; mkdir -p "$TMPDIR"
ART="/root/jass/jobs/results/cpx62-0632-mmto-external-loop/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/cpx62-0632-mmto-external-loop/artefacts"
RES="$ART/RESULTS.txt"; : > "$RES"; say(){ echo "$@" | tee -a "$RES"; }
W=/root/cw-mmtoloop; rm -rf "$W"; mkdir -p "$W"; GEOM=/root/jass-geom32-loop
...
MOV_GZ=jobs/results/ccx33-0625-mmto-scan-teacher/artefacts/scan-teacher-moves.bin.gz
...
LEAFD=5; MAXPP=16; LAM=0.3; ANCHOR=0.05; WSOFF=-1000000000; NITERS=3; NOPEN=96; PAIRS=8
...
git show "origin/main:$PAR_GZ" | gunzip > "$W/parents.jnnw" || { say "ABORT parents"; exit 4; }
git show "origin/main:$MOV_GZ" | gunzip > "$W/moves.bin" || { say "ABORT moves"; exit 4; }
NPAR=$(python3 -c "import struct;print(struct.unpack('<I',open('$W/parents.jnnw','rb').read(8)[4:8])[0])")
say "  ✓ build + corpus parents=$NPAR (maîtres 0624 + prof Scan 0625) ; anchor=$ANCHOR"
...
say ""; say "  GATE : cand_final généraliste > +47 hors-IC => la boucle externe MULTIPLIE (gratuit, même corpus) => itérer + scaler."
