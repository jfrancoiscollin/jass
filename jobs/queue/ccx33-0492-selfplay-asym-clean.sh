#!/usr/bin/env bash
# id: ccx33-0492-selfplay-asym-clean
# description: BRAS ASYMETRIQUE "PUNISHER vs VICTIM" (forcing-ext SPEC §4) — VERSION PROPRE sur ccx33 (DB experte presente).
# Jumeau de 0489 (asym sur cpx62) mais sur la box qui POSSEDE expert_games.db (box-local ccx33) => vrais #2 ballots +
# #6 masters-naturels, PAS le fallback dilf+lidraughts qu'a subi 0489. Resultat : comparaison A EGALITE PARFAITE de
# recette/seeds/masters avec 0486 (OFF/OFF propre, meme box, seeds=BALLOTS, masters=OUI) => la SEULE difference est l'asym
...
# n'a pas. Label = RESULTAT REEL de la partie (rollout) ; NI distillation, NI Scan. Meme echelle que 0486/0489 (10M).
...
ART="/root/jass/jobs/results/ccx33-0492-selfplay-asym-clean/artefacts"; mkdir -p "$ART"
W=/root/cw-spasym-clean; mkdir -p "$W"
NCPU=$(nproc); export TMPDIR=/root/jass/.compile-tmp; mkdir -p "$TMPDIR"
GEOM32=/root/jass-geom32-spasym-clean
...
gzip -c "$W/champ.pjtw" > "$ART/champion-asym-clean.pjtw.gz"
...
say ""; say "================= VERDICT PROPRE (asym-clean 0492 vs 0486 OFF, A EGALITE DE SEEDS) ================="
SEEDKIND=$([ "$SEEDFILE" = "$W/ballots.jnnw" ] && echo BALLOTS || echo dilf+lidraughts)
{ echo "recette : gen ASYMETRIQUE (punisher ext_forcing=1 vs victim OFF) + quiet-only + seeds=$SEEDKIND + masters=$([ -n "$MASTERS" ] && echo OUI || echo non)"
  echo "0440 (asym-clean, 0492) = $M  IC95=[$LO,$HI]  (n=$N)   [base egdbmix = 0.302]"
  echo "vs_egdbmix (self-play) = $VB"
  if [ "$SEEDKIND" = BALLOTS ]; then
    echo "COMPARAISON A EGALITE : 0486 (OFF/OFF, meme box, BALLOTS+masters) = 0.282 [0.233,0.330] n=305. Seule diff = l'asym."
