#!/usr/bin/env bash
# id: cpx62-0724-doe-labels-gym-2x2
...
JOB_ID=cpx62-0724-doe-labels-gym-2x2
exec 9>/root/.jass-0724.lock
if ! flock -n 9; then echo "ABORT 0724 : instance deja active"; exit 0; fi
...
commit_to_main "$ART/corpus_manifest.json" "$ARTREL/corpus_manifest.json" "0724 DOE corpus manifest strict matched" >/dev/null 2>&1 || say "  ⚠ commit corpus manifest"
commit_to_main "$ART/doe_manifest.json" "$ARTREL/doe_manifest.json" "0724 DOE metrics and interaction manifest" >/dev/null 2>&1 || say "  ⚠ commit DOE manifest"
restore_src
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0724 FIN DOE 2x2 labels x gym G1/G$GYM_W" && say "  ✓ RESULTS committé" || say "  ⚠ commit RESULTS"
say "=== 0724 FINI ==="
rm -rf "$W" "$GEOM"
