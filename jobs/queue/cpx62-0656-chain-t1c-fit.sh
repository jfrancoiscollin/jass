#!/usr/bin/env bash
# id: cpx62-0656-chain-t1c-fit
# description: CHAÎNE ITÉRATIVE LONGUE (mémo JFC) — TOUR 1c, FIT. Consomme le self-play champion-driven 0655 (329k WDL +
# 95k prefs, distribution MOBILE). Pipeline : wdl_finetune ANCRÉ gen2-mmto anchor=0.3 (tight au tour 1) => wdlbase' ;
# MMTO last (gen-siblings --leaf-mode WS-OFF ancré wdlbase' + rank_finetune --leaf-pov --chunk anchor 0.05) => champion(1)
# gen3-chain-t1c ; A/B vs gen2-mmto (généraliste ≥38p, mt0.2+mt0.3). GATE compose-vs-sature : compose (+ hors-IC) => promu,
# tour suivant (anchor décroissant, self-play re-généré par champion(1)) ; neutre => vers §3 mémo. Outil validé 0647.
# WS-OFF obligatoire. AUCUN NNUE. gen2-mmto reste champion tant que le gate n'est pas passé.
set -uo pipefail
cd /root/jass
NCPU=$(nproc); export TMPDIR=/root/jass/.compile-tmp; mkdir -p "$TMPDIR"
ART="/root/jass/jobs/results/cpx62-0656-chain-t1c-fit/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/cpx62-0656-chain-t1c-fit/artefacts"
RES="$ART/RESULTS.txt"; : > "$RES"; say(){ echo "$@" | tee -a "$RES"; }
W=/root/cw-chain-t1c-fit; rm -rf "$W"; mkdir -p "$W"; GEOM=/root/jass-geom32-chaint1c
...
WDL_GZ=jobs/results/cpx62-0655-chain-t1c-selfplay/artefacts/chain-t1c-wdl.jnnw.gz
PP_GZ=jobs/results/cpx62-0655-chain-t1c-selfplay/artefacts/chain-t1c-prefs-parents.jnnw.gz
PM_GZ=jobs/results/cpx62-0655-chain-t1c-selfplay/artefacts/chain-t1c-prefs-moves.bin.gz
CORPUS_GZ=jobs/results/cpx62-0556-gen2M-mixdepth/artefacts/corpus-mix2M.jnnw.gz
WDLFT_ANCHOR=0.3; CHUNK_WDL=1000000; MAXIT_WDL=25
...
JB=$(( NCPU>4 ? NCPU : 4 )); cmake --build "$W/build" -j"$JB" --target jass >"$W/build.log" 2>&1 || { say "BUILD FAIL"; tail -12 "$W/build.log"|sed 's/^/  /'; restore_src; exit 6; }
J="$W/build/jass"
...
# ---- WDL corpus tour-1 (champion-driven, distribution mobile) ----
git show "origin/main:$WDL_GZ" | gunzip > "$W/wdl.jnnw" || { say "ABORT wdl chain-t1"; restore_src; exit 4; }
N_WDL=$(jnnw_count "$W/wdl.jnnw"); say "  ✓ build+geom(NP=$NP)+gen2 ; WDL tour-1 = $N_WDL"
[ "$N_WDL" -gt 100000 ] 2>/dev/null || { say "ABORT WDL < 100k ($N_WDL)"; restore_src; exit 7; }
...
commit_to_main "$ART/wdlbase-a$(echo "$WDLFT_ANCHOR"|tr '.' 'p').pjtw.gz" "$ARTREL/wdlbase-a$(echo "$WDLFT_ANCHOR"|tr '.' 'p').pjtw.gz" "0656 wdlbase ancré gen2 anchor=$WDLFT_ANCHOR" >/dev/null 2>&1 || true
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0656 progress : wdlbase fitté" >/dev/null 2>&1 || true

# ---- prefs tour-1 (côté-fort du champion) ----
git show "origin/main:$PP_GZ" | gunzip > "$W/parents.jnnw" || { say "ABORT prefs parents"; restore_src; exit 4; }
git show "origin/main:$PM_GZ" | gunzip > "$W/moves.bin" || { say "ABORT prefs moves"; restore_src; exit 4; }
python3 - "$W/parents.jnnw" "$W/moves.bin" <<'PY'
import struct,sys
pb=open(sys.argv[1],'rb').read(); n=struct.unpack('<I',pb[4:8])[0]; mb=open(sys.argv[2],'rb').read()
assert len(mb)==2*n, f"align: moves {len(mb)} != 2*{n}"; print(n)
PY
N_PAR=$(jnnw_count "$W/parents.jnnw"); say "  prefs tour-1 = $N_PAR parents"
...
gzip -c "$W/gen4.pjtw" > "$ART/gen3-chain-t1c.pjtw.gz"
commit_to_main "$ART/gen3-chain-t1c.pjtw.gz" "$ARTREL/gen3-chain-t1c.pjtw.gz" "0656 candidat gen3-chain-t1c (MMTO last ancré wdlbase')" >/dev/null 2>&1 || true
...
say ""; say "=== (3) A/B gen3-chain-t1c vs gen2-mmto (généraliste) ==="
...
open(outp,'w').write(f"  [gen3-chain-t1c vs gen2-mmto | generaliste mt{mt}] A={a} B={b} D={d} n={g} rate_A={r:.4f}+-{1.96*se:.4f} elo~{elo:+.0f} IC=[{lo:.3f},{hi:.3f}] => {vd}\n")
PY
  cat "$W/.about" | tee -a "$RES"; rm -f "${pref}".* ; }
abcell 0.2
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0656 A/B gen4 mt0.2" >/dev/null 2>&1 || true
abcell 0.3
restore_src
say ""; say "  GATE : gen3-chain-t1c >0.5 hors-IC => le pipeline MMTO-last sur base WDL-calibrée-ancrée AJOUTE sur gen2-mmto => confirm dilf + d9-vs-Scan + bake."
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0656 FIN : gen3-chain-t1c (MMTO last, anchor $WDLFT_ANCHOR) vs gen2-mmto" \
