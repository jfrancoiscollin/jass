#!/usr/bin/env bash
# id: ccx33-0486-selfplay-clean
# description: LA RECETTE SELF-PLAY PROPRE (directive JFC : "lancer proprement le self-play quand tout est implemente").
# Combine les leviers du briefing externe, tous codes+unit-testes ce cycle :
#   #4 --quiet-only au gen (CONFIRME jamais utilise avant ; defaut false) -> n'entraine plus sur des positions a capture
#      en attente (statiquement insensees).
#   #2 --seed-file = BALLOTS (build_ballots.py : ouvertures reelles diverses ply 6-12 + miroir couleur) au lieu de
#      random-open-plies (ouvertures equilibrees generiques).
#   #6 MIX masters-naturels (master_games_to_jnnw.py : positions quietes de vraies parties, label resultat reel, FREQUENCE
#      NATURELLE — PAS d'oversampling de racines comme 0464/0466/0468).
# Gen ext_forcing OFF (briefing §3 : une recherche plus propre au gen RETIRE le signal-shot des labels ; l'extension sert
# au JEU, pas au gen). Pilote = egdbmix. Fit logistic. Juge 0440 vs Scan (d11) + vs_egdbmix. Compare a la base 0.302.
# ROBUSTE : si expert_games.db absent (box-local), FALLBACK sur seeds dilf+lidraughts, mix masters saute. AUCUN NNUE,
# AUCUNE distillation Scan.
set -uo pipefail
cd /root/jass
...
ART="/root/jass/jobs/results/ccx33-0486-selfplay-clean/artefacts"; mkdir -p "$ART"
W=/root/cw-spclean; mkdir -p "$W"
NCPU=$(nproc); export TMPDIR=/root/jass/.compile-tmp; mkdir -p "$TMPDIR"
GEOM32=/root/jass-geom32-spclean
RES="$ART/RESULTS.txt"; CURVE="$ART/VERDICT.txt"; say(){ echo "$@" | tee -a "$RES"; }
[ -f "$RES" ] || : > "$RES"

preflight_build 1; preflight_train "$FRESH" 1
preflight_note "self-play propre : quiet-only + ballots + masters-naturels + fit + juges" 700
preflight_check
...
# ---- gen self-play QUIET-ONLY depuis ballots (ext_forcing OFF au gen) ----
say "=== gen self-play ${FRESH} (--quiet-only, --seed-file ballots frac ${SEED_FRAC}%, pilote egdbmix, ext_forcing OFF) ==="
...
say ""; say "================= VERDICT ================="
{ echo "recette : quiet-only + seeds=$([ "$SEEDFILE" = "$W/ballots.jnnw" ] && echo BALLOTS || echo dilf+lidraughts) + masters=$([ -n "$MASTERS" ] && echo OUI || echo non)"
  echo "0440 = $M  IC95=[$LO,$HI]  (n=$N)   [base egdbmix = 0.302]"
  echo "vs_egdbmix (self-play) = $VB"
  echo "LECTURE : 0440 hors-IC au-dessus de 0.302 => la recette propre deplace la conversion => industrialiser ;"
  echo "          vs_egdbmix>0.55 => casse le point fixe self-play. Plat => consigner (eval-lever) et prioriser la RECHERCHE (ext_forcing, 0485)."
} | tee "$CURVE" | tee -a "$RES"
say "==========================================="
