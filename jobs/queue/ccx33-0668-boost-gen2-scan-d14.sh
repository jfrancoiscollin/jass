#!/usr/bin/env bash
# id: ccx33-0668-boost-gen2-scan-d14
...
ART="/root/jass/jobs/results/ccx33-0668-boost-gen2-scan-d14/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/ccx33-0668-boost-gen2-scan-d14/artefacts"
...
cmake --build "$W/build" -j2 --target jass >"$W/build.log" 2>&1 || { say "BUILD FAIL"; tail -12 "$W/build.log"|sed 's/^/  /'|tee -a "$RES"; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0668 BUILD FAIL"; restore_src; exit 6; }
J="$W/build/jass"
...
monitor_loop(){ while [ ! -f "$W/.stopmon" ]; do sleep 600; [ -f "$W/.stopmon" ] && break
  ...
  commit_to_main "$PROG" "$ARTREL/PROGRESS.txt" "0668 gen +${el}min (${done_sh}/${NCPU}, ~${pp} parents)" >/dev/null 2>&1||true; done; }
...
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0668 generation Scan-d14 finie ($NPAR parents)" >/dev/null 2>&1||true
...
commit_to_main "$ART/cand-boost-d14.pjtw.gz" "$ARTREL/cand-boost-d14.pjtw.gz" "0668 candidat boost Scan-d14 WS-OFF" >/dev/null 2>&1||true
...
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0668 gate gen mt0.2" >/dev/null 2>&1||true
...
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0668 FIN boost Scan-d14 : le prof-fort ajoute-t-il sur gen2-mmto" \
