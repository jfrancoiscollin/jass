...
set -uo pipefail
cd /root/jass
ART="/root/jass/jobs/results/ccx33-0285-egdb-selfcheck/artefacts.src"; mkdir -p "$ART"
...
df -h / | sed 's/^/  before /'
if ls "$EXDIR"/app/db*.idx1 >/dev/null 2>&1; then
  echo "déjà extrait ($(ls "$EXDIR"/app/db*.idx1 2>/dev/null | wc -l) fichiers idx1) → skip extraction"
else
...
DBF=$(find "$EXDIR" -type f \( -iname 'db*.idx1' -o -iname 'db*.cpr1' -o -iname '*.idx*' -o -iname '*.cpr*' \) 2>/dev/null | head -1)
DBROOT=$(dirname "$DBF" 2>/dev/null); { [ -z "$DBROOT" ] || [ "$DBROOT" = "." ]; } && DBROOT="$EXDIR/app"
