#!/usr/bin/env bash
# id: ccx33-0637-reanchor-gen2mmto
# description: HYGIÈNE POST-BAKE — re-ancre la matrice vs Scan sur le NOUVEAU champion gen2-mmto (bake 2026-07-07). Mesure
# combien du résidu gen1 (d9 -310, mt0.3 -161, mt1.0 -133, NPS -134) le gain-éval MMTO a fermé. 4 cellules calibrate (d9,
# mt0.3, mt1.0, NPS-comp) + survie. Nouvelle référence pour toute la suite (prochaines rondes MMTO, couplage WDL). AUCUN NNUE.
set -uo pipefail
cd /root/jass
NCPU=$(nproc); export TMPDIR=/root/jass/.compile-tmp; mkdir -p "$TMPDIR"
ART="/root/jass/jobs/results/ccx33-0637-reanchor-gen2mmto/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/ccx33-0637-reanchor-gen2mmto/artefacts"
RES="$ART/RESULTS.txt"; : > "$RES"; say(){ echo "$@" | tee -a "$RES"; }
W=/root/cw-reanchor2; rm -rf "$W"; mkdir -p "$W"
GEN1_GZ=jobs/results/BAKE-gen2-mmto/artefacts/champion-gen2-mmto.pjtw.gz
...
J="$W/build/jass"
git show "origin/main:$GEN1_GZ" | gunzip > "$W/champ.pjtw" || { say "ABORT gen1"; exit 4; }
...
run_cell(){ local tag="$1" pairs="$2"; shift 2
  python3 tools/calibrate_vs_scan.py --jass "$J" --scan "$SCAN_BIN" --jass-pattern "$W/champ.pjtw" \
...
SURV_SH=8
for s in $(seq 0 $((SURV_SH-1))); do python3 "$W/surv.py" "$J" "$W/champ.pjtw" "$s" "$SURV_SH" "$W/surv.$s" "$W/sfens.tsv" >"$W/surv_$s.log" 2>&1 & done
wait
