...
ART="/root/jass/jobs/results/ccx33-0537-forcing-qs-ab/artefacts"; mkdir -p "$ART"
...
CFG={'qs0':'', 'qs2':'qs_forcing_depth=2', 'qs3':'qs_forcing_depth=3'}
...
hdr=f"  {'tempi':>5} {'n':>3} | mt3s: {'qs0':>4} {'qs2':>4} {'qs3':>4} | d{DFIX}: {'qs0':>4} {'qs2':>4} {'qs3':>4}"
lines=[hdr]; tot={k:[0,0] for k in CFG}
for t in sorted(byt):
    cb=[c for c in combos if c[1]==t]
    if not cb: continue
    r={k:rate(cb,v,movetime=MT) for k,v in CFG.items()}
    rd={k:rate(cb,v,depth=DFIX) for k,v in CFG.items()}
    lines.append(f"  {t:>5} {len(cb):>3} | mt3s: {f(r['qs0']):>4} {f(r['qs2']):>4} {f(r['qs3']):>4} | d{DFIX}: {f(rd['qs0']):>4} {f(rd['qs2']):>4} {f(rd['qs3']):>4}")
...
for cfg in "qs_forcing_depth=0:base" "qs_forcing_depth=2:qsF"; do
  spec="${cfg%%:*}"; lbl="${cfg##*:}"
