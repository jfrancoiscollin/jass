#!/usr/bin/env bash
# id: 0103-nnue-quantize-simd-bench
# description: Bench AVX2 SIMD pour ReLU+quantize int32→int8 dans NNUE eval.
#
# Audit 0103 : NNUE eval déjà SIMD-isée sauf 2 loops scalaires de
# quantization (acc1→h1 et acc2→h2). Pour 128-64 arch = 192 itérations
# scalaires par eval (~1μs sur ~3.3μs total eval = ~30% du eval bucket).
#
# SIMD AVX2 :
#  - Process 8 int32 lanes par iteration
#  - cvtepi32_ps + scale + max(0) + min(127) + add(0.5) + cvttps_epi32
#  - packs_epi32 + permute4x64 + packs_epi16 + storel_epi64
#  - ~10× speedup sur ces loops
#
# Gain attendu : eval bucket 20.1% → ~15% = +5% NPS.
#
# Smoke handcrafted : bit-identique (test_nnue exact-eval passes).
#
# Mesures :
#  1. Breakdown eval bucket (cible <16%)
#  2. NPS prod vs 0091 baseline 917K (gate >= 1.25M, gain attendu vs
#     baseline post-0101 = 1.19M donc +5% → 1.25M)
#  3. Bench vs Scan d10 (gate rate >= 0.85)
#
# expected_duration: ~10-15 min wall
set -uo pipefail
cd /root/jass

OUT_BASE="/root/jass/jobs/results/0103-nnue-quantize-simd-bench"
...
print()
print(f"  eval         : {tot['eval']:>6}ms ({100*tot['eval']/T:>5.1f}%)   [0102: 20.1%]")
...
# 2. NPS prod
echo
echo "=== 2) NPS PROD vs 0091 baseline + 0101 baseline ==="
...
print(f"quantize-simd    : median_depth={md}  median_nps={mn*2}")
print(f"baseline 0091    : median_depth=15  median_nps=917320  (pre capture pre-filter)")
print(f"baseline 0101    : median_depth=15  median_nps=1190216 (post capture pre-filter)")
gain_orig = (mn*2 - 917320) / 917320 * 100
gain_0101 = (mn*2 - 1190216) / 1190216 * 100
print(f"gain NPS vs 0091 : {gain_orig:+.1f}%")
print(f"gain NPS vs 0101 : {gain_0101:+.1f}%")
EOF
...
python3 tools/calibrate_vs_scan.py \
    ...
    --depth 10 --pairs 3 2>&1 | tee "$ART/bench-vs-scan-d10.log" | tail -15

echo
echo "=========================================================="
echo "       0103 NNUE QUANTIZE SIMD VERDICT"
echo "=========================================================="
RATE=$(grep -oE 'score rate: [0-9.]+' "$ART/bench-vs-scan-d10.log" | head -1 | awk '{print $3}')
GAIN_VS_0101=$(grep 'gain NPS vs 0101' "$ART/nps-prod.log" | awk '{print $NF}')
EVAL_PCT=$(grep '  eval' "$ART/breakdown.txt" | grep -oE '\( *[0-9.]+%' | head -1 | tr -d '( %')
echo "Eval pct (était 20.1% en 0102)   : $EVAL_PCT%"
echo "NPS gain vs 0101 (1.19M baseline) : $GAIN_VS_0101"
echo "Rate vs Scan d10                  : $RATE (baseline 0.870)"
echo
echo "Gates :"
echo "  Eval pct < 16%         : $(echo "$EVAL_PCT" | awk '{print ($1 < 16) ? "PASS" : "FAIL"}')"
echo "  NPS gain vs 0101 >= 3% : $(echo "$GAIN_VS_0101" | tr -d '%' | awk '{print ($1 >= 3) ? "PASS" : "FAIL"}')"
