#!/usr/bin/env bash
# id: cpx62-0234-rfe-prune24-v2
# description: RFE GÉOMÉTRIE — BRAS ÉLAGUÉ 24-pat (RELANCE, reset-proof). v1 (0232) a
# été cassé par le reset périodique du runner vers origin/main (32-pat) qui révertait le
# patterns.py 24-pat émis au démarrage → train.py repassait en 32-pat dès gen2 (binaire
# 24-pat ne chargeait plus l'eval). FIX : on épingle la géométrie 24-pat HORS du tree git
# (JASS_PATTERNS_DIR) avant de builder → insensible au reset. Réplique la boucle full-fold
# de 0227 (8 gens, depth4, 300k/gen) + vitesse (knps pattern/hc). Témoin = ccx33-0231 (32-pat).
set -uo pipefail
cd /root/jass
ART="/root/jass/jobs/results/cpx62-0234-rfe-prune24-v2/artefacts.src"; mkdir -p "$ART"
...
DROP=anti_0,anti_1,anti_2,diag_5,sq_0,sq_3,v_top_0,v_bot_0

# --- 1) emit the 24-pat geometry, IMMEDIATELY pin a copy OUTSIDE the git tree, build ---
python3 pattern_jass/tools/gen_patterns.py --variant v4 --drop "$DROP" --emit > "$ART/geom.log" 2>&1 || { echo GEOM FAIL; cat "$ART/geom.log"; exit 4; }
GEOMDIR=/root/active_geom_0234; mkdir -p "$GEOMDIR"
cp pattern_jass/tools/patterns.py "$GEOMDIR/patterns.py"          # reset-proof geometry for train.py
export JASS_PATTERNS_DIR="$GEOMDIR"
GNUM=$(python3 -c "import sys,os;sys.path.insert(0,os.environ['JASS_PATTERNS_DIR']);import patterns;print(patterns.NUM_PATTERNS)")
echo "geometry pinned: JASS_PATTERNS_DIR=$GEOMDIR  NUM_PATTERNS=$GNUM (expect 24)"
[ "$GNUM" = "24" ] || { echo "ABORT: pinned geometry not 24-pat"; exit 4; }
...
# sanity gate : si gen0 ne produit AUCUNE partie, binaire/géométrie désynchronisés -> stop net
case "${ELO[0]}" in 0-0-0*) echo "ABORT: gen0 0-0-0 → binaire pas 24-pat (reset pendant build ?)"; exit 8;; esac
...
echo; echo "=========================================================="
echo "   cpx62-0234 — RFE BRAS ÉLAGUÉ 24-PAT (reset-proof) — TRAJECTOIRE Elo + VITESSE"
for g in $(seq 0 "$NGEN"); do echo "  gen$g  Elo_vs_hc=${ELO[$g]}  proxy=${PRX[$g]}"; done
echo "  gen${NGEN} (60p) : $ELOF"
echo "  SPEED 24-pat : pattern/hc knps ratio=${RATIO}  depth@1s=${DP}"
echo "  → comparer à ccx33-0231 (32-pat témoin) : ΔElo = coût du prune, Δratio = gain vitesse"
echo "=========================================================="
