#!/usr/bin/env bash
# id: 0101-capture-prefilter-bench
# description: Bench capture pre-filter (enemy_reach mask) dans generate_captures.
#
# Verdict 0099/0100 : movegen_capture = 14.3% du total search. Le vrai
# hot path movegen.
#
# Optim :
#  - Calculer enemy_reach = union des squares 1-step adjacents à
#    n'importe quel ennemi (cost ~popcount(enemy)*4 ops)
#  - Iterer SEULEMENT sur friend_men & enemy_reach (skip les men sans
#    ennemi adjacent → extend_man_captures dead-end)
#  - Early-skip si threat_men == 0 && kings == 0 (no capture possible)
#  - Kings unchanged (long-range, pre-filter trop cher)
#
# Smoke handcrafted bit-identique main (432K nodes / depth=13 / même
# PV / score=2) → sémantique préservée.
#
# Mesures :
#  1. Breakdown movegen_capture (était 14.3% en 0099)
#  2. NPS prod vs 0091 baseline (gate >= +3%)
...
OUT_BASE="/root/jass/jobs/results/0101-capture-prefilter-bench"
...
echo "=== host ==="; echo "host: $(hostname)  nproc: $(nproc)"
...
print(f"capture-prefilter : median_depth={md}  median_nps={mn*2}")
...
echo
echo "=========================================================="
echo "       0101 CAPTURE PRE-FILTER VERDICT"
echo "=========================================================="
...
CAP_PCT=$(grep 'capture' "$ART/breakdown.txt" | grep -oE '\( *[0-9.]+%' | head -1 | tr -d '( %')
echo "Capture pct (était 14.3%) : $CAP_PCT%"
echo "NPS gain vs 0091          : $GAIN_NPS"
echo "Rate vs Scan d10          : $RATE (baseline 0.870)"
echo
echo "Gates :"
echo "  Capture pct < 11%       : $(echo "$CAP_PCT" | awk '{print ($1 < 11) ? "PASS" : "FAIL"}')"
echo "  NPS gain >= +3%         : $(echo "$GAIN_NPS" | tr -d '%' | awk '{print ($1 >= 3) ? "PASS" : "FAIL"}')"
