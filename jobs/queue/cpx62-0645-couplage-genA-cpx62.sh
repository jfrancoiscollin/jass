#!/usr/bin/env bash
# id: cpx62-0645-couplage-genA-cpx62
# description: COUPLAGE Phase-A — GENERATION part cpx62 (⅔ du volume). Une passe self-play Scan = 2 objectifs (WDL + prefs
# MMTO). 2 flottes : BAL équilibré fort-vs-fort d8/d9/d10 (prefs OFF, WDL calibré) + ASYM mt0.3/mt0.03 (WDL décisif + prefs
# côté-fort). Sharding GLOBAL disjoint (nshards=48, cpx62 BAL=0..15 ASYM=16..31 ; ccx33 job = 32..47) => aucune ouverture
...
ART="/root/jass/jobs/results/cpx62-0645-couplage-genA-cpx62/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/cpx62-0645-couplage-genA-cpx62/artefacts"
RES="$ART/RESULTS.txt"; : > "$RES"; say(){ echo "$@" | tee -a "$RES"; }
W=/root/cw-cplA-cpx62; rm -rf "$W"; mkdir -p "$W"; GEOM=/root/jass-geom32-cplA
...
# --- Phase-A sizing (cpx62 = ⅔) : 16 shards/flotte × 490 = 7840 BAL + 7840 ASYM = 15680 parties ≈ 2.0M positions ---
NSH=16; GLOBAL_NSHARDS=48; BAL_BASE=0; ASYM_BASE=16; SEED=50645
...
say "=== COUPLAGE Phase-A GEN cpx62 (⅔) — HEAD $(git log --oneline -1|cat) ==="
...
open(outp,'wb').write(b'JNNW'+struct.pack('<I',tot)+bytes(body)); print(tot)
PY
N_WDL=$(jnnw_count "$W/wdl.jnnw"); say "  WDL corpus cpx62 (BAL+ASYM) = $N_WDL positions"
...
print(tot)
PY
N_PAR=$(jnnw_count "$W/parents.jnnw"); say "  prefs ASYM parents cpx62 = $N_PAR"
[ "$N_WDL" -gt 100000 ] 2>/dev/null || { say "⚠ WDL corpus faible ($N_WDL)"; }

gzip -c "$W/wdl.jnnw"     > "$ART/wdl-cpx62.jnnw.gz"
gzip -c "$W/parents.jnnw" > "$ART/prefs-parents-cpx62.jnnw.gz"
gzip -c "$W/moves.bin"    > "$ART/prefs-moves-cpx62.bin.gz"
commit_to_main "$ART/wdl-cpx62.jnnw.gz" "$ARTREL/wdl-cpx62.jnnw.gz" "0645 WDL corpus cpx62 ($N_WDL)" >/dev/null 2>&1 || true
commit_to_main "$ART/prefs-parents-cpx62.jnnw.gz" "$ARTREL/prefs-parents-cpx62.jnnw.gz" "0645 prefs parents cpx62 ($N_PAR)" >/dev/null 2>&1 || true
commit_to_main "$ART/prefs-moves-cpx62.bin.gz" "$ARTREL/prefs-moves-cpx62.bin.gz" "0645 prefs moves cpx62" >/dev/null 2>&1 || true

say ""; say "  => cpx62 gen fait : WDL=$N_WDL prefs=$N_PAR committés. Attend ccx33 (0645-ccx33) puis fit/A-B."
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0645 gen cpx62 FINI : WDL=$N_WDL prefs=$N_PAR" && say "  RESULTS committé ✓" || say "  ⚠ commit échoue"
say "=== fin gen cpx62 ==="
