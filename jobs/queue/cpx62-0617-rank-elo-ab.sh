#!/usr/bin/env bash
# id: cpx62-0617-rank-elo-ab
# description: PISTE (a) — JUGE DE PAIX FINAL (Elo). G1 survie a été PLATE sur les 2 bras (S 0.34->0.26, M 0.34≈champion)
# ALORS QUE le fit APPREND fort (pairwise-acc 0.50->0.62/0.68 sur paires maîtres). Hypothèse : la SURVIE (accord d1↔d11)
# mesure la depth-stability, PAS la qualité de jeu — une éval fine-tunée peut mieux jouer tout en étant moins depth-stable.
# Le VRAI juge = l'Elo. Ce job re-fit bras M (corpus maîtres 0615, oracle externe non-circulaire) à anchor {0.01,0.1,1.0},
# puis A/B Elo DIRECT : pattern-a=candidat vs pattern-b=gen1, MÊME binaire + MÊMES search-params bakés (seule l'éval diffère).
# GATE : un candidat bat gen1 hors-IC (dilf ET/OU gen, mt0.2) => la survie était le mauvais proxy, le rank-loss MARCHE => scale.
# Tous neutres/négatifs => CLAUSE D'ÉCHEC CONFIRMÉE PAR LA VRAIE MÉTRIQUE => on ferme la piste rank-loss proprement
# (le programme repose sur +187 Elo search bakés ; résidu -133/-161 acté = prix des marges). anchor=1.0 ≈ champion = contrôle
# de sanité du harness (doit sortir ~0.500). AUCUN NNUE.
set -uo pipefail
cd /root/jass
NCPU=$(nproc); export TMPDIR=/root/jass/.compile-tmp; mkdir -p "$TMPDIR"
ART="/root/jass/jobs/results/cpx62-0617-rank-elo-ab/artefacts"; mkdir -p "$ART"
ARTREL="jobs/results/cpx62-0617-rank-elo-ab/artefacts"
RES="$ART/RESULTS.txt"; : > "$RES"; say(){ echo "$@" | tee -a "$RES"; }
W=/root/cw-eloab; rm -rf "$W"; mkdir -p "$W"; GEOM=/root/jass-geom32-eloab
...
say "=== PISTE (a) A/B ELO — juge de paix final — HEAD main $(git log --oneline -1|cat) ==="
# binaire = champion (main brut, ordering baké) ; rank_finetune.py depuis develop (non promu)
...
say "  gen1 + master-pairs($NPAIRS) + corpus prets ; NUM_PATTERNS=$NP"
...
# ---- FIT rank-loss bras M à chaque anchor (candidats = champion sauf buckets >=min-pairs) ----
OKA=""
for A in $ANCHORS; do
  say ""; say "=== rank_finetune bras M anchor=$A (min-pairs=5, lam=$LAM) ==="
...
[ -n "$OKA" ] || { say "  => tous les fits ont avorté"; commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0617 A/B Elo : tous fits avortés"; exit 0; }

# ---- openings : DILF (continuité) + GENERALISTE (>=38 pièces, hors-dilf, du corpus) ----
...
# ---- A/B Elo : pattern-a=candidat vs pattern-b=gen1 (même binaire, mêmes search-params bakés) ----
ab(){ local cand="$1" tag="$2" set="$3" openf="$4"
  for s in $(seq 0 $((NCPU-1))); do python3 tools/jass_vs_jass_arch.py \
    ...
    --shard "$s" --nshards "$NCPU" --quiet --openings-file "$openf" >"$W/x_${tag}_${set}.$s" 2>&1 & done; wait
  python3 - "$tag" "$set" "$W"/x_${tag}_${set}.* <<'PY' 2>&1 | tee -a "$RES"
import sys,math; tag,st=sys.argv[1],sys.argv[2]; a=d=b=0
for f in sys.argv[3:]:
  try:
    for l in open(f):
...
print(f"  [{tag} {st}] A(cand)={a} B(gen1)={b} D={d} n={g} rate_A={r:.4f}+-{1.96*se:.4f} elo~{elo:+.0f} IC=[{lo:.3f},{hi:.3f}] => {vd}")
PY
  rm -f "$W"/x_${tag}_${set}.* ; }

say ""; say "=== A/B ELO candidat vs gen1 (mt$MT, dilf + generaliste) ==="
for A in $OKA; do
  say "  -- anchor $A --"
  ab "$W/cand_$A.pjtw" "cand_$A" dilf "$W/dilf.fen"
  ab "$W/cand_$A.pjtw" "cand_$A" gen  "$W/gen.fen"
done
say ""
say "  GATE : un candidat rate_A>0.5 hors-IC => la SURVIE était le mauvais proxy, le rank-loss MARCHE => scale (G2/G3/G4)."
say "  tous neutres/négatifs => clause d'échec CONFIRMÉE par la vraie métrique (Elo) => piste rank-loss CLOSE."
say "  (anchor=1.0 ≈ champion : contrôle de sanité, doit sortir ~0.500)"
commit_to_main "$RES" "$ARTREL/RESULTS.txt" "0617 A/B Elo bras M vs gen1 (juge de paix final rank-loss : survie mauvais proxy ou piste close)" \
