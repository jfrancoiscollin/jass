#!/usr/bin/env bash
# id: cpx62-0352-residual-analysis
# description: ÉTAPE 3 — analyse des RÉSIDUS finale. Sur un set fort, on aligne éval-jass et éval-Scan (d9) et on
# DÉCOMPOSE le gap finale : (a) combien le drawish (÷8/÷2, appliqué en post sur les scores jass) RÉDUIT le résidu
...
ART="/root/jass/jobs/results/cpx62-0352-residual-analysis/artefacts.src"; mkdir -p "$ART"
...
preflight_build 1; preflight_train 240000 1; preflight_note "relabel Scan d9 + analyse résidus" 70; preflight_check
...
PY
echo "=== relabel Scan d9 + éval-jass alignée (rewrite-scores SUR le relabel) ==="
relabel_scan_sharded "$ART/test.jnnw" "$ART/scan.jnnw" "$SCAN_BIN" 9 "$NCPU"
