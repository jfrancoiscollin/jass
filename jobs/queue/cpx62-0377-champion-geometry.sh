#!/usr/bin/env bash
# id: cpx62-0377-champion-geometry
# (relance de 0376 : le fit sur 3,456M a OOM/disque → on SOUS-ÉCHANTILLONNE le pool à 2M, taille prouvée OK en 0224)
...
ART="/root/jass/jobs/results/cpx62-0377-champion-geometry/artefacts"; mkdir -p "$ART"
...
PY
echo "=== sous-échantillonne le pool à 2M (mémoire : 3,456M OOM en 0376) ==="
python3 - "$ART/pooled.jnnw" 2000000 <<'SUB'
...
if n>cap:
    ...
    open(p,'wb').write(b'JNNW'+struct.pack('<I',cap)+bytes(out)); print(f"  subsample {n} -> {cap}")
else: print(f"  pool {n} <= {cap}")
SUB
