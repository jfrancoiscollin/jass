#!/usr/bin/env python3
"""Recover bounded Launch-V2 stage logs for failed CLS-L preflight 2031.

The first bounded diagnostic proved that 2031 failed before the frozen shell
created any ``work/*`` logs. ``run_experiment_stage.py`` writes the launched
command's stdout/stderr and stage receipt at the result root, so this successor
reads only those exact small textual runner-owned files plus the immutable
failure envelope. It never reads CURRENT_2M/CURRICULUM payloads and performs no
fit, search, game, target read, alpha spend, promotion or bake.
"""
...
FAILED_JOB = base.FAILED_JOB
FAILED_ATTEMPT = base.FAILED_ATTEMPT
FAILED_CODE = base.FAILED_CODE
FAILED_PREFIX = base.FAILED_PREFIX
TERMINAL = "CLS_L_2031_STAGE_LOG_DIAGNOSTIC_COMPLETE_V1"
PHASE = "execute-cls-l-2031-stage-log-diagnostic"
...
def main() -> int:
    ...
    work = result_dir / "work" / "failed-2031-stage-logs"
    ...
    if (
        ...
        raise RuntimeError("failed-2031 identity/state drift")
    ...
    if attempt.get("classification") != "TECHNICAL" or attempt.get("scientific_verdict") is not None:
        raise RuntimeError("2031 was not a quarantined technical failure")
    ...
    diagnostic = {
        "schema": "jass.cls_l_2031_stage_log_diagnostic.v1",
        "terminal": TERMINAL,
        "state": "completed",
    ...
    write_json(
        ...
        {
            "schema": "jass.cls_l_2031_stage_log_authentication.v1",
            "authenticated": True,
            "job_id": FAILED_JOB,
            "attempt_id": FAILED_ATTEMPT,
            "code_sha": FAILED_CODE,
    ...
    summary = {
        "schema": "jass.cls_l_2031_stage_log_diagnostic_summary.v1",
        "terminal": TERMINAL,
        "state": "completed",
        "classification": "TECHNICAL_DIAGNOSTIC",
        "scientific_verdict": None,
        ...
        "promotions": 0,
        "bakes": 0,
        "next_stage": "REPAIR_PROVEN_2031_MECHANICS_ONLY" if primary else "RECOVER_PRE_STAGE_LAUNCHER_EVIDENCE",
    }
    runtime_atomic_json(art / "scientific-summary.json", summary)
    write_json(
        art / "manifest.json",
        {
            "schema": "jass.cls_l_2031_stage_log_diagnostic_manifest.v1",
            "terminal": TERMINAL,
            "failed_job_id": FAILED_JOB,
            "failed_attempt_id": FAILED_ATTEMPT,
    ...
    (art / "RESULTS.md").write_text(
        f"# CLS-L 2031 stage-log diagnostic\n\n"
        f"- terminal: `{TERMINAL}`\n"
