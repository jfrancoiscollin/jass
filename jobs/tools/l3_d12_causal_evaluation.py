#!/usr/bin/env python3
"""Aggregate the preregistered L3-PURE D12-vs-D10 causal evaluation."""
...
PROMOTION_SCALE = "D12_PROMOTION_AND_SCALE_REVIEW_READY"
DEPTH_CONFIRMED = "D12_DEPTH_EFFECT_CONFIRMED_REVIEW"
DIRECTIONAL = "D12_DIRECTIONAL_CONFIRMATION_REVIEW"
PLATEAU = "D12_PLATEAU_OR_REGRESSION_REVIEW"
...
def build_evaluation(
    *,
    ...
    d10_evaluation = load(d10_evaluation_path)
    ...
    if (
        ...
        or training.get("experiment_variant") != "D12_CAUSAL_FRESH2M"
        or training.get("play_depth") != 12
    ):
        raise ValueError("D12 training contract mismatch")
    if (
        d10_training.get("verdict") != "M2_TRAINING_SCREEN_READY"
        or d10_training.get("parent") != "F2M"
        or d10_training.get("fresh_only") is not True
    ...
    if (
        d10_evaluation.get("verdict") != "D10_PLATEAU_OR_REGRESSION_REVIEW"
        or not d10_evaluation.get("all_guardrails_pass")
        or d10_evaluation.get("training_summary", {}).get("model_sha256")
        != d10_training.get("model_sha256")
    ):
        raise ValueError("D10 plateau certificate mismatch")
    ...
    if (
        ...
        raise ValueError("D12 independent opening-pool contract mismatch")

    force: dict[str, dict[str, Any]] = {}
    for view in ("q00", "native"):
        for opponent in ("D10", "F2M", "GEN2"):
            force[f"{view}_vs_{opponent}"] = force_row(
                force_dir / f"force-{view}-D12-vs-{opponent}.json"
            )
    ...
    for index, stratum in enumerate(("p3_mince", "p4_egal")):
        ...
        conversion[stratum]["paired_delta_d12_minus_d10"] = comparison
    ...
    primary = {
        view: {
            "positive_point_estimate": force[f"{view}_vs_D10"]["rate"] > 0.5,
            "superiority_established": force[f"{view}_vs_D10"]["ci_low"] > 0.5,
            "regression_not_established": force[f"{view}_vs_D10"]["ci_high"] >= 0.5,
        }
        for view in ("q00", "native")
    }
    versus_f2m = {
    ...
    d10_force = d10_evaluation["force"]
    gen2_delta = {
        view: independent_delta(
            force[f"{view}_vs_GEN2"], d10_force[f"{view}_vs_GEN2"]
        )
        for view in ("q00", "native")
    }
    guardrails = {
        "d10_q00_regression_not_established": primary["q00"][
            "regression_not_established"
        ],
        "d10_native_regression_not_established": primary["native"][
            "regression_not_established"
        ],
        ...
        "p3_absolute_conversion_floor": (
            float(conversion["p3_mince"]["D12"]["conversion"]) >= 0.95
        ),
        "p4_absolute_conversion_floor": (
            float(conversion["p4_egal"]["D12"]["conversion"]) >= 0.95
        ),
        "p3_regression_over_3pp_not_established": (
            conversion["p3_mince"]["paired_delta_d12_minus_d10"]["ci_high"] >= -0.03
        ),
        "p4_regression_over_3pp_not_established": (
            conversion["p4_egal"]["paired_delta_d12_minus_d10"]["ci_high"] >= -0.03
        ),
        "visited_coverage_no_5pct_collapse_vs_d10": (
            coverage["D12"]["visited_buckets"]
            >= 0.95 * coverage["D10"]["visited_buckets"]
        ),
        "ge100_coverage_no_5pct_collapse_vs_d10": (
            coverage["D12"]["ge_100"] >= 0.95 * coverage["D10"]["ge_100"]
        ),
    }
    ...
    if depth_superior and f2m_superior and all_guardrails:
        ...
        recommendation = "human_review_d12_promotion_and_scale"
    elif depth_superior and all_guardrails:
        verdict = DEPTH_CONFIRMED
        recommendation = "confirm_d12_against_f2m_then_scale"
    elif depth_directional and all_guardrails:
        verdict = DIRECTIONAL
        recommendation = "independent_d12_confirmation"
    else:
        verdict = PLATEAU
        recommendation = "stop_single_depth_escalation_and_prepare_distribution_factor"
    ...
    return {
        ...
        "protocol": {
            ...
            "changed_training_factor": "play_depth_10_to_12",
        },
        "force": force,
        "conversion": conversion,
        "coverage": coverage,
        "coverage_delta_d12_minus_d10": {
            "visited_buckets": (
                coverage["D12"]["visited_buckets"] - coverage["D10"]["visited_buckets"]
            ),
            "ge_100": coverage["D12"]["ge_100"] - coverage["D10"]["ge_100"],
        },
        "d12_minus_d10_gen2_independent_delta": gen2_delta,
        ...
        "d10_plateau_certificate": d10_evaluation,
...
def main() -> int:
    ...
    parser.add_argument("--d10-evaluation", required=True, type=Path)
    ...
    parser.add_argument("--seed", type=int, default=973_001)
    ...
    payload = build_evaluation(
        ...
        d10_evaluation_path=args.d10_evaluation,
