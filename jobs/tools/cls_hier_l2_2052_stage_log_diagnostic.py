#!/usr/bin/env python3
"""Recover bounded Launch-V2 stage logs for failed CLS HIER-L2 CONTROL rehearsal 2052.

This stage is technical-diagnostic only. It authenticates the immutable failed
2052 runner publication and reads only runner-owned result-root stage stdout,
...
TERMINAL = "CLS_HIER_L2_2052_STAGE_LOG_DIAGNOSTIC_COMPLETE_V1"
PHASE = "execute-cls-hier-l2-2052-stage-log-diagnostic"
...
def main() -> int:
    ...
    work = result_dir / "work" / "failed-2052-stage-logs"
    ...
    if (
        ...
        raise RuntimeError("failed-2052 identity/state drift")
    ...
    if attempt.get("classification") != "TECHNICAL" or attempt.get("scientific_verdict") is not None:
        raise RuntimeError("2052 was not a quarantined technical failure")
    ...
    diagnostic = {
        "schema": "jass.cls_hier_l2_2052_stage_log_diagnostic.v1",
        "terminal": TERMINAL,
        "state": "completed",
    ...
    write_json(
        ...
        {
            "schema": "jass.cls_hier_l2_2052_stage_log_authentication.v1",
            "authenticated": True,
            "job_id": FAILED_JOB,
            "attempt_id": FAILED_ATTEMPT,
            "code_sha": FAILED_CODE,
    ...
    summary = {
        "schema": "jass.cls_hier_l2_2052_stage_log_diagnostic_summary.v1",
        "terminal": TERMINAL,
        "state": "completed",
        "classification": "TECHNICAL_DIAGNOSTIC",
        "scientific_verdict": None,
        ...
        "bakes": 0,
        "next_stage": (
            "REPAIR_PROVEN_2052_MECHANICS_ONLY"
    ...
    write_json(
        art / "manifest.json",
        {
            "schema": "jass.cls_hier_l2_2052_stage_log_diagnostic_manifest.v1",
            "terminal": TERMINAL,
            "failed_job_id": FAILED_JOB,
            "failed_attempt_id": FAILED_ATTEMPT,
    ...
    (art / "RESULTS.md").write_text(
        f"# CLS HIER-L2 2052 stage-log diagnostic\n\n"
        f"- terminal: `{TERMINAL}`\n"
