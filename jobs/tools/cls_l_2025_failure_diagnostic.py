#!/usr/bin/env python3
"""Bounded technical diagnostic for failed CLS-L normalization preflight 2025.

Reads only the immutable failed runner-v3 envelope plus an explicit allowlist of
small textual stage work logs. It never reads CURRENT_2M/CURRICULUM training
payloads, performs no fit/search/game, and publishes no scientific conclusion.
"""
...
FAILED_JOB = "cpx62-2025-l3-cls-l-source-normalization-preflight-v3"
FAILED_ATTEMPT = "20260917T072337Z-16e199f6"
FAILED_CODE = "16e199f661d266146154507c4d835883039e2488"
FAILED_PREFIX = f"r2:jass-data/runs/{FAILED_JOB}/{FAILED_ATTEMPT}"
TERMINAL = "CLS_L_2025_TECHNICAL_DIAGNOSTIC_COMPLETE_V1"
PHASE = "execute-cls-l-2025-failure-diagnostic"
ERROR_RE = re.compile(
...
def main() -> int:
    ...
    work = result_dir / "work" / "failed-2025"
    ...
    if (
        ...
        raise RuntimeError("failed-2025 identity/state drift")
    ...
    if attempt.get("classification") != "TECHNICAL" or attempt.get("scientific_verdict") is not None:
        raise RuntimeError("2025 was not a quarantined technical failure")

    parsed = bounded_lines(read_gzip(work / "output.log.gz"))
    work_logs = summarize_work_logs(work, work_log_selections)
    diagnostic = {
        "schema": "jass.cls_l_2025_failure_diagnostic.v1",
        "terminal": TERMINAL,
        "state": "completed",
    ...
    atomic_json(art / "source-authentication.json", {
        "schema": "jass.cls_l_2025_failed_source_authentication.v1",
        "authenticated": True,
        "job_id": FAILED_JOB,
        "attempt_id": FAILED_ATTEMPT,
        "code_sha": FAILED_CODE,
    ...
    summary = {
        "schema": "jass.cls_l_2025_failure_diagnostic_summary.v1",
        "terminal": TERMINAL,
        "state": "completed",
        "classification": "TECHNICAL_DIAGNOSTIC",
        "scientific_verdict": None,
        ...
        "promotions": 0,
        "bakes": 0,
        "next_stage": "REPAIR_PROVEN_2025_MECHANICS_ONLY",
    }
    runtime_atomic_json(art / "scientific-summary.json", summary)
    atomic_json(art / "manifest.json", {
        "schema": "jass.cls_l_2025_failure_diagnostic_manifest.v1",
        "terminal": TERMINAL,
        "failed_job_id": FAILED_JOB,
        "failed_attempt_id": FAILED_ATTEMPT,
    ...
    (art / "RESULTS.md").write_text(
        "# CLS-L 2025 technical failure diagnostic\n\n"
        f"- terminal: `{TERMINAL}`\n"
