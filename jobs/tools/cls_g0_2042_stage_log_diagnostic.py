#!/usr/bin/env python3
"""Recover bounded Launch-V2 stage logs for failed CLS-G0 LOCAL rehearsal 2042.

This stage is technical-diagnostic only.  It authenticates the immutable failed
2042 runner publication and reads only runner-owned result-root stage stdout,
...
FAILED_JOB = "cpx62-2042-l3-cls-g0-local-runtime-rehearsal-v1"
FAILED_ATTEMPT = "20260918T113159Z-db842be0"
FAILED_CODE = "db842be0939954a59076cd2b1d8a05041a3ba661"
FAILED_PREFIX = f"r2:jass-data/runs/{FAILED_JOB}/{FAILED_ATTEMPT}"
TERMINAL = "CLS_G0_2042_STAGE_LOG_DIAGNOSTIC_COMPLETE_V1"
PHASE = "execute-cls-g0-2042-stage-log-diagnostic"
...
def main() -> int:
    ...
    work = result_dir / "work" / "failed-2042-stage-logs"
    ...
    if (
        ...
        raise RuntimeError("failed-2042 identity/state drift")
    ...
    if attempt.get("classification") != "TECHNICAL" or attempt.get("scientific_verdict") is not None:
        raise RuntimeError("2042 was not a quarantined technical failure")
    ...
    diagnostic = {
        "schema": "jass.cls_g0_2042_stage_log_diagnostic.v1",
        "terminal": TERMINAL,
        "state": "completed",
    ...
    write_json(
        ...
        {
            "schema": "jass.cls_g0_2042_stage_log_authentication.v1",
            "authenticated": True,
            "job_id": FAILED_JOB,
            "attempt_id": FAILED_ATTEMPT,
            "code_sha": FAILED_CODE,
    ...
    summary = {
        "schema": "jass.cls_g0_2042_stage_log_diagnostic_summary.v1",
        "terminal": TERMINAL,
        "state": "completed",
        "classification": "TECHNICAL_DIAGNOSTIC",
        "scientific_verdict": None,
        ...
        "bakes": 0,
        "next_stage": (
            "REPAIR_PROVEN_2042_MECHANICS_ONLY"
    ...
    write_json(
        art / "manifest.json",
        {
            "schema": "jass.cls_g0_2042_stage_log_diagnostic_manifest.v1",
            "terminal": TERMINAL,
            "failed_job_id": FAILED_JOB,
            "failed_attempt_id": FAILED_ATTEMPT,
    ...
    (art / "RESULTS.md").write_text(
        f"# CLS-G0 2042 stage-log diagnostic\n\n"
        f"- terminal: `{TERMINAL}`\n"
