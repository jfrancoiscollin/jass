#!/usr/bin/env python3
"""Launch-V2 wrapper for terminal CLS-D bottleneck classification."""
...
from jobs.tools import cls_bottleneck_classification_stage as stage  # noqa: E402
from jobs.tools.launch_runtime_v2 import StageEvidence, atomic_json  # noqa: E402

PHASES = ["execute-cls-bottleneck-classification"]
...
def main() -> int:
    ...
    try:
        ...
        stage.run_stage(result / "cls-bottleneck-work", art)
        evidence.value["actual_side_effects"].update({
            "fits": 0, "new_scan_searches": 0, "new_jass_searches": 0,
            "strength_games": 0, "selfplay_games": 0, "promotions": 0,
            "bakes": 0, "test_target_reads": 0,
        })
        evidence.complete()
        evidence.finish()
        return 0
        ...
        atomic_json(art / "scientific-summary.json", {
            "schema": stage.SCHEMA, "state": "failed", "terminal": stage.TECHNICAL_TERMINAL,
