#!/usr/bin/env python3
"""Recover bounded Launch-V2 stage logs for failed CLS HIER-L2 CONTROL rehearsal 2050.

This stage is technical-diagnostic only. It authenticates the immutable failed
2050 runner publication and reads only runner-owned result-root stage stdout,
stderr and receipt files under a strict size bound, plus the immutable failure
envelope. It never reads CURRENT_2M / CONTEXT_30 / CURRICULUM scientific
payloads, fitted model bytes, fit logs, optimizer payloads, targets, search/game
payloads, or any promotion/bake state.
"""
...
FAILED_JOB = "cpx62-2050-l3-cls-hier-l2-control-reproduction-rehearsal-v1"
FAILED_ATTEMPT = "20260918T201651Z-1c8de5ca"
FAILED_CODE = "1c8de5cae04f92e53cb15f0de394be2e2bdb3085"
FAILED_PREFIX = f"r2:jass-data/runs/{FAILED_JOB}/{FAILED_ATTEMPT}"
TERMINAL = "CLS_HIER_L2_2050_STAGE_LOG_DIAGNOSTIC_COMPLETE_V1"
PHASE = "execute-cls-hier-l2-2050-stage-log-diagnostic"
...
def main() -> int:
    ...
    work = result_dir / "work" / "failed-2050-stage-logs"
    ...
    if (
        ...
        raise RuntimeError("failed-2050 identity/state drift")
    ...
    if attempt.get("classification") != "TECHNICAL" or attempt.get("scientific_verdict") is not None:
        raise RuntimeError("2050 was not a quarantined technical failure")
    ...
    diagnostic = {
        "schema": "jass.cls_hier_l2_2050_stage_log_diagnostic.v1",
        "terminal": TERMINAL,
        "state": "completed",
    ...
    write_json(
        ...
        {
            "schema": "jass.cls_hier_l2_2050_stage_log_authentication.v1",
            "authenticated": True,
            "job_id": FAILED_JOB,
            "attempt_id": FAILED_ATTEMPT,
            "code_sha": FAILED_CODE,
    ...
    summary = {
        "schema": "jass.cls_hier_l2_2050_stage_log_diagnostic_summary.v1",
        "terminal": TERMINAL,
        "state": "completed",
        "classification": "TECHNICAL_DIAGNOSTIC",
        "scientific_verdict": None,
        ...
        "bakes": 0,
        "next_stage": (
            "REPAIR_PROVEN_2050_MECHANICS_ONLY"
            if primary else "RECOVER_INNER_HIER_L2_FIT_EVIDENCE"
        ),
    }
    ...
    write_json(
        art / "manifest.json",
        {
            "schema": "jass.cls_hier_l2_2050_stage_log_diagnostic_manifest.v1",
            "terminal": TERMINAL,
            "failed_job_id": FAILED_JOB,
            "failed_attempt_id": FAILED_ATTEMPT,
    ...
    (art / "RESULTS.md").write_text(
        f"# CLS HIER-L2 2050 stage-log diagnostic\n\n"
        f"- terminal: `{TERMINAL}`\n"
        f"- failed attempt: `{FAILED_JOB}` / `{FAILED_ATTEMPT}`\n"
        f"- candidate arm: `CONTROL`\n"
        f"- selected stage logs: `{[remote for remote, _ in selections]}`\n"
        f"- primary mechanical error: `{primary}`\n"
        "- CURRENT_2M/CONTEXT_30/CURRICULUM/model/fit-log reads by this diagnostic: 0\n"
