#!/usr/bin/env python3
"""Bounded technical diagnostic for failed CLS-L normalization preflight 2027.
...
FAILED_JOB = "cpx62-2027-l3-cls-l-source-normalization-preflight-v4"
FAILED_ATTEMPT = "20260917T085939Z-cf8c63d8"
FAILED_CODE = "cf8c63d80e45794a7e60f0f1b6245b03ee7af5ea"
FAILED_PREFIX = f"r2:jass-data/runs/{FAILED_JOB}/{FAILED_ATTEMPT}"
TERMINAL = "CLS_L_2027_TECHNICAL_DIAGNOSTIC_COMPLETE_V1"
PHASE = "execute-cls-l-2027-failure-diagnostic"
ERROR_RE = re.compile(
...
def main() -> int:
    ...
    work = result_dir / "work" / "failed-2027"
    ...
    if (verified.get("job_id") != FAILED_JOB or verified.get("attempt_id") != FAILED_ATTEMPT
        ...
        raise RuntimeError("failed-2027 identity/state drift")
    ...
    if attempt.get("classification") != "TECHNICAL" or attempt.get("scientific_verdict") is not None:
        raise RuntimeError("2027 was not a quarantined technical failure")
    ...
    diagnostic = {
        "schema": "jass.cls_l_2027_failure_diagnostic.v1", "terminal": TERMINAL, "state": "completed",
    ...
    write_json(art / "source-authentication.json", {
        "schema": "jass.cls_l_2027_failed_source_authentication.v1", "authenticated": True,
    ...
    summary = {
        "schema": "jass.cls_l_2027_failure_diagnostic_summary.v1", "terminal": TERMINAL, "state": "completed",
        "classification": "TECHNICAL_DIAGNOSTIC", "scientific_verdict": None,
        ...
        "next_stage": "REPAIR_PROVEN_2027_MECHANICS_ONLY"}
    runtime_atomic_json(art / "scientific-summary.json", summary)
    write_json(art / "manifest.json", {"schema": "jass.cls_l_2027_failure_diagnostic_manifest.v1",
    ...
    (art / "RESULTS.md").write_text(
        f"# CLS-L 2027 technical failure diagnostic\n\n- terminal: `{TERMINAL}`\n"
