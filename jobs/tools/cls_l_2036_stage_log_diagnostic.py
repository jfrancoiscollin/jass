#!/usr/bin/env python3
"""Recover bounded Launch-V2 stage logs for failed CLS-L fit rehearsal 2036.
...
FAILED_JOB = "cpx62-2036-l3-cls-l-three-arm-fit-rehearsal-v1"
FAILED_ATTEMPT = "20260917T183209Z-4b46d3fc"
FAILED_CODE = "4b46d3fc37c631ec46b4b3173ead4c7f719e31ff"
FAILED_PREFIX = f"r2:jass-data/runs/{FAILED_JOB}/{FAILED_ATTEMPT}"
TERMINAL = "CLS_L_2036_STAGE_LOG_DIAGNOSTIC_COMPLETE_V1"
PHASE = "execute-cls-l-2036-stage-log-diagnostic"
...
def main() -> int:
    ...
    work = result_dir / "work" / "failed-2036-stage-logs"
    ...
    if (
        ...
        raise RuntimeError("failed-2036 identity/state drift")
    ...
    if attempt.get("classification") != "TECHNICAL" or attempt.get("scientific_verdict") is not None:
        raise RuntimeError("2036 was not a quarantined technical failure")
    ...
    diagnostic = {
        "schema": "jass.cls_l_2036_stage_log_diagnostic.v1",
        "terminal": TERMINAL,
        "state": "completed",
    ...
    write_json(
        ...
        {
            "schema": "jass.cls_l_2036_stage_log_authentication.v1",
            "authenticated": True,
            "job_id": FAILED_JOB,
            "attempt_id": FAILED_ATTEMPT,
            "code_sha": FAILED_CODE,
    ...
    summary = {
        "schema": "jass.cls_l_2036_stage_log_diagnostic_summary.v1",
        "terminal": TERMINAL,
        "state": "completed",
        "classification": "TECHNICAL_DIAGNOSTIC",
        "scientific_verdict": None,
        ...
        "promotions": 0,
        "bakes": 0,
        "next_stage": "REPAIR_PROVEN_2036_MECHANICS_ONLY" if primary else "RECOVER_PRE_STAGE_LAUNCHER_EVIDENCE",
    }
    runtime_atomic_json(art / "scientific-summary.json", summary)
    write_json(
        art / "manifest.json",
        {
            "schema": "jass.cls_l_2036_stage_log_diagnostic_manifest.v1",
            "terminal": TERMINAL,
            "failed_job_id": FAILED_JOB,
            "failed_attempt_id": FAILED_ATTEMPT,
    ...
    (art / "RESULTS.md").write_text(
        f"# CLS-L 2036 stage-log diagnostic\n\n"
        f"- terminal: `{TERMINAL}`\n"
