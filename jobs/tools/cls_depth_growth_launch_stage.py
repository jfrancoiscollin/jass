#!/usr/bin/env python3
"""Launch-V2 evidence wrapper for the CLS-D depth/growth rehearsal."""
...
from jobs.tools import cls_depth_growth_stage as stage  # noqa: E402
from jobs.tools.launch_runtime_v2 import StageEvidence, atomic_json  # noqa: E402

PHASES = ["execute-rehearsal"]
...
def main() -> int:
    ...
    try:
        ...
        summary = stage.run_stage(result / "cls-depth-growth-work", art)
        ...
        atomic_json(art / "scientific-summary.json", {
            "schema": stage.SCHEMA,
            "state": "failed",
            "terminal": "CLS_DEPTH_GROWTH_TECHNICAL_FAILURE_V1",
        ...
        print(f"CLS-D TECHNICAL: {type(exc).__name__}", file=sys.stderr)
        return 2
