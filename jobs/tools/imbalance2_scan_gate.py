...
def pov(result: object, side: str) -> str:
...
def run(args: argparse.Namespace) -> int:
    ...
    candidate = cv.JassEngine(args.jass, label="candidate", pattern_path=args.candidate,
                              search_params=args.search_params)
    scan = cv.ScanEngine(args.scan, label="Scan", no_book=True, bb_size=args.scan_bb_size)
    defender_candidate = cv.JassEngine(args.jass, label="defender-candidate",
                                       pattern_path=args.defender,
                                       search_params=args.defender_search_params)
    defender_scan = cv.JassEngine(args.jass, label="defender-scan",
                                  pattern_path=args.defender,
                                  search_params=args.defender_search_params)
    ...
    try:
        for index, (rec, m) in enumerate(zip(records, meta, strict=True)):
            ...
            play = {"movetime": args.movetime} if args.movetime is not None else {"depth": args.depth}
            c_white, c_black = ((candidate, defender_candidate) if advantaged == "W"
                                else (defender_candidate, candidate))
            s_white, s_black = ((scan, defender_scan) if advantaged == "W"
                                else (defender_scan, scan))
            try:
                c_result = cv.play_game(c_white, c_black, referee, fen,
                                        max_plies=args.max_plies, **play)
                s_result = cv.play_game(s_white, s_black, referee, fen,
                                        max_plies=args.max_plies, **play)
                rows.append({
                    "index": index,
                    "stratum": m["stratum"],
                    "candidate": pov(c_result, advantaged),
                    "scan": pov(s_result, advantaged),
                    "candidate_reason": c_result.reason,
                    "scan_reason": s_result.reason,
                })
            except Exception as exc:  # one source position must not invalidate the shard
                rows.append({"index": index, "stratum": m["stratum"], "error": str(exc)})
    finally:
        for engine in (candidate, scan, defender_candidate, defender_scan, referee):
            try:
                engine.close()
            except Exception:
                pass

    payload = {"schema": 1, "pool": args.pool, "shard": args.shard,
               "nshards": args.nshards, "rows": rows}
...
def paired_bootstrap(rows: list[dict[str, object]], reps: int, seed: int) -> dict[str, list[float]]:
    ...
    for cat, values in samples.items():
        values.sort()
        out[cat] = [values[int(0.025 * (reps - 1))], values[int(0.975 * (reps - 1))]]
    return out
...
def aggregate(args: argparse.Namespace) -> int:
    ...
    global_pass = all(abs(delta[c]) <= args.global_point_margin for c in CATS) and all(
        ci[c][0] >= -args.global_ci_margin and ci[c][1] <= args.global_ci_margin for c in CATS)
    ...
    for name in sorted(by_stratum, key=lambda s: int(s.split("v", 1)[0])):
        ...
        strata_pass &= passed
        strata[name] = {"n": len(group), "candidate": c, "scan": s, "delta": d, "pass": passed}
...
def main() -> int:
    ...
    r.add_argument("--jass", required=True); r.add_argument("--scan", required=True)
    r.add_argument("--candidate", required=True); r.add_argument("--defender", required=True)
    r.add_argument("--pool", required=True); r.add_argument("--meta", required=True)
    r.add_argument("--search-params"); r.add_argument("--defender-search-params")
    r.add_argument("--calibrate-tool", default="jobs/tools/calibrate_vs_scan.py")
    budget = r.add_mutually_exclusive_group(); budget.add_argument("--depth", type=int, default=8); budget.add_argument("--movetime", type=float)
    r.add_argument("--max-plies", type=int, default=260); r.add_argument("--scan-bb-size", type=int, default=0)
    r.add_argument("--shard", type=int, default=0); r.add_argument("--nshards", type=int, default=1)
    r.add_argument("--out", required=True); r.set_defaults(func=run)

    a = sub.add_parser("aggregate")
    a.add_argument("--inputs", nargs="+", required=True); a.add_argument("--out", required=True)
    a.add_argument("--bootstrap", type=int, default=5000); a.add_argument("--seed", type=int, default=271828)
    ...
    a.add_argument("--min-per-stratum", type=int, default=20); a.add_argument("--max-errors", type=int, default=0)
