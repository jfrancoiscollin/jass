...
> **Version : 4.1 — 18 juillet 2026**
> **Statut : quatre cellules C1-Q1 entraînées ; verdict contractuel v2 préparé, non lancé**
> **Ancienne version :** [L3_PURE_PLAN_V4_0_20260718.md](archives/l3/L3_PURE_PLAN_V4_0_20260718.md)
...
Les quatre cellules primaires ont été exécutées jusqu'à G2 sous les jobs
`0802/0804/0805/0803`. Le job de verdict historique `0806` reste un diagnostic :
il a utilisé une common-search partielle en forcing/promo 6, sans vue native ni
traces positionnelles appariables. Le verdict normatif est donc rejoué sans
entraînement sur les quatre artefacts G2 immuables, selon §7.
...
1. **conversion fixe** : même pool WDL-grounded, même défenseur, global et P1–P4.
   Candidat et défenseur utilisent le fingerprint Q00 complet ; chaque sortie
   conserve `{index, résultat}` pour les statistiques appariées ;
2. **common-search** : poids comparés sous le fingerprint Q00 complet de 63
   clés, forcing/promo/menace/sacs à zéro, pour isoler ce que l'évaluation a
   appris ;
3. **native-search à temps égal** : chaque cellule utilise son fingerprint
   complet face au fingerprint complet de Q00, avec le même movetime des deux
   côtés, pour mesurer le produit réellement jouable.

Une chaîne partielle, un seul fingerprint pour deux côtés natifs, une clé
héritée, ou l'absence d'une des trois vues donne `invalid_science`. Le verdict
Q1 utilise un bootstrap **apparié par index de position** pour les deltas P1–P4,
le global et les trois contrastes factoriels. Un timeout peut être exclu de la
paire, mais l'ensemble des index source doit être identique et complètement
comptabilisé dans les quatre cellules.
...
- la branche coût n'est disponible qu'avec une mesure pré-enregistrée,
  comparable, sur les mêmes positions et le même budget. Sans cette preuve,
  elle vaut faux.
...
Le verdict ne lance aucun job. Un lead passe d'abord en confirmation ; Q2 n'est
copié en GitOps qu'après lecture humaine du rapport, calibration, SHA mergé et
go explicite.
...
## 9. PR du fork et correction du verdict
...
La PR corrective post-Q1 ajoute, sans modifier les quatre entraînements :

- fingerprints distincts A/B et budget movetime dans les runners de gate ;
- fingerprints candidat/défenseur et résultats par index dans la conversion ;
- agrégation schema 2 stricte, puis bootstrap apparié et guards P3/P1–P4 ;
- gates common-search Q00 complet et native-search à temps égal ;
- wrapper v2 préparé hors queue, bloqué sans `EXPECTED_CODE_SHA` et
  `FULL_RUN_APPROVED=1`, avec `automatic_next_job=null`.
