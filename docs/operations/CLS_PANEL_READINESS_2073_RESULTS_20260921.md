...
**81 launch profiles** validated. The complete 78-test Linux suite passed on CPX62 on 21 September with inherited
`RCLONE_BIN=/usr/bin/rclone`, without skips. This includes the exact six-module
launch suite and the raw-audit tests. A complete detached checkout at `12b922e1`
with the two test fixes was used; a preliminary incomplete staging archive was
rejected for missing Git provenance and publisher imports. GitHub CI is pending.
