...
| TI-020 | ED1 1873 / 20260908T140143Z-a3ac3cdc — partial-order label audit launcher | TECHNICAL: stage exit 1 / runner exit 2 before cohort authentication; only a generic 50-byte RESULTS and no scientific verdict, label seal or audit readout. | ED1 exports OMP_NUM_THREADS=1 before checking nproc==16. GNU nproc observes OpenMP overrides, so the single-thread numeric cap contaminates the machine-resource probe. The original guard reproduces exit 1 locally; sparse failure logging obscured the offending check. | Probe available CPUs with only OMP_NUM_THREADS and OMP_THREAD_LIMIT removed from that subprocess, never nproc --all. Keep host=cpx62, available CPUs=16, clean detached pinned code and all numeric library thread caps=1. Preserve phase/line/exit diagnostics and all ED1 scientific parameters byte-identically. | Failed receipt: jass-control/status/cpx62-1873-l3-ed1-partial-order-audit-v1.json, attempt 20260908T140143Z-a3ac3cdc. Six actual-wrapper tests pass locally; original guard fails under OMP_NUM_THREADS=1. Scientific wrapper suffix authenticated against blob dddc4b1567472557436d615624dfa375f15e8282. Remote terminal rerun proof pending.; PR #870 | MITIGATED — terminal deterministic rerun proof pending |
