...
> **SONDE MULTI-TOURS (fork (a), chaîne #337)** : **T2 `ccx33-0762` PROMOTE** (1 h 59 ; gate vs parent-T1 0.4725 IC[0.433-0.512] pass, vs T0 0.49 IC[0.45-0.53] pass, conversion globale 0.657 ≈ plat vs T1 0.667 ; p4_egal 0.565 ↑, p3_mince 0.494 ↓ ; SHAs de filiation réels dans promotion.json). Lecture provisoire : pas de composition visible à T2 (cohérent plafond 1-tour 0726) — **T3 en cours** clôt la sonde (`complete_probe`). Fix chaîne gravé : l'inventaire réel d'un run contient des fichiers 0 octet (`job.lock`) — fetcher corrigé.
