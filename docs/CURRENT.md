...
## 📦 RELAIS DILF — CORPUS PC BLUES : A2 DISPONIBLE (2026-07-12, 1re livraison au fil de l'eau)
> La raffinerie dilf (`dilf/scripts/pcblues/`, contrat = section **EXPORTS d'INTEROP.md dilf**) livre dans `dilf/data/exports/pcblues/` (branche `claude/pcblues-corpus-extraction-2i92bj`) : **J1** `corpus_manifest.json` (60 fiches, 10 165 p. ; découvertes : le gisement combos passe de 5 à **8 volumes** — deel 56 = BK kombinaties 3, 51/58 = Klubkompetitie NL 17-19) ; **J2/A2** `pcblues_combos.jsonl` tag **`pcblues-a2-v1`** = **720 combinaisons certifiées-jouées** (BK België 1934-2013, deel 15+23), **100% `verified=true`** par re-jeu FMJD complet depuis ancre diagramme-pixel (rien ne sort sans), `position_hash` = clé de dédup croisée **à faire côté ingestion jass** (vs master-2000/0464 jnnw), thèmes ALL_DETECTORS. Quarantaine diagnostiquée : 312 séquences (25,6%/33,5% par vol., heuristiques coquilles §4.13 = prochaine passe dilf). Consommation jass : enrichissement combos du gen MAINLINE (conversion jnnw côté jass) + sous-ensemble figé thermomètre. ⛔ **JAMAIS dans le corpus from-scratch** (instruments seulement) ; ⛔ Result ≠ label WDL. A1 (PDN élite), A3 (prefs graduées !/?), A4 (QA finales), A5 (tests) = schémas gelés, extraction à venir (J3-J9).
