...
> **3 pièges infra diagnostiqués/corrigés (gravés)** : ① **R2 501 intermittent par requête** — `rclone cat`/probe rc=0 VIDE sur objet absent (faux « completed » 0732) ; multipart/`copyto --ignore-times` multiplient l'exposition → **pattern publisher runner = `copy --checksum --immutable` + retries** (absorbe le 501-but-stored) ; ② **PrivateTmp+oneshot** : le /tmp privé est démonté à l'exit du service → gcc/cmake des jobs détachés ENOENT → **runner exporte `TMPDIR=<run_dir>/tmp`** (develop `d52b5e9`) ; ③ **merge shards** : glob `sp.*` attrapait les logs `sp.N.log` → merge fullmatch préfixe+numéro (`6d90e72`).
> **🧹 REWRITE HISTORIQUE EXÉCUTÉ (`cpx62-0767`, 59 min, go JFC)** : main+develop **réécrits atomiquement** (force-with-lease, staging refs conservées) — **864 chemins / 9,35 GiB d'artefacts générés retirés de l'historique** (sélection audit v8, préfixes critiques exclus). Rollback complet : bundle git pré-rewrite **1,89 GiB en 4 parts vérifiées sha256** (`historical/git-bundles/…f5410cbf7c38.bundle.parts`) + snapshot tips. Nouveaux tips : develop `1b907771`, main `80e66b56`. Gotcha réparé post-hoc : `worktree add <ref-complète>` = HEAD détaché → les commits de garde (.gitignore artefacts + `docs/HISTORICAL_DATA_R2.md` + `tools/r2_restore_historical.py`) avaient été perdus, re-committés sur develop. Pièges gravés en plus des 3 du matin : ④ `git fetch --prune` + refspecs explicites SUPPRIME les refs de destination au re-run ; ⑤ `rclone lsf --format sp` sépare par `;` (pas espace) ; ⑥ `MIRROR`/vars non exportées → `--git-dir ''` = faux « not ancestor ».
> **SONDE MULTI-TOURS (fork (a), chaîne #337)** : **T2 `ccx33-0762` PROMOTE** (1 h 59 ; gate vs parent-T1 0.4725 IC[0.433-0.512] pass, vs T0 0.49 IC[0.45-0.53] pass, conversion globale 0.657 ≈ plat vs T1 0.667 ; p4_egal 0.565 ↑, p3_mince 0.494 ↓ ; SHAs de filiation réels dans promotion.json). Lecture provisoire : pas de composition visible à T2 (cohérent plafond 1-tour 0726) — **T3 `ccx33-0769` PROMOTE/`complete_probe`** (2 h 07 ; vs parent-T2 0.4967 IC[0.457-0.537], vs T0 0.5033 IC[0.463-0.543], conversion 0.669 : p1 0.841/p2 0.609/p3 0.489/p4 0.513). **⛔ VERDICT SONDE : le fork (a) NE COMPOSE PAS** — conversion T1→T2→T3 = 0.667/0.657/0.669 (plate), gates vs T0 = 0.5125/0.490/0.5033 (plats) : 3 tours de labels adjud n'accumulent RIEN → le plafond 1-tour (0726) est un plafond tout court pour la recette bootstrap-fort+adjud+gymnase-léger. Chaîne mécaniquement irréprochable (filiation SHA vérifiée aux 3 tours) = le signal est propre. **Suite à arbitrer (JFC) : fork (c) repenser le départ, ou étape (6) sparring-vs-Scan.** Fix chaîne gravé : l'inventaire réel d'un run contient des fichiers 0 octet (`job.lock`) — fetcher corrigé.
> **⛔ CAUSAL-TEACHER CLOS — smoke `complete_no_signal` (0777, 2026-07-18)** : mining des trajectoires T3 (1381 parents causaux, 2490 parties, 210k transitions ; siblings certifiés d14+egdb) → 3 bras B1/B2/B3 vs A sur hard-conversion : **B1 −0.010 / B2 +0.007 / B3 −0.019**, tous sous le seuil pré-engagé **+0.02**, tous `eligible:false` → **aucun bras retenu**. La chaîne de confirmation P3 (0781/0782) N'EST PAS soumise (matrice #342). Le prong « teacher causal » rejoint fork(a)/fork(c) : **ne bouge pas la conversion.** Machinerie #342 validée (runner exit-code fix + résumés GitOps live). **⟹ 3 pistes closes le même jour (a/c/teacher) : le plafond conversion ~0.66 ne cède à aucune variation de la famille "re-fit sur données auto-générées + oracle". Seul non-testé = sparring-vs-Scan (labels = vraies parties vs plus fort), Phase 0 smoke en cours (`cpx62-0784`).**
> **⛔ FORK (c) CLOS — C0 `stop_regression` (2026-07-17, détail : [`docs/forkc_c0_verdict_20260717.md`](forkc_c0_verdict_20260717.md))** : départ affaibli **0.3× le fort** (men0.3/king0.9, bundle `forkc-weak-v1`) → gate C0 `cpx62-0774` **reject** : refit-faible vs fort **elo −32.5 ci_high 0.493<0.5 = régression établie**, **hard-conv delta −0.011** (cible +0.02 = conversion NE monte PAS), policy-divergence 5.5→13.7 % réelle mais découplée de la force. Scaler le matériel du départ ne dé-sature pas → **fork (c) mort**. **⟹ vrai pivot = étape (6) sparring-vs-Scan** (labels = résultat réel vs Scan), hors famille « re-fit depuis un départ matériel ». (2 pistes fork-c parallèles réconciliées : la piste Codex #340 pilote ; mon bundle 0773 = 0.3× exact, a servi le C0.)
> **🧹 FIN DE MIGRATION #331 (2026-07-17 soir)** : `develop` = branche par défaut ✓ ; **`main` SUPPRIMÉE** + 10 branches de PRs mergées (job 0771, rollback = `migration/clean-*` + bundle R2) ; ~146 branches historiques non-mergées conservées (épinglent l'ancien historique — second tri optionnel sur go JFC).
> **⭐ T1-bis ADJ+G1 natif `ccx33-0756` (launcher #336, science 0729/0741 inchangée, 1 h 16)** : gen 300 parties d10 → **206 648 pos, G1=3 848** ; relabel d14+egdb strict ; fit ancré 0.05 ; **conversion WDL-grounded globale 0.667** (p1_net 0.836 / p2_moyen 0.574 / p3_mince 0.517 / p4_egal 0.530, 862 décisives, 0 erreur) ; **gate vs parent(=réf fixe) 0.5125 n=600 IC[0.473-0.552] (+8,7 Elo, non-régression)** → **`promotion_decision=promote`, `scientific_status=continue_probe`**. Cohérent 0726 : labels adjud = petit gain généraliste, conversion ≈ plafond 1-tour (0.66) — la suite est la CAMPAGNE multi-tours (fork (a)). Candidat : `runs/ccx33-0756-…/artefacts/candidate.pjtw.gz`. **Ancre runtime ré-mesurée : gen d10 ≈ 1 min/300 parties (≠ 45 s/partie movetime) ; T1-bis complet ≈ 1 h 15 sur ccx33.**

## 🔚 VERDICTS 2026-07-13 SOIR — LIGNE SEARCH CLOSE (O falsifié), PIVOT ORACLES L3
> **0675 CLOS (0702, wash).** Gate cand(adjud-tenu 4/24, fit 0675 déjà payé — le FIT avait tourné, seul le compose-gate était mort sur le bug n=0 openings ; consommé tel quel, fix openings=dilf_combinations) vs T2 : **W=448 L=442 D=310 n=1200 rate=0.5025 ±0.028 elo~+2 IC=[0.474,0.531] = WASH.** Tenir l'adjud depuis T2 ne récupère RIEN ⟹ **aucune preuve du fade-adjud (b)** ; cohérent avec **(a) d10 pratiquement épuisé** (sans le trancher à 99 %). **Décision JFC : clore 0675 tel quel** — pas d'or caché dans le timing de l'adjud, pas de haut-N a/b (point à +2 ne vaut pas ~3 h). Pour monter : **d12/R2**, pas l'adjud. **d10 = plafond pratique de L2.**
> **⭐ LEVIER O (offer-no-reduce) FALSIFIÉ AVANT CONSTRUCTION (0703, profil détection D).** Instrument `tools/thermo_detection.py` (bestmove jass `--search-profile` TT-froid + Scan HUB à budgets croissants vs 1er coup certifié ; séquence K=2,3 via referee ; branche corpus). Sur les 224 combos figées : **détection 1er coup jass K1=0.31 / K3=0.81 ≈ Scan K1=0.35 / K3=0.88 (quasi identiques)**, **plate avec le budget** (jass d7=0.36→d13=0.33 ; Scan idem). Or conversion 0688 = **jass 0.136 vs Scan 0.904**. Deux faits tranchent : (1) **jass ≈ Scan sur toute la détection** — si le LMR de jass enterrait les offres calmes, jass serait très en dessous de Scan au K=1 ; il est à égalité ⟹ **prémisse « offres enterrées par réduction » FALSIFIÉE** ; (2) **plus de budget ne lève la détection pour aucun des deux** (le 1er coup certifié n'est pas le coup *uniquement* préféré ; K=2/3 remonte car captures forcées). ⟹ jass **trouve** les combinaisons aussi bien que Scan mais **convertit** 0.136 vs 0.904 : **le trou est en AVAL (conversion/technique de finale), PAS la détection/search** = territoire A4/TB/oracles L3. **O attaquerait un non-problème → retiré.** *(auto-label du job a dit « rediagnostic » — corrigé : il ne regardait que jass-absolu, pas le contraste jass≈Scan qui est le vrai signal.)*
> **⟹ LIGNE ÉVAL+SEARCH DU CORPUS DÉFINITIVEMENT CLOSE** : B1(ensemencement)/B2(dense-NPS)/B3(cuts)/B4(exceptions-TB) tous neutres-négatifs **+ O falsifié par D** = 5 leviers morts. La valeur du corpus PC Blues est **oracles/QA + conversion**, jamais l'éval ni le search.
> **P2 MORT comme prédicat statique (0701).** `blocage_structurel` : **précision ≤7p = 5,26 % (1/19), sous-puissant** — un test STATIQUE ne monte pas à 99,9 %. **Théorème : le remède ply-cap n'est plus un prédicat mais un ORACLE de recherche = arbitre-d14-au-cap** (position cappée adjugée par recherche profonde + TB-probe, `--deep-relabel 14 --egdb` = exact si TB atteignable sinon signe d14). Coût borné (~19 % des parties × 1 recherche), vérité incomparable, zéro concept neuf (« le budget dit vrai »). C'est par là qu'on nettoie le ply-cap pour L3.
> **P1 (percée) VETO — L'ÉTAGE VERDICTS TOMBE (0706, 2026-07-14).** Fire-targeting endgames (min-pieces 4) a réparé la sous-puissance 0701 : **829 908 fires** (≤7p=133 914 TB-exact, >7p=695 994). **Précision ≤7p (TB EXACT) = 85,01 %** (WIN 113 842 / nulle 5 986 / LOSS 14 086) — n=133k = **décisif**, pas sous-puissant. La percée « imprenable » MENT 15 % du temps, dont **14 086 positions où le camp désigné PERD** (réfutation tactique dynamique invisible au prédicat statique). **Même théorème que P2 : un prédicat STATIQUE ne peut pas adjuger à 99,9 %** → P1 rejeté comme verdict. (>7p d14 = 79,2 %, cohérent.) **⟹ L'étage verdicts N'EXISTE PAS. Pile L3 stabilisée sur sa FORME MINIMALE (déjà solide) : TB + arbitre-d14-au-cap + escalier(conv_self) + vetos.** Ça VALIDE la direction (pas de raccourci-prédicat, on s'appuie sur les oracles de recherche). **B2 (gymnase de conversion) devient LA pièce centrale** — la conversion est le vrai levier (trou 0703).
> **⚙️ 4 BLOQUANTS L3 — CODE COMPLET + GATES (2026-07-14, branche corpus ; develop intact)** — les pièces qui font passer L3 de « L2-avec-bootstrap » (plafonne, 0702) à une lignée qui ENSEIGNE la conversion. **B4** ✅ `make_bootstrap_eval.py` + `bootstrap.pjtw` : eval SEED matériel-conscient (black-POV, patterns=0) — gate 0707 : **écrase zero 0.9896** + eval matériellement correct (les 2 « échecs » du gate = artefacts : assertion POV + test_scan_book pré-existant). **B3** ✅ `conv_self.py` v2 **défenseur-fixe** — gate 0711 **ADMIS** : conv_self monotone (zero 0.53 < gen2 0.62 ; v1 self-play était confondu, anti-corrélé). ⭐ **Révèle le trou 0703** : gen2 ne convertit que **62 %** des positions TB-gagnées. **B1** ✅ arbitre-d14-au-cap (`cap_costing.py` + `scan_selfplay_gen --cap-arbiter d14`) — chiffrage 0710/0712/0713 : **plycap≈0 même en comptant les 25-move**, coût_d14=0,025 s ⟹ arbitre **cheap** (surcoût≈0, ADMIS) mais phénomène **rare** dès qu'un pilote matériel-conscient (le bootstrap) joue → le « ~19 % » était le régime tour-0 aveugle que **B4 fait éviter à L3**. **B2** ✅ gymnase TB-adjacent (`build_conversion_pool.py` + `conversion_pool.fen` = 147 positions GAGNÉES certifiées-TB, ∩ thermo-224=∅ + `scan_selfplay_gen --seed-pool/--seed-frac`, pairing couleurs) — labels exacts par TB-terminate (échappe au piège B1-ensemencement). **Intégration gen commune B1+B2 dans `scan_selfplay_gen` (flags OFF = backward-compat), validée offline.** ⟹ **P1 VETO confirme la stratégie** : pas d'étage-verdicts, la CONVERSION (B2) est le seul vrai levier ; l'arbitre (B1) est assurance quasi-gratuite. **SMOKE CHAÎNE L3 `cpx62-0714` FINI (2026-07-14) — MACHINERIE ✓, PAS DE COMPOSE À T1.** Manifest T0=bootstrap→T1 : positions=3,4M ; **pool_games=294 ✓** (gymnase tire) ; **cap_arbiter_fires=106 ✓** (arbitre tire, rare = cohérent B1) ; **conv_self 0.50→0.56 (Δ+0.065) ✓ LA CONVERSION MONTE** (le gymnase enseigne le trou 0703) ; MAIS **gate cand vs bootstrap = rate 0.436 IC[0.408,0.464] elo −45 ✗ cand PERD**. Tous les compteurs d'oracle tirent = **machinerie L3 validée de bout en bout**. Diagnostic du non-compose : le fit **échange du milieu contre de la conversion** — le bootstrap est déjà fort en matériel (B4), son self-play produit des milieux positionnellement NAÏFS → labels WDL milieu faibles → fit dégrade le milieu pendant que le gymnase (labels TB-exacts) améliore la finale. **Problème classique du bootstrap** : sauter l'apprentissage-matériel via B4 marche, mais le POSITIONNEL ne s'apprend pas en 1 tour (la chaîne from-scratch composait à T2, pas T1). Pas un bug — un choix de recette. **FORK OUVERT (décision JFC)** : (a) campagne multi-tours T1→T3 ; (b) tuner la recette (anchor 0.10-0.15 plus serré / gymnase seed-frac 0.30) ; (c) repenser le départ (zero vs bootstrap-faible). **⟹ DIAGNOSTIC EN COURS (mémo JFC 2026-07-14)** : H1 = le retrait de l'adjud (design 0714) a rendu les labels ON-POLICY d'un pilote jeune ALÉATOIRES (bruit qui remplit les patterns partis de 0) — l'adjud ne servait pas qu'à éviter le ply-cap (plycap≈0, B1), elle rendait les ISSUES VRAIES (ce qui a fait composer L2). **D1 fit-ablation `cpx62-0715` FINI (2026-07-14) — H1 DÉPLACÉE, 3 findings.** Corpus T1 committé (anti-perte). Composition : ONP=3,39M (99.5%) / GYM=1349 (0.04%) / CAP=15740 → **oracle=17k (0.5%)**. 3 fits (anchor 0.05) vs bootstrap : **full −27** (conv 0.547), **oracle −26** (conv 0.539), **onp −47** (conv 0.630). D3 |Δw| : full/onp bougent l'**EXTRA (matériel/PST) de max 0.405** alors que les **patterns bougent à peine (~550 buckets)**. **LECTURE (3 findings)** : **(i) le VRAI coupable transversal = l'anchor 0.05 (trop lâche) ÉRODE le matériel hand-set du bootstrap** (D3, régresse toutes variantes oracle compris) — PAS « patterns remplis de bruit » (H1 réfutée sur ce point) ; **(ii) H1 confirmée en DIRECTION** (oracle −26 > on-policy −47 : labels oracle-vrais meilleurs, l'adjud aiderait l'on-policy) mais pas en absolu ; **(iii) le gymnase est AFFAMÉ (0.04% du corpus)** — `seed-frac` compte les PARTIES, mais les parties-gymnase sont des finales courtes → signal conversion noyé (bug recette : pondérer par position). **F-ANCHOR `cpx62-0716` FINI (2026-07-14) — (i) RÉFUTÉ COMME LEVIER : l'anchor N'AIDE PAS.** Re-fit du corpus T1 committé (AUCUNE regen) à anchor serré vs contrôle 0715 full@0.05 (conv 0.547 elo −27) : **full@0.15 conv 0.311 elo −292**, **full@0.30 conv 0.378 elo −220**, **oracle@0.15 conv 0.289 elo −34**. Serrer l'anchor FIGE le modèle près du bootstrap (patterns nz 26→13, il n'apprend plus) ⟹ elo s'effondre. Le contrôle (anchor le + LÂCHE) reste le meilleur. **⟹ l'érosion matérielle (i) N'ÉTAIT PAS le levier principal** (contre-intuitif vs D3 : le modèle a besoin d'APPRENDRE le positionnel, pas d'être bridé au matériel). **Leviers vivants restants : (ii) adjud/H1** (oracle −26 > onp −47 : labels oracle-vrais meilleurs) **et (iii) gymnase affamé** (0.04%). **SUITE = F1 adjud (ii) + nourrir le gymnase (iii).** **G4-TIP en cours (iii)** : le corpus T1 (3,4M) contient DÉJÀ **27 533 candidats tip uniques on-distribution** (marge-VALEUR 0/1 : p4_egal 15192, p3_mince 12341, ×6,5 le tip actuel ~4,2k) — AUCUNE perturbation synthétique. Outil `mine_conversion_pool` étendu (`--val-margin-max` extract tip, `--value-adv` filter garde marge0 en enseignant le gagnant = les gains valeur-égale purs que l'ancien filtre-pièces droppait). **eval stratifié conv_self FIGÉ committé** (0717 carve : 250/palier p1-p4, disjonction dure eval∩train=0 + ∩ancien-plat=0). **`ccx33-0718` FINI (~15 min) — tip GRAS.** 27 533 cand → certif d14+egdb decisive 88 % → **tip certifié 18 045** (p4_egal 12 789, p3_mince 5 256 : le matériel valeur-égale certifie bien près de la frontière TB). Fusion + re-carve → **training v2 = 52 682, eval stratifié v2 FIGÉ 1600 (400/palier), eval∩train=0**. Pyramide tip : **p4_egal 1900→12 681 (×6,7), p3_mince 1800→6052 (×3,4)** — gymnase nourri pour la conversion fine, jauge figée avant le 1er tour. **⚠️ ANCHOR = DIAG EN COURS, PAS UN VERDICT (mémo JFC 2026-07-14)** : le tableau 0716 VIOLE la monotonie de l'ancrage (serrer 0.05→0.15→0.30 = −27→−292→−220 = s'ÉLOIGNE de la référence). Code+T3 réfutent déjà **H-A** (référence fausse) : `wdl_finetune` charge w0=bootstrap COMPLET (EXTRA inclus), objectif `‖dw‖∝1/anchor` monotone vers w0, `|Δw|EXTRA` DÉCROÎT 0.405→0.223→0.139 (converge vers hand-set). **T4 confirmé** : anchor global vs data `/N` ⟹ λ incomparable entre corpus (oracle@0.15 −34 ≠ full@0.15 −292). Reste **H-B** (vallée Elo = matériel distordu sans compensation-patterns à λ moyen, retour ~0 à λ∞ — possible SANS bug). **`cpx62-0719` DIAG FINI (2026-07-14) — AUCUN bug, mais SMOKING-GUN d'ÉCHELLE trouvé.** SANITÉ boot-vs-boot=**0.5000** (harness SAIN) · POV-gate Spearman(X·w0,eval)=**0.9988** les 3 fits (référence alignée runtime = **H-A définitivement réfutée**) · **Elo(λ)** : 0.05:−27 · 0.15:−292 · 0.30:−220 · 1:−149 · **10:+16** · **100:+9** (→~0 quand λ↑ ⟹ la vallée 0.15-1.0 = **H-B non-convexité RÉELLE, PAS un bug**). **⭐ z-stats (identiques 3 fits) : `z0/T min=−5.25 max=+11.35 std=2.63` → σ SATURÉE** (σ(11.35)=0.99998 ; `wdl_finetune` flague « augmenter --logit-scale »). **LECTURE CLÉ** : à T=1 le bootstrap sature la logistique sur les positions matériellement tranchées — or les **échecs de conversion** (matériel haut mais NON gagné = trou 0703) sont EXACTEMENT là : bootstrap prédit σ→1, vrai label nulle/perte → CE énorme mais **gradient σ'(z)≈0 → le fit NE PEUT PAS apprendre à corriger la conversion**. **⟹ candidat sérieux du non-compose smoke (−45) : le fit tournait SATURÉ, aveugle aux positions-conversion** ; grossir le gymnase (0718) est inutile si le fit ne peut pas apprendre dessus. Fix mémo-T2b = 1 paramètre `--logit-scale T` (ou re-scaler bootstrap) pour dé-saturer. **`cpx62-0720` SWEEP LOGIT-SCALE FINI (2026-07-14) — dé-saturer NE débloque PAS ; le blocage = les LABELS.** Refit full@0.05 × T∈{1,4,8} sur jauge conv_self v2 FIGÉE (baseline bootstrap=0.355) : **T=1** z0/T_max 11.35, nz **551**, conv 0.340, gate **−279** (= la recette 0715 exacte) · **T=4** z0/T 2.84, nz **7**, conv 0.390, gate −148 · **T=8** z0/T 1.42, nz **0**, conv 0.328, gate −31. **MÉCANISME (contre-intuitif)** : le gradient data est `X·(p−y)/T` → augmenter T **DIVISE tout le signal** → le fit GÈLE (nz 551→7→**0**) et le candidat RETOMBE sur le bootstrap (elo→−31). conv_self ne monte pas de façon fiable (best +0.035 à T=4 mais nz=7 = quasi-gel = bruit n=397). **⟹ la saturation N'ÉTAIT PAS le bloqueur.** Le fit PEUT bouger (551 patterns à T=1) mais dans une **MAUVAISE DIRECTION** : à T=1 conv_self **RÉGRESSE** 0.355→0.340 et le généraliste s'effondre (−279). **⛔ DIAGNOSTIC BOUCLÉ — 3 pistes éliminées, une par job** : anchor (0716) ❌ · référence/harness (0719) ❌ sains · saturation/échelle (0720) ❌ dé-saturer gèle le fit. **⟹ Ce qui reste = la QUALITÉ DES LABELS (ii adjud), exactement ce que D1 pointait** (oracle −26 > on-policy −47). Le corpus T1 = self-play on-policy du bootstrap (fort matériel, NAÏF positionnel) → ses issues WDL d9 reflètent son jeu faible → les fitter RÉGRESSE l'éval ; le gymnase (0.5 %) trop dilué pour renverser. **⟹ F1 ADJUD (le « hold » du mémo est levé)** : relabéliser les issues on-policy via l'arbitre **d14+egdb** pour qu'elles soient VRAIES (pas le jeu naïf) + up-weight gymnase. **F1 ADJUD `cpx62-0721` FINI (2026-07-15) — LABELS = 67 % FAUX, adjud stoppe la régression mais ne suffit pas.** Même sous-échantillon (N=70-80k) fitté 2× : **ON-POLICY** conv 0.466 gate **−35** vs **ADJUD (d14+egdb)** conv 0.454 gate **+2**. **⭐ l'arbitre RENVERSE 67,1 % des labels** (46 965/70 000) — le bootstrap joue d9 et se trompe d'issue 2 fois sur 3 vs vérité d14+egdb : corpus massivement corrompu. **Effet-label isolé (même N/positions) = +37 elo** (−35→+2) : labels vrais STOPPENT la régression du généraliste (candidat ÉGALE le bootstrap). **MAIS (a) c'est un NUL (+2) pas un gain — labels propres = nécessaire, pas suffisant ; (b) conv_self ne monte pas** (0.454 vs 0.466 = bruit). **Bonus** : micro-sonde **d14+egdb = 0,033 s/pos même en milieu** → relabel du corpus ENTIER (3,4M) faisable ~2h. **⟹ 2 leviers confirmés, à COMBINER : (ii) adjud-full stoppe la régression + (iii) gymnase (0718) fournit le gain-conversion.** **⚠️ 2 DÉFAUTS MÉTHODO 0721 (critique JFC, confirmés code) : (1) le 67 % est INVALIDE** — le merge concatène les shards (tri lexical) sans dé-entrelacer → la stat compare des positions DIFFÉRENTES par index (le FIT reste valide, records auto-portants ; seule la stat est cassée) ; **(2) +37 elo CONFONDU** — 2 shards ont CRASHÉ (pas timeout) → contrôle 80k vs adjud 70k, garde n≥20k trop laxiste → une part du +37 vient du N réduit (moins de fit = moins mauvais, cf 0720), pas des labels. ⟹ **0721 = indication forte, PAS expérience isolée.** **CORRECTIFS APPLIQUÉS** : **A** — 6 outils L3 (`conv_self`, `make_bootstrap_eval`, `mine_conversion_pool`, `promotion_gate`, `build_conversion_pool`, `cap_costing`) **promus sur develop** (SHA `43ec5e14`, fin du code hybride multi-branches) ; **B** — job **corpus APPARIÉ** (2 fits sur exactement les mêmes positions survivantes) + diff **par-shard** (% valide) + log succès/shard ; **C** — manifeste enregistre le SHA develop. **`cpx62-0722` RECETTE CORRIGÉE FINI (2026-07-15) — la recette NE COMPOSE PAS : plafond à 1 tour.** 3 cellules APPARIÉES (N=105k, gym=31383, dev pinné 43ec5e14) : **on-policy** conv 0.488 gate **−46** · **adjud-W0** conv 0.458 gate **+3** · **adjud-W4** (gym×4=230k) conv 0.447 gate **−3**. **% renversé VALIDE = 61,5 %** (le vrai chiffre par-shard, remplace le 67 % cassé). **Effet-LABEL propre (adjud-W0 vs on-policy, MÊMES positions, sans confond N) = +49 elo** (−46→+3, conv −0.031) : le +37 de 0721 CONFIRMÉ ET RENFORCÉ — labels vrais réparent le généraliste. **MAIS effet-GYMNASE ×4 : conv −0.011, elo −6 → up-weight NE MONTE PAS la conversion.** Meilleure cellule (adjud-W0) = juste NUL vs bootstrap (+3), la conversion ne bouge JAMAIS. **⟹ ni labels ni gymnase ne produisent un GAIN à 1 tour ; le bootstrap-fort-naïf est un plafond 1-tour → pivot CAMPAGNE multi-tours / repenser le départ.** **⚠️ RÉSERVE MESURE** : conv_self de 0722 dérive le camp gagnant du MATÉRIEL (men=1,dame=3) → mesure MAL les p4_egal (matériel égal). **PR Codex #329 (DOE 2×2 `cpx62-0723`)** corrige ça : vrai plan factoriel (comble la 4ᵉ cellule onp_g4 → interaction labels×gymnase mesurable) + **conv WDL-grounded** (gagnant depuis label d14+egdb, pas matériel) + merge ordonné vérifié par-record + tests unitaires + SHAs. **Revue Claude** : PR excellente MAIS `assert-decisive` allait ABORT (0722 prouve ~40 % du pool relabélise NULLE via draw-band-50) → **3 correctifs poussés sur `agent/doe-labels-gym-2x2`** (filter-décisif au lieu d'asserter, desserrer abort conv strict, PAIRS=1). 0723 tranchera « vrai plafond » vs « artefact de mesure conv-matériel » (sa conv WDL-grounded peut révéler du signal p4_egal masqué). **PR #329 MERGÉE (squash 5cd2fee3) + 3 incidents runtime corrigés successivement : 0723 ABORT (OOM : 16 shards × cache-egdb 2048Mo = 32Go = RAM box → fix cache→512) · 0724 ABORT (conv_fixed_wdl : le moteur egdb meurt au startup de ~6/16 shards → cascade broken-pipe 35% → fix conv_fixed_wdl RESTART-ON-DEATH sur main) · `cpx62-0726` FINI rc=0 (DOE propre, conv robuste conv_n≈420/436).** ⭐ **`cpx62-0726` DOE 2×2 FINI (2026-07-15) — VERDICT DÉFINITIF, conv WDL-grounded : la conversion est un PLAFOND À 1 TOUR (pas un artefact), les LABELS sont le seul levier, le GYMNASE up-weight est un cul-de-sac.** 4 cellules appariées (base 120k, gym 35849 décisif ×1/×4, dev 43ec5e14) : **onp_g1** conv 0.673 gate −8.1 · **adj_g1** conv 0.671 gate **+18.6** [CI 0.492,0.561] · **onp_g4** conv 0.663 gate −29.0 · **adj_g4** conv 0.663 gate +16.2. Contrastes directs : LABEL +7.5(g1)/**+23.2**(g4) · GYMNASE −2.9(onp)/+5.8(adj). **(1) LABELS = LE levier** (cellules adjud +16 à +19 vs on-policy −8 à −29). **(2) GYMNASE ×4 = cul-de-sac** : baisse conv (−0.01), dégrade le gate sous on-policy (−29), interaction NULLE (conv +0.0016). **(3) ⭐ CONVERSION NE BOUGE PAS** — avec la métrique CORRECTE (WDL-grounded, mesure enfin les p4_egal) toutes les cellules ≈ **0.66** ; ni labels ni gymnase ne la montent ⟹ **l'hypothèse « signal p4_egal masqué par la métrique-matériel » est RÉFUTÉE, 0722 avait raison : plafond 1-tour RÉEL.** **Meilleure cellule = `adj_g1`** (adjud base + gymnase léger ×1) : gate +18.6 (petit gain généraliste réel, CI borne-basse 0.492 = quasi non-inférieur), conversion tenue. **⟹ RECETTE GYMNASE-LOURD ABANDONNÉE ; le seul vrai levier = labels adjud d14+egdb (~+18 généraliste/1 tour, zéro conversion). Prochaine étape = CAMPAGNE MULTI-TOURS à labels adjud** (fork (a) — la chaîne from-scratch composait à T2 : la conversion ne s'apprend pas en 1 tour mais peut s'accumuler). **Fork à trancher (JFC) : multi-tours adjud vs repenser le départ.** NB infra : runner cpx62 était très en retard (pas mort) — a tourné 0721 puis 0722 ; ccx33 doublon inoffensif. **Gotcha gravé : sur cette box 32Go, tout job d14+egdb shardé → `cache_mb × nshards < ~24Go` (512×16=8Go OK) ; et les moteurs egdb peuvent mourir au startup en masse-parallèle → tout harnais de jeu doit RESTARTER sur mort (fait pour conv_fixed_wdl ; jass_vs_jass_arch tient déjà).** Garde-fou gravé : cellule λ-énorme + z-stats saturation dans tout job de fit. NB : §III.4 « gen-sans-adjud » reste à réviser (l'adjud rend les issues vraies) — mais D3 dit que l'anchor est le 1er lever, pas l'adjud. NB : corpus 0714 non committé + sans label_src → regen forcé, instrumenté `scan_selfplay_gen --label-src-out` + committé (anti-perte). **F2 GRAVÉ (à appliquer quel que soit D1)** : `promotion_gate.py` — un tour n'est promu que si **conv_self ↑ (≥+0.03, pilote) ET généraliste NON-RÉGRESSIF hors-IC vs champion(t−1) (garde)**. Le cand smoke (conv↑ mais −45) est REJETÉ. La conversion pilote, le généraliste garde. (Le doc SPEC_MAITRESSE n'étant pas dans le repo, CURRENT fait foi côté code.) Design figé : gen = scan_selfplay_gen piloté bootstrap **sans adjud** (B4 remplace le stabilisateur adjud dès T0) ; escalier-adjud multi-tours = raffinement campagne. Seuil B1 +25 % (JFC).
...
**EXPLOITATION MAX CORPUS ORIGINAL (E1-E5, 2026-07-13, dilf main `91e9183`)** — décision JFC : exploiter À FOND, routage post-0691 (oracles/QA + RAG, ⛔ jamais prefs-éval). **73 → 802 positions certifiées re-jeu + ~6 100 passages RAG.** **E1** finales à l'échelle (port hypothèses-de-dames A4) : Dubois 4→66 (20 dames) + CID 1.The_endgame 211 (62 dames) = 277 endgames QA + seeds. **E2** Locks : 113 verrous → nourrit la notation P2-blocage. **E3** Goedemoed Exercise_2/3 : 412 combos. **E4** stratégie/cours → RAG : 13 volumes ~6 100 passages. **E5** systèmes = prose (déjà E4). Tout `verified_engine=false` → **revalidation d14+TB côté jass À FAIRE** avant gate dur (+ harnais notation P2-blocage sur les 113 verrous). Outillage dilf : `extract_volume_v3`, `extract_cid.py` (+détecteur goedemoed).
**VERDICTS BOX 2026-07-13 (B4 + reval TB + P2-blocage)** : **B4 (0698) NEUTRE** — cellule FINALE mt0.2 rate 0.5013 (+1 Elo, neutre) : le contingent exceptions-TB équilibré (200k, anchor 0.05) n'apporte pas de gain mesurable même en finale (généralistes n=0 = bug openings, FINALE seule valide mais pertinente). ⟹ **SÉRIE B CLOSE : B1(ensemencement)/B2(dense-NPS)/B3(cuts)/B4(exceptions-TB) tous neutres-négatifs → levier ÉVAL+SEARCH du corpus ÉPUISÉ (confirmé 4×)**. La valeur du corpus est aux ORACLES/QA, pas à l'éval. — **REVAL TB oracles (0699) : 85,3 % d'accord** verdict-livre vs TB (133/156 non-null ; cid_endgame 89,8 %, dubois 86,2 %) → **133 oracles engine-certifiés (`verified_engine=true`), ~23 quarantinés** (coquilles extraction/parse ou erreurs livre) ; le gate a fait son travail. Les 197 >7p = follow-up search-d14. — **P2-blocage (0700) : INCONCLUANT** — seulement 2 fires (`mutual_blocked` fire très rarement sur self-play gen2 ; puissance insuffisante) + ⚠️ le 1 fire ≤7p vérifié TB = FAUX POSITIF → `mutual_blocked` sur-fire possiblement (pas détecteur-nulle propre). ⟹ gen élargi (0701) pour trancher la precision.
