...
> • **CHAIN AUTONOME `0674`** (UN job qui itère les tours overnight, cap 6, E3 stop-2-plateaux) : **compose tour après tour**. **T2 = +170 Elo** vs champion(t−1) (rate 0.727 hors-IC, d9). adjud fade E1 marche (26%→39% nulles quand adjud→off), cov20 ~82% à 256k/tour. En **T4** au 2026-07-11 matin, champions committés `champion-tour{N}` + `champion-current`. **Chaque tour = génération 100% NEUVE on-policy (256k, champion(t−1) self-play) + fit ANCRÉ (`wdl_finetune --anchor 0.05`, poids capitalisent, jamais refit-zéro) + gate compose.**
> • ⟹ **L'éval grimpe zero → tour-N sans AUCUN prof externe** (analogue αβ d'AlphaZero : αβ_d10(éval)>éval, on entraîne l'éval sur les parties αβ_d10). La piste from-scratch est **VIVANTE et compose** (là où re-enseigner gen2-mmto était CLOSE).
