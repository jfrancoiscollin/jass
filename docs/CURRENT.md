...
> **⚙️ 4 BLOQUANTS L3 EN LIVRAISON (mémo 2026-07-14, offline develop/branche corpus)** — les pièces qui font passer L3 de « L2-avec-bootstrap » (plafonne, 0702) à une lignée qui ENSEIGNE la conversion : **B4** `make_bootstrap_eval.py` + `bootstrap.pjtw.gz` (eval SEED matériel-conscient, black-POV, patterns=0 ; gate `cpx62-0707` queué) ✓ ; **B3** `conv_self.py` shardable (P(vraie victoire|avantage≥3 pièces), lot témoin adjud-OFF = critère escalier ; gate `cpx62-0708` queué) ✓ ; **B1** ⭐ arbitre-d14-au-cap : `cap_costing.py` + chiffrage `ccx33-0709` queué (surcoût = plycap×coût_d14/coût_partie vs seuil +25% ; intégration flag `--cap-arbiter d14` SI admis) — cause du mensonge trouvée : `play_game` renvoie GameResult('D','ply cap') au max_plies ; **B2** ⭐ gymnase TB-adjacent = à faire (après verdict B1). Seuil B1 +25% (JFC).
