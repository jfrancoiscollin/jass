...
> **⏳ `0648` en cours EN PARALLÈLE** : **cpx62 = SCREEN** (bases `wdlft` sweep anchor {0.03,0.1,0.3} → A/B base-nue vs gen2-mmto mt0.2 : la calibration seule aide-t-elle ?) ; **ccx33 = FULL** (wdl_finetune anchor 0.1 → **MMTO last** gen-siblings WS-OFF + rank_finetune → candidat `gen4-wdlft-mmto` → A/B mt0.2+0.3). Verdicts attendus screen ~+2-2.5h / full ~+3.5-4h. **gen2-mmto reste champion.**
