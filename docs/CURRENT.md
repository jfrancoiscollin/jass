...
> **▶ PISTE 1 en cours** (choix JFC) : `wdl_finetune.py` (nouvel outil develop) = **fine-tune WDL ANCRÉ gen2-mmto** (garder la base, la calibrer sur le nouveau WDL : min CE + 0.5·anchor·‖w−w0‖², fit streamé espace-champion comme rank_finetune, seules colonnes visitées bougent). Smoke `0647` (slice 200k, POV/grad/logloss/charge, sweep anchor) → puis re-couplage complet (wdl_finetune sweep anchor + MMTO last layer + A/B vs gen2-mmto). **gen2-mmto reste champion.**
