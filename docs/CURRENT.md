...
**PLAN BOOST gen2-mmto (mémo JFC 2026-07-13, « A et A ») — jauge commune = thermomètre-224 (baseline gen2-mmto au trait 0.136)** : **B3a CLOS** (0657 asp30/knobs neutre haut-N, cf bloc DOE ci-dessus). **B1 étape-0 QUEUÉ `cpx62-0694-b1-seed-ab`** (main `44223ab3`) : screen d'ensemencement, A/B à variable unique — les 2 bras = même `scan_selfplay_gen` (pilote gen2-mmto self-asym d8/d3 → WDL game-outcome) + même `wdl_finetune` ancré gen2-mmto anchor 0.05 ; **seule diff = pool de graines** : bras A = base standard (corpus-mix2M ≥32p), bras B = base + **25 % combos pcblues** (16 160 ×2 couleurs-échangées, réplique `seed_frac=0.25`). Juge : compose A/B généraliste mt0.2+0.3 (candB vs candA décisif + candB vs gen2 ancre) + thermo-224 candB (guardé Scan). GATE : candB>candA hors-IC ⟹ l'ensemencement AJOUTE ⟹ scale B1 step-2 (gen 1-3M + finetune, go JFC). Est. ~5-7 h cpx62 (nuit), progress+artefacts au fil de l'eau. Règle 3 : bash -n + 5 heredocs py_compile + round-trip seed-pool (FEN/dames/miroir couleur) + round-trip thermo (format `game-*.json`) tous ✅. **B2 remap-dense → CHANTIER MOTEUR v9 (go JFC 2026-07-13)** — RATIONNEL : le champion pattern gen2-mmto est une table `int32[base^K]` par pattern (32×3^12 ≈ 17M buckets, ~136 Mo) mais **~740k buckets seulement sont vivants** (le reste épinglé à 0 faute de data, cf verdict couverture ≥5 visites = 99,7% du jeu). Le gather d'éval = **index DIRECT** `p.weights[idx]` → 32 lectures dispersées dans 136 Mo par position. HYPOTHÈSE : un format **dense (~16 Mo) tient en L2/L3 → moins de cache-miss → plus de NPS → plus de profondeur → Elo movetime**, à **éval byte-identique** (buckets occurrents gardent leur poids exact ; non-vus = 0 = fallback). ⚠️ **CAVEAT HONNÊTE (≠ mémo « zéro risque, pur gain »)** : le shrink impose une **map compacte** (`bucket→slot` ~740k entrées ; un remap-array plein = 68 Mo = pas de shrink) → **hash/binary-search au gather = +compute** ; et les buckets d'un même sous-arbre sont **corrélés** (working-set peut-être déjà cache-résident) ⟹ **le gain NPS est EMPIRIQUE, pas acquis**. PLAN (sur `develop`, jamais main pour le code) : (v8 déjà pris = embeddings+MLP) → **nouveau format JPAT v9 sparse** (`n_kept` + `[bucket:u32, poids:i32]×n_kept` par pattern ; load → hashmap open-addressing) + **converter v4→v9** (champion + census `bucket_census.py`) + **`--eval-selfcheck` prouvant byte-identique vs v4** (gate de CORRECTION, obligatoire) → **microbench NPS v4-vs-v9** (eval/s + search movetime, cpx62) qui **TRANCHE** si B2 vaut un bake. Aucun bake sans microbench NPS positif ET byte-id prouvé. **B3b** (re-DOE cuts) / **B4** (exceptions-TB équilibrées, corpus C4 dédup+50/50 → wdl_finetune gaté finale) = étapes suivantes gatées.
