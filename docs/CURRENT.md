...
> (règles permanentes), [ARBRE_DECISION.md](archives/ARBRE_DECISION.md) (principe). MAJ : **2026-07-13 soir** (verdicts en tête).
...
> **P1 (percée) = DERNIER CANDIDAT DE L'ÉTAGE VERDICTS — notation queuée (0704).** P1 échappe au théorème qui a tué P2 : la percée n'est pas une propriété statique subtile, c'est une **course de tempi calculée EXACTEMENT** (`adjud/predicates.py::p1_runaway_win`, déjà codé). Harnais `ccx33-0704` (queué, ETA ~1-1,5 h) : `p1_runaway_win` testé 2 camps → fires ; **précision P(WIN-camp|fire) : ≤7p egdb-relabel EXACT (LE GATE, ≥99,9 % n≥30) + >7p deep-relabel d14 (arbitre-au-cap)** ; fire-targeting endgames (min-pieces 4) pour réparer la sous-puissance 0701. **Lecture pré-engagée : ADMIS ⟹ l'étage verdicts EXISTE (pile L3 = TB + arbitre-d14-au-cap + P1 + escalier + vetos) ; VETO ⟹ étage tombe ⟹ pile L3 minimale (TB + d14-cap + escalier + vetos), déjà honorable.**
