...
> **Mise à jour v4** : les 60 volumes sont passés dans la raffinerie. **A2 = 20 763 combos vérifiées sur 56 volumes** (1 934 dup_of inter-volumes marqués — les compilations type deel 9=7+8 sont tracées) ; **A3 = 10 081 prefs graduées** (5 617 !/!! + 4 360 ?/?? négatives certifiées) ; **A5 = 960 tests** ; **A1 = 26 parties**. **A4 dispo en pilote** : détection de DAMES opérationnelle (rendu bleu, pièces double-empilées, validée par re-jeu) + claims déclaratifs du livre (`endgame_deel05.jsonl`, book_claim=true, prudence>volume — à élargir). Reste dilf : passe §4.13 sur les 30 volumes J9 (quarantaines diagnostiquées committées), élargissement A4, extract_combos sur 36/42. **jass peut ingérer par tag de version dès maintenant.**
