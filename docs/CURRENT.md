...
## 📦 RELAIS DILF — CORPUS PC BLUES : A1+A2+A3 DISPONIBLES (2026-07-12, livraison v2 au fil de l'eau)
> La raffinerie dilf (`dilf/scripts/pcblues/`, contrat = section **EXPORTS d'INTEROP.md dilf**) livre dans `dilf/data/exports/pcblues/` (branche `claude/pcblues-corpus-extraction-2i92bj`) : **J1** `corpus_manifest.json` (60 fiches, 10 165 p. ; le gisement combos passe de 5 à **8 volumes** — deel 56 = BK kombinaties 3, 51/58 = Klubkompetitie NL 17-19). **A2 `pcblues-a2-v1`** = **3 639 combinaisons certifiées-jouées** sur **9 volumes** (12, 15, 17, 23, 37, 43, 51, 56, 58), 100% `verified=true` par re-jeu FMJD complet depuis ancre diagramme-pixel, 188 récupérées par la passe coquilles §4.13 (solution unique, documentées), 1 510 en quarantaine diagnostiquée (~29%), `position_hash` = clé de dédup croisée **à faire côté ingestion jass** (master-2000/0464 jnnw), thèmes ALL_DETECTORS. **A1 `pcblues-a1-v1`** = `pcblues_games.pdn`, **25 parties complètes GMI/élite** (deel 36+42 : Hoogland, Weiss, Gantvarg, Andreiko…) rejouées intégralement depuis la position initiale. **A3 `pcblues-a3-v1`** = `pcblues_prefs_graded.jsonl`, **2 161 préférences graduées** (fen exacte avant coup) : **941 positives certifiées (!/!!)** + **1 195 NÉGATIVES certifiées (?/??)** — le signal inédit (ni Scan ni played-moves n'en donnent) pour `rank_finetune`. Consommation jass : enrichissement combos du gen MAINLINE (conversion jnnw côté jass), paires prefs, sous-ensemble figé thermomètre. ⛔ **JAMAIS dans le corpus from-scratch** (instruments seulement) ; ⛔ Result ≠ label WDL. Reste dilf : J4/J5 (A5 tests 47+57, A4 finales 5+49+30+60-62), deel 2 (Kaan, 411 p.), BK complets (A1 massif), 2e passe quarantaine.
...
> **🔎 DOE SEARCH `0652` (8 knobs OAT) : tout NEUTRE** — LMR/NMP/ProbCut/LMP/aspiration/RFP seuls vs défaut (baseline sanity +9). Seul lead : **asp30 (aspiration 50→30) = +17** (dans l'IC). `0657` round-2 en cours (confirm haut-N asp30 + combo asp30+lmr_log+nmp_r3 + asp20) — **seul levier vivant restant = le search** (convertir le d9 +34/+46 en movetime).
